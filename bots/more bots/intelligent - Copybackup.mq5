...
PairSettings g_pairSettings[MAX_PAIRS];
int g_pairCount = 0; // Track number of pairs currently in the array
...
struct CHOCHModifiedPosition {
   ...
   datetime modifiedTime;  // Time when the position was modified
};
...
//+------------------------------------------------------------------+
int EvaluateSignalQuality(int signal) {
...
//+------------------------------------------------------------------+
bool ProcessTradeSignal(int signal) {
   Print("[TRADE] Processing signal: ", signal);
   
   // Get pair-specific settings for current symbol
   string symbol = Symbol();
   int pairIndex = GetPairSettingsIndex(symbol);
...
void DetectOrderBlocks()
{
   ...
   double volatilityRatio = g_pairSettings[pairIndex].volatilityRatio;
...
//+------------------------------------------------------------------+
//| Detect CHOCH (Change of Character) patterns                      |
//+------------------------------------------------------------------+
void DetectCHOCHPatterns()
{
   string symbol = Symbol();
   int pairIndex = GetPairSettingsIndex(symbol);
   
   Print("[CHOCH] Starting CHOCH pattern detection for ", symbol);
   ...
   int swingWindow = IsHighValueAsset(symbol) ? 2 : 3; // Smaller window for high-value assets
   ...
   // Reset CHOCH patterns
   for(int i=0; i<MAX_CHOCHS; i++) {
      g_chochPatterns[i].valid = false;
   }
   
   int validChochs = 0;
   
   // Detect bullish CHOCH patterns
   // A bullish CHOCH occurs when price makes a lower low (swing low),
   // then rallies to break above the previous swing high
   if(swingLowCount >= 2) {
      for(int i=0; i<swingLowCount-1; i++) {
         int currentLow = swingLows[i];
         int previousLow = swingLows[i+1];
         
         // Check if current low is lower than previous low (downtrend)
         if(rates[currentLow].low < rates[previousLow].low) {
            // Find a swing high between these lows
            int midHighIndex = -1;
            for(int j=0; j<swingHighCount; j++) {
               if(swingHighs[j] > currentLow && swingHighs[j] < previousLow) {
                  midHighIndex = swingHighs[j];
                  break;
               }
            }
            
            if(midHighIndex >= 0) {
               // Check if price has broken above this swing high after making the lower low
               bool breakAbove = false;
               for(int j=0; j<currentLow; j++) {
                  if(rates[j].close > rates[midHighIndex].high) {
                     breakAbove = true;
                     break;
                  }
               }
               ...
               if(breakAbove) {
                  // We have a valid bullish CHOCH pattern
                  g_chochPatterns[validChochs].valid = true;
                  g_chochPatterns[validChochs].isBullish = true;
                  g_chochPatterns[validChochs].time = rates[currentLow].time;
                  g_chochPatterns[validChochs].price = rates[currentLow].low;
                  g_chochPatterns[validChochs].strength = 5; // Default strength
                  g_chochPatterns[validChochs].confirmed = false;
                  g_chochPatterns[validChochs].used = false;
                  
                  Print("[CHOCH] Detected bullish CHOCH at ", TimeToString(rates[currentLow].time), 
                        " price: ", DoubleToString(rates[currentLow].low, _Digits));
                  
                  validChochs++;
                  if(validChochs >= MAX_CHOCHS) break;
               }
            }
         }
      }
   ...
   // Detect bearish CHOCH patterns
   // A bearish CHOCH occurs when price makes a higher high (swing high),
   // then drops to break below the previous swing low
   if(swingHighCount >= 2 && validChochs < MAX_CHOCHS) {
      for(int i=0; i<swingHighCount-1; i++) {
         int currentHigh = swingHighs[i];
         int previousHigh = swingHighs[i+1];
         
         // Check if current high is higher than previous high (uptrend)
         if(rates[currentHigh].high > rates[previousHigh].high) {
            // Find a swing low between these highs
            int midLowIndex = -1;
            for(int j=0; j<swingLowCount; j++) {
               if(swingLows[j] > currentHigh && swingLows[j] < previousHigh) {
                  midLowIndex = swingLows[j];
                  break;
               }
            }
            
            if(midLowIndex >= 0) {
               // Check if price has broken below this swing low after making the higher high
               bool breakBelow = false;
               for(int j=0; j<currentHigh; j++) {
                  if(rates[j].close < rates[midLowIndex].low) {
                     breakBelow = true;
                     break;
                  }
               }
               ...
               if(breakBelow) {
                  // We have a valid bearish CHOCH pattern
                  g_chochPatterns[validChochs].valid = true;
                  g_chochPatterns[validChochs].isBullish = false;
                  g_chochPatterns[validChochs].time = rates[currentHigh].time;
                  g_chochPatterns[validChochs].price = rates[currentHigh].high;
                  g_chochPatterns[validChochs].strength = 5; // Default strength
                  g_chochPatterns[validChochs].confirmed = false;
                  g_chochPatterns[validChochs].used = false;
                  
                  Print("[CHOCH] Detected bearish CHOCH at ", TimeToString(rates[currentHigh].time), 
                        " price: ", DoubleToString(rates[currentHigh].high, _Digits));
   ...
   Print("[CHOCH] Found ", validChochs, " valid CHOCH patterns for ", symbol);
}
...
void ModifyStopsOnCHOCH()
{
   ...
   // Only proceed if we have a good success rate with CHOCH patterns
   if(g_pairSettings[pairIndex].chochSuccessRate < 0.4) {
      Print("[CHOCH] Skipping stop modification due to low success rate: ", 
            DoubleToString(g_pairSettings[pairIndex].chochSuccessRate, 2));
      return;
   }
   ...
   for(int i=0; i<totalPositions; i++) {
      ...
      // Find the most relevant CHOCH pattern for this position
      for(int j=0; j<MAX_CHOCHS; j++) {
         if(!g_chochPatterns[j].valid || g_chochPatterns[j].used) continue;
         
         // For buy positions, look for bullish CHOCH patterns to raise stop loss
         if(posType == POSITION_TYPE_BUY && g_chochPatterns[j].isBullish) {
            // Only modify if the CHOCH level is above the current stop loss and below the entry price
            double chochLevel = g_chochPatterns[j].price;
            
            if(chochLevel > currentSL && chochLevel < openPrice) {
               // Apply a small buffer for safety
               double newSL = chochLevel - (10 * _Point);
               
               // Ensure the new SL is valid
               if(newSL > 0 && MathAbs(newSL - currentSL) > (30 * _Point)) {
                  Print("[CHOCH] Modifying BUY stop loss based on bullish CHOCH: ", 
                        DoubleToString(currentSL, _Digits), " -> ", DoubleToString(newSL, _Digits));
                  
                  if(trade.PositionModify(ticket, newSL, currentTP)) {
                     g_chochPatterns[j].used = true;
                     Print("[CHOCH] Successfully modified stop loss for ticket #", ticket);
                  } else {
                     Print("[CHOCH] Failed to modify stop loss: ", GetLastError());
                  }
               }
            }
         }
         // For sell positions, look for bearish CHOCH patterns to lower stop loss
         else if(posType == POSITION_TYPE_SELL && !g_chochPatterns[j].isBullish) {
            // Only modify if the CHOCH level is below the current stop loss and above the entry price
            double chochLevel = g_chochPatterns[j].price;
            ...
            if(chochLevel < currentSL && chochLevel > openPrice) {
               ...
               // Ensure the new SL is valid
               if(newSL > 0 && MathAbs(newSL - currentSL) > (30 * _Point)) {
                  Print("[CHOCH] Modifying SELL stop loss based on bearish CHOCH: ", 
                        DoubleToString(currentSL, _Digits), " -> ", DoubleToString(newSL, _Digits));
...
bool IsHighValueAsset(string symbol)
{
   // Check for cryptocurrencies (BTC, ETH, etc.)
   if(StringFind(symbol, "BTC") >= 0 || StringFind(symbol, "ETH") >= 0 || 
      ...
      StringFind(symbol, "XMR") >= 0 || StringFind(symbol, "BNB") >= 0) {
      return true;
   }
   
   // Check for precious metals (GOLD, SILVER, etc.)
   if(StringFind(symbol, "GOLD") >= 0 || StringFind(symbol, "XAU") >= 0 || 
      ...
      StringFind(symbol, "PALLADIUM") >= 0 || StringFind(symbol, "XPD") >= 0) {
      return true;
   }
...
void UpdateAdaptivePairSettings(string symbol)
{
   ...
   // Skip if we don't have enough trades for meaningful adaptation
   if(g_pairSettings[pairIndex].performance.totalTrades < 5) {
      Print("[ADAPTIVE] Not enough trades for ", symbol, " to update adaptive settings. Trades: ",
            g_pairSettings[pairIndex].performance.totalTrades);
      return;
   }
   
   Print("[ADAPTIVE] Updating adaptive settings for ", symbol, ". Win rate: ", 
         DoubleToString(g_pairSettings[pairIndex].performance.winRate*100, 1), "%");
   
   // Check if this is a high-value asset
   bool isHighValue = IsHighValueAsset(symbol);
   
   // 1. Adapt stop loss multiplier based on win rate and stop loss hit rate
   double stopHitRate = 0;
   if(g_pairSettings[pairIndex].performance.losingTrades > 0) {
      stopHitRate = (double)g_pairSettings[pairIndex].performance.stopLossHits / 
                    g_pairSettings[pairIndex].performance.losingTrades;
   }
   
   // If stop losses are being hit too frequently, increase the buffer
   if(stopHitRate > 0.7) {
      g_pairSettings[pairIndex].slMultiplier = MathMin(3.0, g_pairSettings[pairIndex].slMultiplier * 1.2);
      Print("[ADAPTIVE] Increased SL multiplier to ", 
            DoubleToString(g_pairSettings[pairIndex].slMultiplier, 2), 
            " due to high stop hit rate (", DoubleToString(stopHitRate*100, 1), "%)");
   }
   // If win rate is good but could be better, try tightening stops slightly
   else if(g_pairSettings[pairIndex].performance.winRate > 0.6 && stopHitRate < 0.3) {
      g_pairSettings[pairIndex].slMultiplier = MathMax(0.5, g_pairSettings[pairIndex].slMultiplier * 0.95);
      Print("[ADAPTIVE] Decreased SL multiplier to ", 
            DoubleToString(g_pairSettings[pairIndex].slMultiplier, 2), 
            " due to good win rate and low stop hit rate");
   }
   
   // 2. Adapt order block parameters based on performance
   if(g_pairSettings[pairIndex].performance.winRate < 0.4) {
      // Poor performance - be more selective with order blocks
      g_pairSettings[pairIndex].orderBlockMinStrength = MathMin(3.0, g_pairSettings[pairIndex].orderBlockMinStrength * 1.2);
      Print("[ADAPTIVE] Increased order block min strength to ", 
            DoubleToString(g_pairSettings[pairIndex].orderBlockMinStrength, 2), 
            " due to poor win rate");
   }
   else if(g_pairSettings[pairIndex].performance.winRate > 0.6) {
      // Good performance - can be slightly less selective
      g_pairSettings[pairIndex].orderBlockMinStrength = MathMax(1.0, g_pairSettings[pairIndex].orderBlockMinStrength * 0.95);
      Print("[ADAPTIVE] Decreased order block min strength to ", 
            DoubleToString(g_pairSettings[pairIndex].orderBlockMinStrength, 2), 
            " due to good win rate");
   }
   
   // 3. Adapt spread threshold based on performance and asset type
   if(isHighValue) {
      // High-value assets can tolerate higher spreads
      ...
      // Poor performance - be more selective with spreads
      g_pairSettings[pairIndex].spreadThreshold = MathMax(0.1, g_pairSettings[pairIndex].spreadThreshold * 0.9);
      Print("[ADAPTIVE] Decreased spread threshold to ", 
            DoubleToString(g_pairSettings[pairIndex].spreadThreshold, 2), 
            " due to poor win rate");
   }
   
   // 4. Adapt order block age hours based on asset type and performance
   if(isHighValue) {
      // High-value assets can use older blocks
      g_pairSettings[pairIndex].orderBlockAgeHours = MathMax(8.0, g_pairSettings[pairIndex].orderBlockAgeHours);
   }
   else if(g_pairSettings[pairIndex].performance.winRate < 0.4) {
      // Poor performance - use fresher blocks
      g_pairSettings[pairIndex].orderBlockAgeHours = MathMax(1.0, g_pairSettings[pairIndex].orderBlockAgeHours * 0.8);
      Print("[ADAPTIVE] Decreased order block age to ", 
            DoubleToString(g_pairSettings[pairIndex].orderBlockAgeHours, 2), 
            " hours due to poor win rate");
   }
   
   // 5. Update CHOCH pattern success rate
   if(g_pairSettings[pairIndex].performance.chochPatternUses > 0) {
      g_pairSettings[pairIndex].performance.chochSuccessRate = 
         (double)g_pairSettings[pairIndex].performance.chochPatternSuccesses / 
         g_pairSettings[pairIndex].performance.chochPatternUses;
      
      Print("[ADAPTIVE] Updated CHOCH success rate for ", symbol, ": ", 
            DoubleToString(g_pairSettings[pairIndex].performance.chochSuccessRate*100, 1), "%");
   }
   
   // 6. Analyze market regime and adjust parameters accordingly
...
void AnalyzeMarketRegime(string symbol)
{
   ...
   g_pairSettings[pairIndex].marketRegime = volState;
