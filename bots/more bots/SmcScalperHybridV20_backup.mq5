...
//+------------------------------------------------------------------+
//| Dynamic Volatility Adapter - Integrated into main file           |
//+------------------------------------------------------------------+
...
struct VolatilityContext {
    ...
    string marketDescription;               // Human-readable description
...
//+------------------------------------------------------------------+
//| Get verbose description of volatility state                      |
//+------------------------------------------------------------------+
string GetVolatilityStateDescription(ENUM_VOLATILITY_STATE state) {
    switch(state) {
        case VOLATILITY_VERY_LOW:  return "Very Low Volatility";
        case VOLATILITY_LOW:       return "Low Volatility";
        case VOLATILITY_NORMAL:    return "Normal Volatility";
        case VOLATILITY_HIGH:      return "High Volatility";
        case VOLATILITY_VERY_HIGH: return "Very High Volatility";
        default:                   return "Unknown Volatility State";
    }
}

//+------------------------------------------------------------------+
//| Calculate and update volatility state and parameters             |
//+------------------------------------------------------------------+
void UpdateVolatilityContext(string symbol, ENUM_TIMEFRAMES timeframe) {
    // Calculate volatility metrics
    double currentATR = GetATR(symbol, timeframe, 14, 0);
    double avg20ATR = GetATR(symbol, timeframe, 14, 20);  // 20-period average
    double avg50ATR = GetATR(symbol, timeframe, 14, 50);  // 50-period average
    double prevATR = GetATR(symbol, timeframe, 14, 1);    // Previous period
    ...
    // Check longer-term volatility context
    double longerTermRatio = avg20ATR / avg50ATR;
    ...
    if(higherTF != timeframe) {
        ...
        if(higherAvgATR > 0) higherTFVolRatio = higherCurrentATR / higherAvgATR;
    }
    ...
    if(volRatio < 0.5 || (volRatio < 0.7 && isContracting && higherTFVolRatio < 0.8)) {
        volatilityState = VOLATILITY_VERY_LOW;
    }
    else if(volRatio < 0.8 || (volRatio < 0.9 && isContracting)) {
        volatilityState = VOLATILITY_LOW;
    }
    else if(volRatio > 2.0 || (volRatio > 1.5 && isExpanding && higherTFVolRatio > 1.3)) {
    ...
    switch(volatilityState) {
        case VOLATILITY_VERY_LOW:
            // Very permissive settings for low volatility
            g_volatilityContext.orderBlockMinStrength = 1;           // Accept weaker blocks
            g_volatilityContext.spreadThresholdMultiplier = 1.5;     // More tolerant of spread
            g_volatilityContext.orderBlockAgeHours = 8.0;           // Blocks valid longer
            g_volatilityContext.slMultiplier = 0.6;                 // Tighter stops (less risk)
            g_volatilityContext.tpMultiplier = 1.2;                 // Reasonable targets
            g_volatilityContext.signalQualityThreshold = 0.3;       // Lower quality threshold
            g_volatilityContext.volRangeMultiplier = 0.5;           // Smaller ranges
            break;
            
        case VOLATILITY_LOW:
            // Moderately permissive settings
            g_volatilityContext.orderBlockMinStrength = 2;           // Slightly stronger blocks
            g_volatilityContext.spreadThresholdMultiplier = 1.2;     // Slightly more tolerant of spread
            g_volatilityContext.orderBlockAgeHours = 6.0;           // Blocks valid quite long
            g_volatilityContext.slMultiplier = 0.7;                 // Slightly tighter stops
            g_volatilityContext.tpMultiplier = 1.4;                 // Slightly higher targets
            g_volatilityContext.signalQualityThreshold = 0.4;       // Moderate quality threshold
            g_volatilityContext.volRangeMultiplier = 0.8;           // Slightly smaller ranges
            break;
            
        case VOLATILITY_NORMAL:
            // Standard settings
            g_volatilityContext.orderBlockMinStrength = 3;           // Standard block strength
            g_volatilityContext.spreadThresholdMultiplier = 1.0;     // Standard spread tolerance
            g_volatilityContext.orderBlockAgeHours = 4.0;           // Standard block age
            g_volatilityContext.slMultiplier = 0.8;                 // Standard stops
            g_volatilityContext.tpMultiplier = 1.6;                 // Standard targets
            g_volatilityContext.signalQualityThreshold = 0.5;       // Standard quality threshold
            g_volatilityContext.volRangeMultiplier = 1.0;           // Standard ranges
            break;
            
        case VOLATILITY_HIGH:
            // More conservative settings
            g_volatilityContext.orderBlockMinStrength = 4;           // Stronger blocks required
            g_volatilityContext.spreadThresholdMultiplier = 0.8;     // Less tolerant of spread
            g_volatilityContext.orderBlockAgeHours = 3.0;           // Blocks valid for less time
            g_volatilityContext.slMultiplier = 1.0;                 // Wider stops for safety
            g_volatilityContext.tpMultiplier = 2.0;                 // Higher targets to compensate
            g_volatilityContext.signalQualityThreshold = 0.6;       // Higher quality threshold
            g_volatilityContext.volRangeMultiplier = 1.2;           // Larger ranges
            break;
            
        case VOLATILITY_VERY_HIGH:
            // Very conservative settings
            g_volatilityContext.orderBlockMinStrength = 5;           // Much stronger blocks required
            g_volatilityContext.spreadThresholdMultiplier = 0.6;     // Much less tolerant of spread
            g_volatilityContext.orderBlockAgeHours = 2.0;           // Blocks valid for much less time
            g_volatilityContext.slMultiplier = 1.2;                 // Much wider stops for safety
            g_volatilityContext.tpMultiplier = 2.4;                 // Much higher targets
            g_volatilityContext.signalQualityThreshold = 0.7;       // Much higher quality threshold
            g_volatilityContext.volRangeMultiplier = 1.5;           // Much larger ranges
            break;
    }
    
    // Generate human-readable description
    g_volatilityContext.marketDescription = GetVolatilityStateDescription(volatilityState) + 
        StringFormat(" (Ratio: %.2f, %s)", 
                    volRatio, 
                    (isExpanding ? "Expanding" : (isContracting ? "Contracting" : "Stable")));
}

//+------------------------------------------------------------------+
//| Apply volatility-adjusted parameters to EA variables              |
//+------------------------------------------------------------------+
void ApplyVolatilityParameters(double &slAtrMultParam, double &tpAtrMultParam, double &spreadThresholdParam, 
                              int &minOrderBlockStrengthParam, double &orderBlockMaxAgeParam) {
    // Apply the volatility-adjusted parameters to the EA's variables
    slAtrMultParam = g_volatilityContext.slMultiplier;
    tpAtrMultParam = g_volatilityContext.tpMultiplier;
    spreadThresholdParam *= g_volatilityContext.spreadThresholdMultiplier;
    minOrderBlockStrengthParam = (int)g_volatilityContext.orderBlockMinStrength;
    orderBlockMaxAgeParam = g_volatilityContext.orderBlockAgeHours;
    
    // Log the adjustment
    if(DisplayDebugInfo) {
        Print("[VOLATILITY ADAPTER] Applied parameters for ", g_volatilityContext.marketDescription);
        Print("[VOLATILITY ADAPTER] SL_ATR_Mult=", SL_ATR_Mult, 
              ", TP_ATR_Mult=", TP_ATR_Mult, 
              ", SpreadThresholdMult=", g_volatilityContext.spreadThresholdMultiplier,
              ", MinOrderBlockStrength=", MinOrderBlockStrength,
              ", OrderBlockMaxAge=", OrderBlockMaxAge);
    }
}

//+------------------------------------------------------------------+
//| Get a higher timeframe from current                              |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Check if we should enter in current volatility conditions        |
//+------------------------------------------------------------------+
bool ShouldTradeInCurrentVolatility(int signal) {
    // Don't open new positions in extremely high volatility unless we have a very strong signal
    if(g_volatilityContext.volatilityState == VOLATILITY_VERY_HIGH) {
        // For very high volatility, we need strong confirmation
        if(signal != 0 && GetSignalStrength(signal) < 0.8) {
            if(DisplayDebugInfo) {
                Print("[VOLATILITY FILTER] Avoided trade in very high volatility - signal strength insufficient");
            }
            return false;
        }
    }
...
//+------------------------------------------------------------------+
//| Get current volatility state as readable string                  |
//+------------------------------------------------------------------+
string GetCurrentVolatilityDescription() {
    return g_volatilityContext.marketDescription;
}

//+------------------------------------------------------------------+
//| Adjust position size based on volatility                         |
//+------------------------------------------------------------------+
double AdjustPositionSizeForVolatility(double baseSize) {
    double adjustmentFactor = 1.0;
    
    // Scale position size based on volatility state
    switch(g_volatilityContext.volatilityState) {
        case VOLATILITY_VERY_LOW:
            adjustmentFactor = 1.2;  // Increase size in very low volatility
            break;
        case VOLATILITY_LOW:
            adjustmentFactor = 1.1;  // Slightly increase size in low volatility
            break;
        case VOLATILITY_HIGH:
            adjustmentFactor = 0.8;  // Reduce size in high volatility
            break;
        case VOLATILITY_VERY_HIGH:
            adjustmentFactor = 0.5;  // Significantly reduce size in very high volatility
            break;
        default:
            adjustmentFactor = 1.0;  // No change for normal volatility
    }
    
    return baseSize * adjustmentFactor;
}

// Forward declaration for function used in ShouldTradeInCurrentVolatility
double GetSignalStrength(int signal);

//+------------------------------------------------------------------+
//| Advanced Predictive Analytics System                             |
//+------------------------------------------------------------------+

// Structure to hold predictive model results
struct PredictiveModel {
    double buyProbability;    // Probability of upward movement (0-1)
    double sellProbability;   // Probability of downward movement (0-1)
    double neutralProbability; // Probability of sideways movement (0-1)
    double predictionQuality; // Quality score of the prediction (0-1)
    string dominantDirection; // "BUY", "SELL", or "NEUTRAL"
    string reasonBuy;         // Reasons for buy prediction
    string reasonSell;        // Reasons for sell prediction
    string reasonNeutral;     // Reasons for neutral prediction
};

// Global predictive model instance
PredictiveModel g_predictiveModel;

//+------------------------------------------------------------------+
//| Generate multi-indicator prediction model                        |
//+------------------------------------------------------------------+
bool GeneratePredictiveModel(string symbol, ENUM_TIMEFRAMES timeframe) {
    if(!EnablePredictiveAnalytics) return false;
    
    // Add timestamp to track when the prediction was generated
    static datetime lastPredictionTime = 0;
    datetime currentTime = TimeCurrent();
    
    // Only regenerate prediction model every 5 minutes to avoid excessive calculations
    if(currentTime - lastPredictionTime < 300 && lastPredictionTime > 0) {
        return true; // Use existing prediction if it's recent
    }
    
    lastPredictionTime = currentTime;
    
    // Reset model
    g_predictiveModel.buyProbability = 0;
    g_predictiveModel.sellProbability = 0;
    g_predictiveModel.neutralProbability = 0;
    g_predictiveModel.predictionQuality = 0;
    g_predictiveModel.reasonBuy = "";
    g_predictiveModel.reasonSell = "";
    g_predictiveModel.reasonNeutral = "";
    
    // Initialize counters for each direction
    double buySignals = 0;
    double sellSignals = 0;
    double neutralSignals = 0;
    double totalSignalQuality = 0;
    double totalSignals = 0;
    
    // ------------------------------------------------------------------
    // 1. RSI Analysis - Predictive power for reversals
    // ------------------------------------------------------------------
    int rsiPeriod = 14;
    double rsiSignalQuality = 0.75; // RSI has good predictive quality
    double rsiValue = CalculateRSI(symbol, timeframe, rsiPeriod, 0);
    double rsiPrev = CalculateRSI(symbol, timeframe, rsiPeriod, 1);
    
    if(rsiValue < 30) {
        buySignals += rsiSignalQuality;
        g_predictiveModel.reasonBuy += "RSI oversold; ";
    }
    else if(rsiValue > 70) {
        sellSignals += rsiSignalQuality;
        g_predictiveModel.reasonSell += "RSI overbought; ";
    }
    else {
        if(rsiValue > 50 && rsiValue > rsiPrev) {
            buySignals += rsiSignalQuality * 0.5; // Partial signal
            g_predictiveModel.reasonBuy += "RSI rising above 50; ";
        }
        else if(rsiValue < 50 && rsiValue < rsiPrev) {
            sellSignals += rsiSignalQuality * 0.5; // Partial signal
            g_predictiveModel.reasonSell += "RSI falling below 50; ";
        }
        else {
            neutralSignals += rsiSignalQuality;
            g_predictiveModel.reasonNeutral += "RSI neutral; ";
        }
    }
    
    totalSignals += rsiSignalQuality;
    totalSignalQuality += rsiSignalQuality;
    
    // ------------------------------------------------------------------
    // 2. MACD Analysis - Predictive power for trend direction
    // ------------------------------------------------------------------
    double macdSignalQuality = 0.8; // MACD has high predictive quality
    double macdMain=0, macdSignal=0, macdPrevMain=0, macdPrevSignal=0;
    
    CalculateMACD(symbol, timeframe, 12, 26, 9, 0, macdMain, macdSignal);
    CalculateMACD(symbol, timeframe, 12, 26, 9, 1, macdPrevMain, macdPrevSignal);
    
    bool macdCrossUp = (macdPrevMain < macdPrevSignal) && (macdMain > macdSignal);
    bool macdCrossDown = (macdPrevMain > macdPrevSignal) && (macdMain < macdSignal);
    
    if(macdCrossUp) {
        buySignals += macdSignalQuality;
        g_predictiveModel.reasonBuy += "MACD bullish crossover; ";
    }
    else if(macdCrossDown) {
        sellSignals += macdSignalQuality;
        g_predictiveModel.reasonSell += "MACD bearish crossover; ";
    }
    else if(macdMain > macdSignal && macdMain > 0) {
        buySignals += macdSignalQuality * 0.7; // Ongoing uptrend
        g_predictiveModel.reasonBuy += "MACD positive and above signal; ";
    }
    else if(macdMain < macdSignal && macdMain < 0) {
        sellSignals += macdSignalQuality * 0.7; // Ongoing downtrend
        g_predictiveModel.reasonSell += "MACD negative and below signal; ";
    }
    else {
        neutralSignals += macdSignalQuality;
        g_predictiveModel.reasonNeutral += "MACD neutral; ";
    }
    
    totalSignals += macdSignalQuality;
    totalSignalQuality += macdSignalQuality;
    
    // ------------------------------------------------------------------
    // 3. Stochastic Analysis - Predictive power for momentum shifts
    // ------------------------------------------------------------------
    double stochSignalQuality = 0.7; // Stochastic has good predictive quality
    double k=0, d=0, kPrev=0, dPrev=0;
    
    CalculateStochastic(symbol, timeframe, 5, 3, 3, 0, k, d);
    CalculateStochastic(symbol, timeframe, 5, 3, 3, 1, kPrev, dPrev);
    
    if(k < 20 && kPrev < k) {
        buySignals += stochSignalQuality;
        g_predictiveModel.reasonBuy += "Stochastic oversold and rising; ";
    }
    else if(k > 80 && kPrev > k) {
        sellSignals += stochSignalQuality;
        g_predictiveModel.reasonSell += "Stochastic overbought and falling; ";
    }
    else if(k > d && kPrev < dPrev) {
        buySignals += stochSignalQuality * 0.6; // Bullish crossover
        g_predictiveModel.reasonBuy += "Stochastic bullish cross; ";
    }
    else if(k < d && kPrev > dPrev) {
        sellSignals += stochSignalQuality * 0.6; // Bearish crossover
        g_predictiveModel.reasonSell += "Stochastic bearish cross; ";
    }
    else {
        neutralSignals += stochSignalQuality;
        g_predictiveModel.reasonNeutral += "Stochastic neutral; ";
    }
    
    totalSignals += stochSignalQuality;
    totalSignalQuality += stochSignalQuality;
    
    // ------------------------------------------------------------------
    // 4. Bollinger Bands Analysis - Predictive power for volatility breakouts
    // ------------------------------------------------------------------
    double bbSignalQuality = 0.75; // BB has good predictive quality
    double middle=0, upper=0, lower=0;
    double close = iClose(symbol, timeframe, 0);
    
    CalculateBollingerBands(symbol, timeframe, 20, 2, 0, middle, upper, lower);
    
    double bbWidth = (upper - lower) / middle;
    double bbPrevWidth = 0;
    
    double prevMiddle=0, prevUpper=0, prevLower=0;
    CalculateBollingerBands(symbol, timeframe, 20, 2, 5, prevMiddle, prevUpper, prevLower);
    bbPrevWidth = (prevUpper - prevLower) / prevMiddle;
    
    bool bbNarrowing = bbWidth < bbPrevWidth * 0.9;
    bool bbWidening = bbWidth > bbPrevWidth * 1.1;
    
    // Price relative to bands
    if(close > upper) {
        sellSignals += bbSignalQuality * 0.8; // Price above upper band - potential reversal
        g_predictiveModel.reasonSell += "Price above upper BB; ";
    }
    else if(close < lower) {
        buySignals += bbSignalQuality * 0.8; // Price below lower band - potential reversal
        g_predictiveModel.reasonBuy += "Price below lower BB; ";
    }
    else if(bbNarrowing) {
        neutralSignals += bbSignalQuality * 0.7; // Consolidation phase
        g_predictiveModel.reasonNeutral += "BB narrowing (consolidation); ";
    }
    else if(bbWidening && close > middle) {
        buySignals += bbSignalQuality * 0.6; // Expanding volatility with price above middle
        g_predictiveModel.reasonBuy += "BB widening with price above middle; ";
    }
    else if(bbWidening && close < middle) {
        sellSignals += bbSignalQuality * 0.6; // Expanding volatility with price below middle
        g_predictiveModel.reasonSell += "BB widening with price below middle; ";
    }
    else {
        neutralSignals += bbSignalQuality * 0.5;
        g_predictiveModel.reasonNeutral += "Price within normal BB range; ";
    }
    
    totalSignals += bbSignalQuality;
    totalSignalQuality += bbSignalQuality;
    
    // ------------------------------------------------------------------
    // 5. Order Block and Market Structure Analysis (using existing SMC logic)
    // ------------------------------------------------------------------
    double smcSignalQuality = 0.85; // SMC has high predictive quality
    int validBullishBlocks = 0;
    int validBearishBlocks = 0;
    
    // Count valid order blocks from our existing analysis
    for(int i = 0; i < MAX_BLOCKS; i++) {
        if(recentBlocks[i].valid) {
            if(recentBlocks[i].isBuy) validBullishBlocks++;
            else validBearishBlocks++;
        }
    }
    
    if(validBullishBlocks > validBearishBlocks && validBullishBlocks >= 2) {
        buySignals += smcSignalQuality;
        g_predictiveModel.reasonBuy += "Multiple valid bullish order blocks; ";
    }
    else if(validBearishBlocks > validBullishBlocks && validBearishBlocks >= 2) {
        sellSignals += smcSignalQuality;
        g_predictiveModel.reasonSell += "Multiple valid bearish order blocks; ";
    }
    else if(validBullishBlocks > 0 && validBearishBlocks == 0) {
        buySignals += smcSignalQuality * 0.7;
        g_predictiveModel.reasonBuy += "Only bullish order blocks present; ";
    }
    else if(validBearishBlocks > 0 && validBullishBlocks == 0) {
        sellSignals += smcSignalQuality * 0.7;
        g_predictiveModel.reasonSell += "Only bearish order blocks present; ";
    }
    else {
        neutralSignals += smcSignalQuality * 0.5;
        g_predictiveModel.reasonNeutral += "Mixed or no clear order block structure; ";
    }
    
    totalSignals += smcSignalQuality;
    totalSignalQuality += smcSignalQuality * 0.8; // Weight SMC slightly lower in prediction quality
    
    // ------------------------------------------------------------------
    // Calculate final probabilities
    // ------------------------------------------------------------------
    if(totalSignals > 0) {
        g_predictiveModel.buyProbability = buySignals / totalSignals;
        g_predictiveModel.sellProbability = sellSignals / totalSignals;
        g_predictiveModel.neutralProbability = neutralSignals / totalSignals;
        g_predictiveModel.predictionQuality = totalSignalQuality / totalSignals;
        
        // Determine dominant direction
        if(g_predictiveModel.buyProbability > g_predictiveModel.sellProbability && 
           g_predictiveModel.buyProbability > g_predictiveModel.neutralProbability) {
            g_predictiveModel.dominantDirection = "BUY";
        }
        else if(g_predictiveModel.sellProbability > g_predictiveModel.buyProbability && 
                g_predictiveModel.sellProbability > g_predictiveModel.neutralProbability) {
            g_predictiveModel.dominantDirection = "SELL";
        }
        else {
            g_predictiveModel.dominantDirection = "NEUTRAL";
        }
        
        // Log the prediction if debugging is enabled
        if(DisplayDebugInfo) {
            LogInfo(StringFormat("[PREDICTIVE MODEL] Generated for %s: Buy=%.2f, Sell=%.2f, Neutral=%.2f, Quality=%.2f", 
                             symbol, 
                             g_predictiveModel.buyProbability, 
                             g_predictiveModel.sellProbability,
                             g_predictiveModel.neutralProbability,
                             g_predictiveModel.predictionQuality));
            
            if(g_predictiveModel.dominantDirection == "BUY") {
                LogInfo("[PREDICTIVE MODEL] Buy reasons: " + g_predictiveModel.reasonBuy);
            }
            else if(g_predictiveModel.dominantDirection == "SELL") {
                LogInfo("[PREDICTIVE MODEL] Sell reasons: " + g_predictiveModel.reasonSell);
            }
            else {
                LogInfo("[PREDICTIVE MODEL] Neutral reasons: " + g_predictiveModel.reasonNeutral);
            }
        }
...
//+------------------------------------------------------------------+
//| Calculate RSI value for indicator-based prediction               |
//+------------------------------------------------------------------+
double CalculateRSI(string symbol, ENUM_TIMEFRAMES timeframe, int period, int shift) {
    double rsiValue = 50; // Default neutral value
    
    // Get RSI
    int rsiHandle = iRSI(symbol, timeframe, period, PRICE_CLOSE);
...
//+------------------------------------------------------------------+
//| Calculate MACD values for indicator-based prediction             |
//+------------------------------------------------------------------+
bool CalculateMACD(string symbol, ENUM_TIMEFRAMES timeframe, int fastEMA, int slowEMA, 
                 int signalPeriod, int shift, double &macdMain, double &macdSignal) {
    ...
    if(macdHandle != INVALID_HANDLE) {
        ...
        bool success = CopyBuffer(macdHandle, 0, shift, 1, macdMainBuffer) > 0 &&
                      CopyBuffer(macdHandle, 1, shift, 1, macdSignalBuffer) > 0;
...
//+------------------------------------------------------------------+
//| Calculate Stochastic values for indicator-based prediction       |
//+------------------------------------------------------------------+
bool CalculateStochastic(string symbol, ENUM_TIMEFRAMES timeframe, int kPeriod, int dPeriod, 
                       int slowing, int shift, double &k, double &d) {
    // Initialize with neutral values
    k = 50;
    d = 50;
    ...
    if(stochHandle != INVALID_HANDLE) {
        ...
        bool success = CopyBuffer(stochHandle, 0, shift, 1, kBuffer) > 0 &&
                      CopyBuffer(stochHandle, 1, shift, 1, dBuffer) > 0;
...
//+------------------------------------------------------------------+
//| Calculate Bollinger Bands values for indicator-based prediction  |
//+------------------------------------------------------------------+
bool CalculateBollingerBands(string symbol, ENUM_TIMEFRAMES timeframe, int period, double deviation, 
                           int shift, double &middle, double &upper, double &lower) {
    // Initialize with neutral values based on current price
    double price = iClose(symbol, timeframe, shift);
    middle = price;
    upper = price * 1.01; // 1% above
    lower = price * 0.99; // 1% below
    
    // Get Bollinger Bands
    int bbHandle = iBands(symbol, timeframe, period, 0, deviation, PRICE_CLOSE);
    if(bbHandle != INVALID_HANDLE) {
        double upperBuffer[], middleBuffer[], lowerBuffer[];
        bool success = CopyBuffer(bbHandle, 0, shift, 1, middleBuffer) > 0 &&
                      CopyBuffer(bbHandle, 1, shift, 1, upperBuffer) > 0 &&
                      CopyBuffer(bbHandle, 2, shift, 1, lowerBuffer) > 0;
...
//+------------------------------------------------------------------+
//| Apply predictive model to enhance trading decision               |
//+------------------------------------------------------------------+
double EnhanceSignalWithPrediction(int signal) {
    if(!EnablePredictiveAnalytics) return 1.0; // No adjustment if disabled
    
    // Log predictive model data for debugging
    if(LogPredictiveAnalytics) {
        LogInfo("Predictive Model - Buy: " + DoubleToString(g_predictiveModel.buyProbability, 2) + 
               ", Sell: " + DoubleToString(g_predictiveModel.sellProbability, 2) + 
               ", Neutral: " + DoubleToString(g_predictiveModel.neutralProbability, 2) + 
               ", Quality: " + DoubleToString(g_predictiveModel.predictionQuality, 2));
    }
                     
    double enhancement = 1.0; // Default - no change
    
    // Strengthen or weaken signal based on predictive model
    if(signal > 0) { // Buy signal
        if(g_predictiveModel.dominantDirection == "BUY") {
            // Strengthen buy signal when prediction agrees
            enhancement = 1.0 + (g_predictiveModel.buyProbability * g_predictiveModel.predictionQuality);
            LogInfo(StringFormat("[PREDICTION] Strengthening BUY signal by %.2f%%", (enhancement-1.0)*100));
        }
        else if(g_predictiveModel.dominantDirection == "SELL") {
            // Weaken buy signal when prediction disagrees
            enhancement = 1.0 - (g_predictiveModel.sellProbability * g_predictiveModel.predictionQuality * 0.5);
            LogInfo(StringFormat("[PREDICTION] Weakening BUY signal by %.2f%%", (1.0-enhancement)*100));
        }
    }
    else if(signal < 0) { // Sell signal
        if(g_predictiveModel.dominantDirection == "SELL") {
            // Strengthen sell signal when prediction agrees
            enhancement = 1.0 + (g_predictiveModel.sellProbability * g_predictiveModel.predictionQuality);
            LogInfo(StringFormat("[PREDICTION] Strengthening SELL signal by %.2f%%", (enhancement-1.0)*100));
        }
        else if(g_predictiveModel.dominantDirection == "BUY") {
            // Weaken sell signal when prediction disagrees
            enhancement = 1.0 - (g_predictiveModel.buyProbability * g_predictiveModel.predictionQuality * 0.5);
            LogInfo(StringFormat("[PREDICTION] Weakening SELL signal by %.2f%%", (1.0-enhancement)*100));
        }
    }
    
    // Apply signal quality adjustment based on prediction quality and directional alignment
    if(signal > 0 && g_predictiveModel.buyProbability >= 0.6) {
        // Strong buy signal reinforcement
        enhancement *= 1.0 + (g_predictiveModel.buyProbability - 0.5) * g_predictiveModel.predictionQuality;
        LogInfo("Buy signal enhanced by " + DoubleToString(enhancement, 2) + " due to strong predictive buy probability (" + 
               DoubleToString(g_predictiveModel.buyProbability, 2) + ")");
    }
    else if(signal < 0 && g_predictiveModel.sellProbability >= 0.6) {
        // Strong sell signal reinforcement
        enhancement *= 1.0 + (g_predictiveModel.sellProbability - 0.5) * g_predictiveModel.predictionQuality;
        LogInfo("Sell signal enhanced by " + DoubleToString(enhancement, 2) + " due to strong predictive sell probability (" + 
               DoubleToString(g_predictiveModel.sellProbability, 2) + ")");
    }
    else if((signal > 0 && g_predictiveModel.sellProbability > 0.7) || 
             (signal < 0 && g_predictiveModel.buyProbability > 0.7)) {
        // Signal contradicts strong prediction - reduce strength
        enhancement *= 0.5;
        LogInfo("Signal reduced by 50% due to contradictory predictive model direction");
    }
    
    // Apply additional quality-based adjustment
    if(g_predictiveModel.predictionQuality < 0.5) {
        // Low quality predictions get less weight
        enhancement *= 0.7 + (0.6 * g_predictiveModel.predictionQuality);
        LogInfo("Signal reduced due to low prediction quality");
    }
    
    // Cap enhancement to reasonable bounds
    if(enhancement > 2.0) enhancement = 2.0;
    if(enhancement < 0.3) enhancement = 0.3;
...
void LogInfo(string message) {
    if(DisplayDebugInfo || LogLevel >= 2) {
        Print("[INFO] ", message);
        
        if(LogToFile) {
            // Append to log file with timestamp
            string filename = "SMC_EA_Log_" + Symbol() + ".log";
            int handle = FileOpen(filename, FILE_WRITE|FILE_READ|FILE_SHARE_READ|FILE_TXT|FILE_ANSI);
            if(handle != INVALID_HANDLE) {
                FileSeek(handle, 0, SEEK_END);
                FileWriteString(handle, TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES|TIME_SECONDS) + 
                                " [INFO] " + message + "\n");
                FileClose(handle);
            }
        }
...
void LogError(string message) {
    ...
    // Also create a dedicated error log file for easier troubleshooting
    string errorFilename = "SMC_EA_Errors_" + Symbol() + ".log";
    int errorHandle = FileOpen(errorFilename, FILE_WRITE|FILE_READ|FILE_SHARE_READ|FILE_TXT|FILE_ANSI);
    if(errorHandle != INVALID_HANDLE) {
        FileSeek(errorHandle, 0, SEEK_END);
        FileWriteString(errorHandle, TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES|TIME_SECONDS) + 
                        " " + message + "\n");
        FileClose(errorHandle);
    }
}
...
double DetermineOptimalStopLoss(int signal, double entryPrice) {
    ...
    double minStopDistancePoints = GetMinimumStopDistance();
    
    // Get current symbol point value
    double point = SymbolInfoDouble(Symbol(), SYMBOL_POINT);
    if(point == 0) {
        LogError("Failed to get point value, using default 0.00001");
        point = 0.00001; // Default fallback
    }
    
    // Convert minimum stop distance from points to price
    double minStopDistance = minStopDistancePoints * point;
    LogInfo(StringFormat("Minimum stop distance: %.5f (%.1f points)", minStopDistance, minStopDistancePoints));
    ...
    // Use the point value we already calculated above
    int stopLevel = (int)SymbolInfoInteger(Symbol(), SYMBOL_TRADE_STOPS_LEVEL);
    
    LogInfo("Stop validation - Point: " + DoubleToString(point, 8) + ", Stop level: " + IntegerToString(stopLevel) + " points");
...
input bool EnablePredictiveAnalytics = true;  // Enable advanced predictive analytics system
...
double CalculatePositionSize(int signal, double entryPrice, double stopLoss) {
    ...
    // 3. Adjust risk based on volatility state
    double volatilityAdjustment = 1.0;
    switch(g_volatilityContext.volatilityState) {
        case VOLATILITY_VERY_LOW:
            volatilityAdjustment = 1.2;  // Increase size in very low volatility
            LogInfo("[ADAPTIVE RISK] Increasing risk by 20% in very low volatility conditions");
            break;
        case VOLATILITY_LOW:
            volatilityAdjustment = 1.1;  // Slightly increase size in low volatility
            LogInfo("[ADAPTIVE RISK] Increasing risk by 10% in low volatility conditions");
            break;
        case VOLATILITY_HIGH:
            volatilityAdjustment = 0.8;  // Reduce size in high volatility
            LogInfo("[ADAPTIVE RISK] Reducing risk by 20% in high volatility conditions");
            break;
        case VOLATILITY_VERY_HIGH:
            volatilityAdjustment = 0.5;  // Significantly reduce size in very high volatility
            LogInfo("[ADAPTIVE RISK] Reducing risk by 50% in very high volatility conditions");
            break;
        default:
            volatilityAdjustment = 1.0;  // No change for normal volatility
    }
    riskPercentage *= volatilityAdjustment;
    
    // 4. Apply asset-specific position size adjustment
    double assetAdjustment = g_assetConfig.positionSizeMult;
    if(assetAdjustment != 1.0) {
        riskPercentage *= assetAdjustment;
        LogInfo(StringFormat("[ASSET RISK] Adjusting risk by %.0f%% for %s asset class", 
                         (assetAdjustment-1.0)*100, g_assetConfig.assetClass));
    }
    
    // 5. Apply predictive model adjustment if enabled
    if(EnablePredictiveAnalytics && g_predictiveModel.predictionQuality > 0.5) {
        double predictionAdjustment = 1.0;
        if(signal > 0 && g_predictiveModel.dominantDirection == "BUY") {
            // Increase size for buy signals when prediction is bullish
            predictionAdjustment = 1.0 + (g_predictiveModel.buyProbability * 0.3); // Up to 30% increase
            LogInfo(StringFormat("[PREDICTION RISK] Increasing position size by %.1f%% based on bullish prediction",
                             (predictionAdjustment-1.0)*100));
        }
        else if(signal < 0 && g_predictiveModel.dominantDirection == "SELL") {
            // Increase size for sell signals when prediction is bearish
            predictionAdjustment = 1.0 + (g_predictiveModel.sellProbability * 0.3); // Up to 30% increase
            LogInfo(StringFormat("[PREDICTION RISK] Increasing position size by %.1f%% based on bearish prediction",
                             (predictionAdjustment-1.0)*100));
        }
        else if((signal > 0 && g_predictiveModel.dominantDirection == "SELL") ||
                (signal < 0 && g_predictiveModel.dominantDirection == "BUY")) {
            // Decrease size when signal and prediction disagree
            predictionAdjustment = 0.7; // 30% reduction
            LogInfo("[PREDICTION RISK] Reducing position size by 30% due to conflicting prediction");
        }
        
        riskPercentage *= predictionAdjustment;
    }
    
    // 4. Adjust risk based on time of day
...
//+------------------------------------------------------------------+
//| Asset-Agnostic Configuration System                             |
//+------------------------------------------------------------------+

// Structure to hold asset-specific configuration parameters
struct AssetConfig {
    string assetClass;             // Asset class identifier: "forex", "crypto", "indices", "metals", etc.
    double orderBlockStrengthMult; // Multiplier for OrderBlockStrength
    double spreadThresholdMult;    // Multiplier for SpreadThreshold
    double orderBlockAgeMult;      // Multiplier for OrderBlockMaxAge
    double slAtrMult;              // Multiplier for SL_ATR_Mult
    double tpAtrMult;              // Multiplier for TP_ATR_Mult
    double signalQualityThreshold; // Minimum signal quality for this asset class
    double positionSizeMult;       // Position size multiplier
    bool requireConfirmation;      // Whether to require additional confirmation
};

// Global asset configuration instance
AssetConfig g_assetConfig;
...
string DetectAssetClass(string symbol) {
    ...
    if(StringFind(symbol, "BTC") >= 0 || StringFind(symbol, "ETH") >= 0 || 
       StringFind(symbol, "XRP") >= 0 || StringFind(symbol, "BCH") >= 0 ||
    ...
    if(StringFind(symbol, "GOLD") >= 0 || StringFind(symbol, "XAU") >= 0 || 
       ...
       StringFind(symbol, "PLATINUM") >= 0 || StringFind(symbol, "XPT") >= 0) {
        return "metals";
    }
    
    // Indices detection
    if(StringFind(symbol, "30") >= 0 || StringFind(symbol, "US500") >= 0 || 
       StringFind(symbol, "NAS") >= 0 || StringFind(symbol, "SPX") >= 0 ||
       StringFind(symbol, "UK100") >= 0 || StringFind(symbol, "DE30") >= 0 ||
       StringFind(symbol, "JP225") >= 0 || StringFind(symbol, "DJ") >= 0) {
        return "indices";
    }
    
    // Major forex pairs
    if(StringLen(symbol) <= 7) { // Most forex pairs are 6-7 chars
...
//+------------------------------------------------------------------+
void ConfigureForAssetClass(string symbol) {
    string assetClass = DetectAssetClass(symbol);
    
    // Initialize with neutral values
    g_assetConfig.assetClass = assetClass;
    g_assetConfig.orderBlockStrengthMult = 1.0;
    g_assetConfig.spreadThresholdMult = 1.0;
    g_assetConfig.orderBlockAgeMult = 1.0;
    g_assetConfig.slAtrMult = 1.0;
    g_assetConfig.tpAtrMult = 1.0;
    g_assetConfig.signalQualityThreshold = 0.5;
    g_assetConfig.positionSizeMult = 1.0;
    g_assetConfig.requireConfirmation = false;
    
    if(assetClass == "crypto") {
        // Cryptocurrencies need specific adjustments due to higher volatility
        g_assetConfig.orderBlockStrengthMult = 0.7;   // More permissive - accept weaker blocks
        g_assetConfig.spreadThresholdMult = 2.5;      // Much higher spread tolerance
        g_assetConfig.orderBlockAgeMult = 2.0;        // Blocks stay valid longer
        g_assetConfig.slAtrMult = 1.3;                // Wider stops for high volatility
        g_assetConfig.tpAtrMult = 1.8;                // Higher profit targets
        g_assetConfig.signalQualityThreshold = 0.6;   // Require higher quality signals
        g_assetConfig.positionSizeMult = 0.7;         // Reduce position size for risk management
        g_assetConfig.requireConfirmation = true;     // Require extra confirmation
        
        LogInfo("[ASSET CONFIG] Applied cryptocurrency-specific parameters");
    }
    else if(assetClass == "metals") {
        // Metals like Gold have specific characteristics
        g_assetConfig.orderBlockStrengthMult = 0.8;   // Slightly more permissive
        g_assetConfig.spreadThresholdMult = 1.5;      // Higher spread tolerance
        g_assetConfig.orderBlockAgeMult = 1.2;        // Blocks valid slightly longer
        g_assetConfig.slAtrMult = 1.1;                // Slightly wider stops
        g_assetConfig.tpAtrMult = 1.2;                // Slightly higher targets
        g_assetConfig.signalQualityThreshold = 0.55;  // Require slightly higher quality
        g_assetConfig.positionSizeMult = 0.8;         // Slightly reduced position size
        g_assetConfig.requireConfirmation = false;    // Extra confirmation not required
        
        LogInfo("[ASSET CONFIG] Applied metals-specific parameters");
    }
    else if(assetClass == "indices") {
        // Stock indices have their own characteristics
        g_assetConfig.orderBlockStrengthMult = 0.9;   // Nearly standard requirements
        g_assetConfig.spreadThresholdMult = 1.2;      // Slightly higher spread tolerance
        g_assetConfig.orderBlockAgeMult = 0.8;        // Blocks valid for less time
        g_assetConfig.slAtrMult = 0.9;                // Slightly tighter stops
        g_assetConfig.tpAtrMult = 1.1;                // Slightly higher targets
        g_assetConfig.signalQualityThreshold = 0.5;   // Standard quality
        g_assetConfig.positionSizeMult = 0.9;         // Nearly standard position size
        g_assetConfig.requireConfirmation = false;    // Extra confirmation not required
        
        LogInfo("[ASSET CONFIG] Applied indices-specific parameters");
    }
    else { // forex and default
        // Standard forex settings
        g_assetConfig.orderBlockStrengthMult = 1.0;   // Standard requirements
        g_assetConfig.spreadThresholdMult = 1.0;      // Standard spread tolerance
        g_assetConfig.orderBlockAgeMult = 1.0;        // Standard block age
        g_assetConfig.slAtrMult = 1.0;                // Standard stops
        g_assetConfig.tpAtrMult = 1.0;                // Standard targets
        g_assetConfig.signalQualityThreshold = 0.5;   // Standard quality
        g_assetConfig.positionSizeMult = 1.0;         // Standard position size
        g_assetConfig.requireConfirmation = false;    // Extra confirmation not required
        
        LogInfo("[ASSET CONFIG] Applied standard forex parameters");
    }
}

//+------------------------------------------------------------------+
//| Apply asset-specific configuration to actual parameters          |
//+------------------------------------------------------------------+
void ApplyAssetConfiguration() {
    // Apply asset class multipliers to actual parameters
    MinOrderBlockStrength = (int)(MinOrderBlockStrength * g_assetConfig.orderBlockStrengthMult);
    if(MinOrderBlockStrength < 1) MinOrderBlockStrength = 1; // Ensure minimum 1
    
    SpreadThreshold *= g_assetConfig.spreadThresholdMult;
    OrderBlockMaxAge *= g_assetConfig.orderBlockAgeMult;
    SL_ATR_Mult *= g_assetConfig.slAtrMult;
    TP_ATR_Mult *= g_assetConfig.tpAtrMult;
    MinSignalQuality = g_assetConfig.signalQualityThreshold;
    
    // Log applied configuration
    if(DisplayDebugInfo) {
        LogInfo(StringFormat("[ASSET CONFIG] Applied %s configuration: MinOrderBlockStrength=%d, SpreadThreshold=%.1f, OrderBlockMaxAge=%.1f", 
                        g_assetConfig.assetClass, MinOrderBlockStrength, SpreadThreshold, OrderBlockMaxAge));
    }
}
...
// CHOCH (Change of Character) detection and handling

//+------------------------------------------------------------------+
//| Detect Change of Character (CHOCH) pattern                      |
//+------------------------------------------------------------------+
bool DetectCHOCH(bool &isBullish, double &chochLevel) {
    ...
    // Get sufficient data for pattern detection
    int bars = 100; // Look back enough bars to detect meaningful swings
    ...
    // Find important swing points
    SwingPoint swings[];
    int swingCount = 0;
    int maxSwings = 10; // Maximum swing points to analyze
    ArrayResize(swings, maxSwings);
    
    // Detect bullish CHOCH (V-shaped recovery)
    bool foundBullishCHOCH = false;
    double bullishLevel = 0;
    
    // Detect bearish CHOCH (inverted V-shaped breakdown)
    bool foundBearishCHOCH = false;
    double bearishLevel = 0;
    
    // Find significant swing lows for bullish CHOCH
    for(int i = 5; i < bars-5; i++) {
        // Check if this is a swing low (lower than both sides)
        if(low[i] < low[i+1] && low[i] < low[i+2] && 
           low[i] < low[i-1] && low[i] < low[i-2]) {
            
            // Store as a swing point
            if(swingCount < maxSwings) {
                swings[swingCount].price = low[i];
                swings[swingCount].isBullish = true; // Swing low is potential bullish CHOCH point
                swings[swingCount].index = i;
                swings[swingCount].strength = 0; // Will calculate
                swingCount++;
            }
        }
        
        // Check if this is a swing high (higher than both sides)
        if(high[i] > high[i+1] && high[i] > high[i+2] && 
           high[i] > high[i-1] && high[i] > high[i-2]) {
            ...
            if(swingCount < maxSwings) {
                ...
                swings[swingCount].isBullish = false; // Swing high is potential bearish CHOCH point
                swings[swingCount].index = i;
                swings[swingCount].strength = 0; // Will calculate
                swingCount++;
            }
    ...
    // No significant swing points found
    if(swingCount < 2) {
        return false;
    }
    
    // Sort swing points by index (most recent first)
    for(int i = 0; i < swingCount - 1; i++) {
        for(int j = 0; j < swingCount - i - 1; j++) {
            if(swings[j].index > swings[j+1].index) {
                // Swap
    ...
    // Calculate strength of each swing point based on surrounding price action
    for(int i = 0; i < swingCount; i++) {
        int idx = swings[i].index;
        if(swings[i].isBullish) { // Swing low
            // Check how many bars respect this as support
            int supportCount = 0;
            for(int j = idx-1; j >= MathMax(0, idx-10); j--) {
                if(low[j] >= swings[i].price * 0.9998) supportCount++;
            }
            for(int j = idx+1; j < MathMin(bars, idx+10); j++) {
                if(low[j] >= swings[i].price * 0.9998) supportCount++;
            }
            swings[i].strength = supportCount;
        }
        else { // Swing high
            // Check how many bars respect this as resistance
            int resistanceCount = 0;
            for(int j = idx-1; j >= MathMax(0, idx-10); j--) {
                if(high[j] <= swings[i].price * 1.0002) resistanceCount++;
            }
            for(int j = idx+1; j < MathMin(bars, idx+10); j++) {
                if(high[j] <= swings[i].price * 1.0002) resistanceCount++;
            }
            swings[i].strength = resistanceCount;
        }
    }
    
    // Now look for CHOCH patterns by analyzing swing point relationships
    for(int i = 0; i < swingCount-1; i++) {
        // Bullish CHOCH: Two swing lows where the second is higher than the first
        // indicating a change of character from downtrend to uptrend
        if(swings[i].isBullish && swings[i+1].isBullish && 
           swings[i+1].price > swings[i].price && 
           swings[i].strength >= 3 && swings[i+1].strength >= 3) {
            
            // Additional confirmation: We've broken above a prior swing high
            bool confirmedBreakout = false;
            for(int j = 0; j < swingCount; j++) {
                if(!swings[j].isBullish && swings[j].index > swings[i].index && 
                   swings[j].index < swings[i+1].index && close[0] > swings[j].price) {
            ...
            if(confirmedBreakout) {
                foundBullishCHOCH = true;
                bullishLevel = swings[i+1].price; // Use the higher low as the CHOCH level
                LogInfo("Bullish CHOCH detected at level " + DoubleToString(bullishLevel, _Digits));
            }
        }
        
        // Bearish CHOCH: Two swing highs where the second is lower than the first
        // indicating a change of character from uptrend to downtrend
        if(!swings[i].isBullish && !swings[i+1].isBullish && 
           swings[i+1].price < swings[i].price && 
           swings[i].strength >= 3 && swings[i+1].strength >= 3) {
            
            // Additional confirmation: We've broken below a prior swing low
            bool confirmedBreakdown = false;
            for(int j = 0; j < swingCount; j++) {
                if(swings[j].isBullish && swings[j].index > swings[i].index && 
                   swings[j].index < swings[i+1].index && close[0] < swings[j].price) {
                    confirmedBreakdown = true;
                    break;
                }
            }
            
            if(confirmedBreakdown) {
                foundBearishCHOCH = true;
                bearishLevel = swings[i+1].price; // Use the lower high as the CHOCH level
                LogInfo("Bearish CHOCH detected at level " + DoubleToString(bearishLevel, _Digits));
            }
        }
    }
    
    // Set the return values based on what we found
    if(foundBullishCHOCH) {
        isBullish = true;
        chochLevel = bullishLevel;
        return true;
    }
    else if(foundBearishCHOCH) {
        isBullish = false;
        chochLevel = bearishLevel;
        return true;
    }
...
//+------------------------------------------------------------------+
//| Check for CHOCH and modify stops if needed                       |
//+------------------------------------------------------------------+
void CheckAndModifyStopsOnCHOCH() {
    if(!EnableTrailingStop) return; // CHOCH stop modifications require trailing enabled
    
    bool isBullish = false;
    double chochLevel = 0;
    
    // Only proceed if we detect a valid CHOCH pattern
    if(!DetectCHOCH(isBullish, chochLevel)) return;
    
    // Loop through all open positions to find ones that can be modified
    int totalPositions = PositionsTotal();
    for(int i = 0; i < totalPositions; i++) {
        ulong ticket = PositionGetTicket(i);
        if(ticket <= 0) continue;
        
        // Only modify our own positions with our magic number
        if(PositionGetInteger(POSITION_MAGIC) != MagicNumber) continue;
        
        // Only modify positions for the current symbol
        if(PositionGetString(POSITION_SYMBOL) != Symbol()) continue;
        
        // Check if this is a buy or sell position
        bool isBuy = (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY);
        
        // Get current stop loss and entry price
        double currentSL = PositionGetDouble(POSITION_SL);
        double entryPrice = PositionGetDouble(POSITION_PRICE_OPEN);
        
        // Check if we should modify based on the CHOCH direction and position type
        bool shouldModify = false;
        double newSL = 0;
        
        // For buy positions, only modify if we have a bullish CHOCH (protect profits)
        // and if the CHOCH level is above our entry but below current price
        if(isBuy && isBullish) {
            double currentPrice = SymbolInfoDouble(Symbol(), SYMBOL_ASK);
            
            // Only move the stop if the CHOCH level is better than current stop
            // and the CHOCH level is above our entry price (locking profits)
            if(chochLevel > entryPrice && (currentSL < chochLevel || currentSL == 0)) {
                shouldModify = true;
                newSL = chochLevel - (SymbolInfoDouble(Symbol(), SYMBOL_POINT) * 10); // Small buffer
                LogInfo("Modifying BUY stop loss based on bullish CHOCH level");
            }
        }
        // For sell positions, only modify if we have a bearish CHOCH (protect profits)
        // and if the CHOCH level is below our entry but above current price
        else if(!isBuy && !isBullish) {
            double currentPrice = SymbolInfoDouble(Symbol(), SYMBOL_BID);
            
            // Only move the stop if the CHOCH level is better than current stop
            // and the CHOCH level is below our entry price (locking profits)
            if(chochLevel < entryPrice && (currentSL > chochLevel || currentSL == 0)) {
                shouldModify = true;
                newSL = chochLevel + (SymbolInfoDouble(Symbol(), SYMBOL_POINT) * 10); // Small buffer
                LogInfo("Modifying SELL stop loss based on bearish CHOCH level");
            }
        }
        
        // If we decided to modify the stop, do it
        if(shouldModify && newSL > 0) {
            CTrade trade;
            trade.SetDeviationInPoints(10);
            
            // Keep the current take profit
            double currentTP = PositionGetDouble(POSITION_TP);
            
            // Modify the position with the new stop loss
            if(!trade.PositionModify(ticket, newSL, currentTP)) {
                LogError("Failed to modify stop on CHOCH: " + 
                         IntegerToString(trade.ResultRetcode()) + ": " + 
                         trade.ResultRetcodeDescription());
            }
            else {
                LogInfo(StringFormat("Successfully modified stop to %.5f based on CHOCH level", newSL));
            }
        }
    }
}
...
input int LogLevel = 2;             // Logging level: 0=Errors only, 1=Warnings, 2=Info, 3=Debug
input bool EnableAdvancedLogging = false; // Enable detailed algorithm logging
input bool LogPredictiveAnalytics = true; // Log predictive analytics results

// Maximum order blocks already defined elsewhere

// Global configuration variables - these will be adjusted by asset class detection

// Define global configuration variables
double SpreadThreshold = 20.0;      // Maximum allowed spread in points
int MinOrderBlockStrength = 3;      // Minimum strength for valid order blocks
double OrderBlockMaxAge = 4.0;      // Maximum age for order blocks in hours
double MinSignalQuality = 0.5;      // Minimum quality for trade signals
...
void OnTimer() {
    ...
    if(currentTime - lastIndicatorCalc >= 2) {
        ...
        // Check for CHOCH patterns and modify stops if needed
        static datetime lastChochCheck = 0;
        if(currentTime - lastChochCheck >= 60) { // Check every minute
            CheckAndModifyStopsOnCHOCH();
            lastChochCheck = currentTime;
        }
    }
}
...
int OnInit()
{
    ...
    ConfigureForAssetClass(Symbol());
    ApplyAssetConfiguration();
    LogInfo("Asset configuration applied for " + Symbol());
    CreateDashboard();
...
//+------------------------------------------------------------------+
//| Enhanced CHOCH (Change of Character) detection                   |
//+------------------------------------------------------------------+
...
bool CanTrade() {
    ...
    // Check if we should avoid trading in the current volatility conditions
    bool volatilityOK = true;
    if(g_volatilityContext.volatilityState == VOLATILITY_VERY_HIGH) {
        // In extremely high volatility, only trade if we have a very strong signal quality
        // and only with reduced position size (already handled in CalculatePositionSize)
        volatilityOK = false;
        LogInfo("[VOLATILITY FILTER] Trading restricted in very high volatility - " + g_volatilityContext.marketDescription);
    }
    
    if(autoTradingEnabled && !emergencyMode && !marketClosed && !isWeekend && marginOK && !spreadOK) {
        // If current volatility is high, allow a wider spread tolerance based on volatility state
        double volatilitySpreadMultiplier = 1.0;
        
        // Apply spread threshold adjustment based on volatility state
        switch(g_volatilityContext.volatilityState) {
            case VOLATILITY_VERY_LOW:
            case VOLATILITY_LOW:
                volatilitySpreadMultiplier = 0.8; // More strict in low volatility
                break;
            case VOLATILITY_HIGH:
                volatilitySpreadMultiplier = 1.3; // More permissive in high volatility
                break;
            case VOLATILITY_VERY_HIGH:
                volatilitySpreadMultiplier = 1.5; // Much more permissive in very high volatility
                break;
            default:
                volatilitySpreadMultiplier = 1.0; // No change for normal volatility
        }
        ...
        if(currentRegime == REGIME_BREAKOUT || currentRegime == REGIME_HIGH_VOLATILITY) {
            volatilitySpreadMultiplier = 1.5; // 50% allowance during volatile conditions
...
void DetectOrderBlocks() {
    ...
    for(int i=0; i<MAX_BLOCKS; i++) {
        ...
        if(recentBlocks[i].valid) {
            // Check if price has touched/invalidated the block - use a more forgiving approach based on volatility
            // and asset class characteristics
            
            // Get current volatility state for dynamic adjustments
            bool isLowVol = (g_volatilityContext.volatilityState == VOLATILITY_VERY_LOW || 
                             g_volatilityContext.volatilityState == VOLATILITY_LOW);
            bool isHighVol = (g_volatilityContext.volatilityState == VOLATILITY_HIGH || 
                              g_volatilityContext.volatilityState == VOLATILITY_VERY_HIGH);
                              
            // Calculate dynamic invalidation threshold based on volatility
            double volFactor = isLowVol ? 1.5 : (isHighVol ? 0.8 : 1.0);
            
            // Adjust age check based on volatility - blocks remain valid longer in low volatility
            double maxAgeHours = workingOrderBlockMaxAge * volFactor;
...
void OnTick() {
    // 1. Dynamic asset-specific configuration
    static string lastSymbol = "";
    string currentSymbol = Symbol();
    
    // Configure asset-specific parameters when symbol changes
    if(currentSymbol != lastSymbol) {
        ConfigureForAssetClass(currentSymbol);
        lastSymbol = currentSymbol;
    }
    
    // Store original parameters before adjustments
    double originalSL_ATR_Mult = SL_ATR_Mult;
    double originalTP_ATR_Mult = TP_ATR_Mult;
    double originalSpreadThreshold = SpreadThreshold;
    int originalMinOrderBlockStrength = MinOrderBlockStrength;
    double originalOrderBlockMaxAge = OrderBlockMaxAge;
    double originalMinSignalQuality = MinSignalQuality;
    
    // 2. Apply asset-specific configuration
    ApplyAssetConfiguration();
    
    // 3. Dynamic volatility adaptation on top of asset configuration
    UpdateVolatilityContext(Symbol(), Period());
    
    // Apply volatility-based parameter adjustments
    ApplyVolatilityParameters(SL_ATR_Mult, TP_ATR_Mult, SpreadThreshold, 
                            MinOrderBlockStrength, OrderBlockMaxAge);
    
    // Log current state
    string volatilityDescription = GetCurrentVolatilityDescription();
    LogInfo("[MARKET ANALYSIS] Asset class: " + g_assetConfig.assetClass + ", Volatility: " + volatilityDescription);
    
    // 4. Generate predictive model
    if(EnablePredictiveAnalytics) {
        GeneratePredictiveModel(Symbol(), Period());
        
        // Add prediction info to dashboard
        if(g_predictiveModel.predictionQuality > 0.5) {
            LogInfo(StringFormat("[PREDICTION] %s (%.1f%% probability, %.1f%% quality)", 
                          g_predictiveModel.dominantDirection, 
                          (g_predictiveModel.dominantDirection == "BUY" ? 
                              g_predictiveModel.buyProbability : 
                              g_predictiveModel.sellProbability) * 100,
                          g_predictiveModel.predictionQuality * 100));
        }
    }
