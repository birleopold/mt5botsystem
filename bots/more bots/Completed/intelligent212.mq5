...
int OnInit()
{
   ...
   // Set up trade parameters for better execution
   CTrade trade;
   trade.SetExpertMagicNumber(MagicNumber);
...
double CalculateOptimalStopLoss(int signal, double entryPrice)
{
   ...
   for(int i=0; i<MAX_CHOCHS; i++) {
      ...
      if(signal > 0 && !recentCHOCHs[i].isBullish) {
         chochStop = recentCHOCHs[i].price - (atrValue * 0.5); // Add buffer below CHOCH
         foundChochStop = true;
      ...
      // For SELL positions, use bullish CHOCH as potential stop
         ...
         chochStop = recentCHOCHs[i].price + (atrValue * 0.5); // Add buffer above CHOCH
         foundChochStop = true;
   ...
   if(signal > 0) { // BUY position, look for recent swing low
      ...
      if(isSwingLow) {
         swingStop = lowestLow - (atrValue * 0.3); // Add buffer below swing
         foundSwingStop = true;
      ...
      if(isSwingHigh) {
         swingStop = highestHigh + (atrValue * 0.3); // Add buffer above swing
         foundSwingStop = true;
...
//+------------------------------------------------------------------+
bool RetryTradeExecutionWithErrorHandler(string symbol, datetime currentTime)
{
   ...
   double localSlMultiplier = 1.5 * localTfMultiplier;
   double localTpMultiplier = 2.0 * localTfMultiplier;
   
   double localStopLoss = currentPrice - (localAtrValue * localSlMultiplier);
   double localTakeProfit = currentPrice + (localAtrValue * localTpMultiplier);
   
   // Format price values as strings to avoid ambiguous access
   ...
   // Special handling for high-value assets (BTC, XAU, etc) with wider spreads
   if(StringFind(symbol, "BTC") >= 0 || StringFind(symbol, "XBT") >= 0 || 
      StringFind(symbol, "XAU") >= 0 || StringFind(symbol, "GOLD") >= 0) {
       
      // Use wider stops for high-value assets - higher ATR multipliers
      localSlMultiplier = 2.0 * localTfMultiplier;
      localTpMultiplier = 3.0 * localTfMultiplier;
      
      // Recalculate SL and TP with higher multipliers
   ...
   double tradeVolume = SymbolInfoDouble(symbol, SYMBOL_VOLUME_MIN); // Start with minimum volume
   ...
   {
      // Place a real buy order with proper stop loss and take profit
      if(trade.Buy(tradeVolume, symbol, 0, localStopLoss, localTakeProfit, "REAL")) {
         Print("[REAL-EXECUTION] Trade placed successfully with SL=", 
   ...
   // Final result report
   if(success) {
      Print("[REAL-EXECUTION] Trade execution successful after ", retryCount, " attempt(s)");
   } else {
      Print("[REAL-EXECUTION] FAILED trade execution after ", maxRetries, " attempts. Last error: ", 
...
bool RetryTrade(int signal, double price, double sl, double tp, double size, int maxRetries=3)
{
   ...
   double slMultiplier = 1.5 * tfMultiplier;
   double tpMultiplier = 2.0 * tfMultiplier;
   ...
   if(StringFind(Symbol(), "BTC") >= 0 || StringFind(Symbol(), "XBT") >= 0 || 
      ...
      // Use wider stops for high-value assets - higher ATR multipliers
      slMultiplier = 2.0 * tfMultiplier;
      tpMultiplier = 3.0 * tfMultiplier;
   ...
   Print("[REAL-TRADE] Using ATR-based SL/TP: ATR=", DoubleToString(atrValue/_Point, 1), " pips, SL=", 
         DoubleToString(MathAbs(adjustedPrice-newSL)/_Point, 1), " pips, TP=", 
         DoubleToString(MathAbs(newTP-adjustedPrice)/_Point, 1), " pips");
   ...
   for(int attempts = 0; attempts < maxRetries; attempts++) {
      ...
      bool result = false;
      if(signal > 0) {
         result = tradeMgr.Buy(size, Symbol(), 0, newSL, newTP, "REAL");
      } else {
         result = tradeMgr.Sell(size, Symbol(), 0, newSL, newTP, "REAL");
      }
      
      // Check result
      if(result) {
         Print("[REAL-TRADE] Trade placed successfully! Ticket:", tradeMgr.ResultOrder(), 
               " SL:", DoubleToString(newSL, digits), 
               " TP:", DoubleToString(newTP, digits));
               
         // DO NOT CLOSE THE TRADE - Let it run with proper SL/TP
         return true;
      } else {
         int error = GetLastError();
         Print("[REAL-TRADE] Attempt", attempts+1, "failed - Error:", error, 
               " (", tradeMgr.ResultRetcodeDescription(), ")");
         Sleep(500); // Longer delay before retry
      }
   }
   
   Print("[REAL-TRADE] All trade attempts failed");
   return false;
}
...
void UpdateVolatilityContext(string symbol, ENUM_TIMEFRAMES timeframe) {
    ...
    // Handle special case for BTC and high value assets like XAU
    if(StringFind(symbol, "BTC") >= 0 || StringFind(symbol, "XBT") >= 0 || 
       StringFind(symbol, "XAU") >= 0 || StringFind(symbol, "GOLD") >= 0) {
   ...
   if(HistoryDealSelect(dealTicket)) {
      ...
      if(magic == MagicNumber && symbol == Symbol()) {
         ...
         if(dealEntry == DEAL_ENTRY_OUT) { // Position closed
            ...
            // Track consecutive wins/losses for safety features
             if(!isWin) {
                ConsecutiveLosses++;
