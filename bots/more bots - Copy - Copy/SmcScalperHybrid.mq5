...
int GetTradingSignal() {
   ...
   if(EnableMarketRegimeFiltering && currentRegime >= 0) {
      ...
      if(currentRegime == CHOPPY && bestScore < MinBlockStrength + 1) {
         if(DisplayDebugInfo) Print("[SMC] Regime CHOPPY requires stronger block. No signal.");
         return 0;
      }
      
      // In high volatility, be more selective
      if(currentRegime == HIGH_VOLATILITY && bestScore < MinBlockStrength + 1) {
         if(DisplayDebugInfo) Print("[SMC] Regime HIGH_VOLATILITY requires stronger block. No signal.");
         return 0;
      }
      ...
      if(currentRegime == BREAKOUT) {
         ...
         if((shortTermUp && !recentBlocks[bestBlock].bullish) || 
            (!shortTermUp && recentBlocks[bestBlock].bullish)) {
      ...
      if((currentRegime == TRENDING_UP && !recentBlocks[bestBlock].bullish) || 
         (currentRegime == TRENDING_DOWN && recentBlocks[bestBlock].bullish)) {
         // We can still trade counter-trend, but require stronger confirmation
         if(bestScore < MinBlockStrength + 2) {
            if(DisplayDebugInfo) Print("[SMC] Counter-trend trade requires stronger block. No signal.");
            return 0;
         }
      }
   }
   ...
   if(!RequireTrendConfirmation) {
      if(recentBlocks[bestBlock].bullish) {
         if(DisplayDebugInfo) Print("[SMC] Buy signal generated. BlockScore=", bestScore);
         return 1;
      } else {
         if(DisplayDebugInfo) Print("[SMC] Sell signal generated. BlockScore=", bestScore);
         return -1;
      }
   }
   
   // Traditional trend confirmation
   ...
   if(recentBlocks[bestBlock].bullish && trendUp) {
      if(DisplayDebugInfo) Print("[SMC] Buy signal generated. BlockScore=", bestScore);
      return 1;
   }
   if(!recentBlocks[bestBlock].bullish && trendDown) {
      if(DisplayDebugInfo) Print("[SMC] Sell signal generated. BlockScore=", bestScore);
      return -1;
   }
   
   if(DisplayDebugInfo) Print("[SMC] No matching trend confirmation. No signal.");
   return 0;
}
...
void ManageTrailingStops() {
    ...
    for(int i=0; i<PositionsTotal(); i++) {
        ...
        if(comment != "SMC Buy" && comment != "SMC Sell") continue;
        ...
        if(profitPct >= TrailingActivationPct) {
            ...
            if(newSL != 0 && MathAbs(newSL - currentSL) > (3 * _Point)) {
                if(trade.PositionModify(ticket, newSL, currentTP)) {
                    if(DisplayDebugInfo) Print("[SMC] Updated trailing stop for ticket ", ticket, " to ", newSL);
                }
            }
        }
    }
