...
// --- MQL5 Time Helper Functions (substitute for MQL4 time functions) ---
int TimeHour(datetime dt) { MqlDateTime t; TimeToStruct(dt, t); return (int)t.hour; }
int TimeMinute(datetime dt) { MqlDateTime t; TimeToStruct(dt, t); return (int)t.min; }
int DayOfWeek(datetime dt) { MqlDateTime t; TimeToStruct(dt, t); return (int)t.day_of_week; }
...
input double SL_Pips = 10.0;           // Initial stop loss in pips (used if ATR-based stops disabled)
input double TP_Pips = 30.0;           // Initial take profit in pips (used if ATR-based TPs disabled)
...
input double MaxSpreadMultiplier = 1.5; // Maximum allowed spread (as multiple of average spread)
input int SpreadAveragingPeriod = 20;  // Period for calculating average spread

// ATR-Based Risk Management
input bool UseATRBasedStops = true;   // Use ATR for calculating stop loss distance
input double SL_Multiplier = 1.5;     // Stop loss as multiplier of ATR
input double TP_Multiplier = 3.0;     // Take profit as multiplier of ATR
input int ATR_Period = 14;            // Period for ATR calculation

// Early Exit Parameters
input bool EnableEarlyExit = true;    // Enable early exit for losing trades
input double EarlyExitATRMultiplier = 0.5; // Exit losing trades at this ATR multiple
input int EarlyExitTimeSeconds = 300; // Time in seconds before early exit activates

// Gold-Specific Parameters
input bool OptimizeForGold = true;    // Enable optimizations specifically for Gold trading
input bool UseGoldTimeFilter = true;  // Use time-based filtering for Gold trades
input int GoldAvoidStartHour = 12;    // Hour to start avoiding Gold trades (GMT)
input int GoldAvoidEndHour = 14;      // Hour to resume Gold trades (GMT)
input double GoldVolatilityMultiplier = 1.5; // Increase stop distance for Gold by this multiplier

// Gold Advanced Trading Features
input group "Gold Advanced Trading Features"
input bool UseAdvancedGoldTrailing = true;   // Use specialized trailing for Gold
input double GoldTrendTrailingFactor = 0.5; // Trailing factor during trending periods (lower = tighter)
input double GoldChoppyTrailingFactor = 0.8; // Trailing factor during choppy periods (higher = wider)

// Session-Based Adjustments
input bool UseSessionBasedParameters = true; // Adjust parameters based on market session
input double AsianSessionVolatilityFactor = 0.7; // Reduce volatility expectations in Asian session
input double EuropeanSessionVolatilityFactor = 1.2; // Increase volatility expectations in European session
input double USSessionVolatilityFactor = 1.5; // Increase volatility expectations in US session

// Gold Time-Based Exit
input bool UseGoldTimeBasedExit = true; // Enable time-based exits for Gold positions
input int MaxGoldTradeHoldingMinutes = 240; // Maximum time to hold Gold positions (minutes)
input double MinimumTargetPercent = 30.0; // Minimum target percent to achieve before time exit

// News Protection
input bool EnableNewsProtection = true; // Enable protection around news events
input int NewsProtectionMinutesBefore = 15; // Minutes to avoid trading before news
input int NewsProtectionMinutesAfter = 15; // Minutes to avoid trading after news
input bool ReducePositionSizeOnNewsDay = true; // Reduce position size on days with major news
input double NewsPositionSizeMultiplier = 0.5; // Position size multiplier during news days
...
bool isGold = false; // Whether current symbol is Gold

// Spread monitoring
double averageSpread = 0.0;
double spreadHistory[]; // Historical spread values

// ATR tracking
double currentATR = 0.0; // Ensure global declaration for current ATR
double atrBuffer[]; // Declare atrBuffer as a global array
...
double sessionVolatilityFactor = 1.0;

// Gold-specific tracking
datetime tradeOpenTimes[]; // Track open time of positions
datetime lastNewsTime = 0; // Last detected news event
bool inNewsProtectionPeriod = false;
double newsAdjustmentFactor = 1.0; // For position sizing adjustment during news
...
//+------------------------------------------------------------------+
//| Detect active market session based on GMT time                   |
//+------------------------------------------------------------------+
void DetectMarketSession()
{
    // Get current broker server time (should be GMT for most brokers)
    ...
    // Overlaps: 08:00-09:00 GMT (Asian-European) and 13:00-16:00 GMT (European-US)
    
    int previousSession = currentSession;
    
    if(hour >= 0 && hour < 8) {
        currentSession = SESSION_ASIAN;
        sessionVolatilityFactor = AsianSessionVolatilityFactor;
    }
    else if(hour >= 8 && hour < 13) {
        currentSession = SESSION_EUROPEAN;
        sessionVolatilityFactor = EuropeanSessionVolatilityFactor;
    }
    else if(hour >= 13 && hour < 16) {
        currentSession = SESSION_OVERLAP; // European-US overlap
        sessionVolatilityFactor = (EuropeanSessionVolatilityFactor + USSessionVolatilityFactor) / 2.0;
    }
    else if(hour >= 16 && hour < 21) {
        currentSession = SESSION_US;
        sessionVolatilityFactor = USSessionVolatilityFactor;
    }
    else {
        currentSession = SESSION_ASIAN; // Late US/Early Asian
        sessionVolatilityFactor = AsianSessionVolatilityFactor;
    }
    
    // Print session change if in debug mode
    if(previousSession != currentSession && DisplayDebugInfo) {
        string sessionName;
        switch(currentSession) {
            case SESSION_ASIAN: sessionName = "Asian"; break;
            case SESSION_EUROPEAN: sessionName = "European"; break;
            case SESSION_US: sessionName = "US"; break;
            case SESSION_OVERLAP: sessionName = "Overlap"; break;
            default: sessionName = "Unknown";
        }
        Print("[SMC] Market session changed to: ", sessionName, ", volatility factor: ", sessionVolatilityFactor);
    }
}
...
int OnInit()
{
    ...
    winStreak = 0;
    lossStreak = 0;
    Print("[Init] SMC Scalper Hybrid V3 initialized successfully");
    return INIT_SUCCEEDED;

    // Check if symbol is Gold (XAUUSD)
    isGold = (StringCompare(Symbol(), "XAUUSD") == 0);
    
    // Initialize spread history array
    ArrayResize(spreadHistory, SpreadAveragingPeriod);
...
void OnDeinit(const int reason)
{
    ...
    if(LogPerformanceStats) {
        ...
        // Log statistics
        Print("[Deinit] SMC Scalper Hybrid V3 terminated. Reason: ", reason);
...
//+------------------------------------------------------------------+
//| Check for gold positions that have been held too long             |
//+------------------------------------------------------------------+
void CheckGoldTimeBasedExits() {
    if(!UseGoldTimeBasedExit || !isGold || PositionsTotal() == 0) return;
    ...
    for(int i=0; i<PositionsTotal(); i++) {
        ...
        // Calculate how long this position has been open
        int minutesOpen = (int)((TimeCurrent() - openTime) / 60);
        
        // Calculate what percent of target has been achieved
        double totalPossiblePips = 0;
        double currentPips = 0;
        
        if(posType == POSITION_TYPE_BUY) {
            totalPossiblePips = (takeProfit - openPrice) / _Point;
            currentPips = (currentPrice - openPrice) / _Point;
        } else {
            totalPossiblePips = (openPrice - takeProfit) / _Point;
            currentPips = (openPrice - currentPrice) / _Point;
        }
        
        double achievedPercent = (totalPossiblePips > 0) ? (currentPips / totalPossiblePips * 100.0) : 0;
        
        // Exit if position has been open too long and hasn't achieved minimum target
        if(minutesOpen >= MaxGoldTradeHoldingMinutes && achievedPercent < MinimumTargetPercent) {
            // Check if we should really exit based on market conditions
            bool shouldExit = true;
            
            // If in a strong trend matching our position, maybe give a bit more time
            int regime = FastRegimeDetection(Symbol());
            if((posType == POSITION_TYPE_BUY && regime == TRENDING_UP) ||
               (posType == POSITION_TYPE_SELL && regime == TRENDING_DOWN)) {
                // In a strong trend matching our position direction
                shouldExit = (achievedPercent < MinimumTargetPercent / 2); // If it's made at least half progress, give more time
            }
            
            if(shouldExit) {
                trade.PositionClose(ticket);
                if(DisplayDebugInfo) {
                    Print("[SMC] Time-based exit for Gold position after ", minutesOpen, 
                          " minutes. Target achieved: ", achievedPercent, "%");
                }
            }
        }
    }
...
//+------------------------------------------------------------------+
//| Check for early exit conditions on open positions                |
//+------------------------------------------------------------------+
void CheckEarlyExits() {
    if(!EnableEarlyExit || PositionsTotal() == 0) return;
    ...
    for(int i=0; i<PositionsTotal(); i++) {
        ...
        // Exit if price moves against us by more than EarlyExitATRMultiplier * ATR after being open for at least EarlyExitTimeSeconds
        datetime openTime = (datetime)PositionGetInteger(POSITION_TIME);
        double atr = GetATR(Symbol(), PERIOD_CURRENT, ATR_Period, 0);
        double atrPips = atr / _Point;
        
        if(TimeCurrent() - openTime > EarlyExitTimeSeconds && profitPips < -EarlyExitATRMultiplier * atrPips) {
            // Gold positions may need special handling
            bool shouldExit = true;
            
            // For gold, be more careful with early exits during certain market conditions
            if(isGold && OptimizeForGold) {
                // Check if we're in a current strong trend - if so, maybe give more room
                int regime = FastRegimeDetection(Symbol());
                if((posType == POSITION_TYPE_BUY && regime == TRENDING_UP) ||
                   (posType == POSITION_TYPE_SELL && regime == TRENDING_DOWN)) {
                    // In a strong trend matching our position, maybe give a bit more room
                    shouldExit = (profitPips < -EarlyExitATRMultiplier * atrPips * 1.2);
                }
            }
            
            if(shouldExit) {
                trade.PositionClose(ticket);
                if(DisplayDebugInfo) Print("[SMC] Early exit - cutting loss after ", 
                                          (TimeCurrent() - openTime), " seconds, ", 
                                          profitPips, " pips");
            }
        }
    }
...
//+------------------------------------------------------------------+
//| Evaluate quality of potential entry based on volatility patterns |
//+------------------------------------------------------------------+
double EvaluateEntryQuality(int signal) {
    // Start with base quality score of 1.0
    double qualityScore = 1.0;
    
    // A. Check volatility trend
    double atr1 = GetATR(Symbol(), PERIOD_CURRENT, ATR_Period, 1);
    double atr2 = GetATR(Symbol(), PERIOD_CURRENT, ATR_Period, 2);
    double atr3 = GetATR(Symbol(), PERIOD_CURRENT, ATR_Period, 3);
    
    // Rising volatility is often good for new entries
    bool volatilityRising = (currentATR > atr1 && atr1 > atr2);
    bool volatilityFalling = (currentATR < atr1 && atr1 < atr2);
    
    if(volatilityRising) qualityScore *= 1.2; // Better entries during volatility expansion
    if(volatilityFalling) qualityScore *= 0.8; // Worse entries during volatility contraction
    
    // B. Check current market session
    if(UseSessionBasedParameters) {
        // Adjust score based on session - lower during Asian, higher during US
        if(currentSession == SESSION_ASIAN) qualityScore *= 0.85;
        if(currentSession == SESSION_EUROPEAN) qualityScore *= 1.0;
        if(currentSession == SESSION_US) qualityScore *= 1.15;
        if(currentSession == SESSION_OVERLAP) qualityScore *= 1.1;
    }
    
    // C. Check for excessive volatility (might be too risky)
    double avgATR = (currentATR + atr1 + atr2 + atr3) / 4;
    if(currentATR > avgATR * 2.0) {
        qualityScore *= 0.7; // Penalize entries during extreme volatility
    }
    
    // D. For gold, apply special considerations
    if(isGold) {
        // Gold performs better on trend continuations rather than reversals
        int regime = FastRegimeDetection(Symbol());
        
        if((signal > 0 && regime == TRENDING_UP) || 
           (signal < 0 && regime == TRENDING_DOWN)) {
            qualityScore *= 1.3; // Reward trend-following entries for gold
        }
        
        if((signal > 0 && regime == TRENDING_DOWN) || 
           (signal < 0 && regime == TRENDING_UP)) {
            qualityScore *= 0.7; // Penalize counter-trend entries for gold
        }
        
        // Gold entries during ranging markets might be problematic
        if(regime == RANGING_NARROW || regime == CHOPPY) {
            qualityScore *= 0.75; // Reduce score for entries during choppy conditions
        }
    }
    
    // E. Check for entry near key SMC structures
    // Higher quality if we have multiple structure confirmations
    int confirmationCount = 0;
    if(bosCount > 0) confirmationCount++;
    if(chochCount > 0) confirmationCount++;
    
    qualityScore *= (1.0 + 0.1 * confirmationCount); // Each confirmation adds 10%
...
void OnTick()
{
    // Step 0: Update market status and session information
    DetectMarketSession(); // Update market session awareness
    ...
    // Update ATR value
    currentATR = GetATR(Symbol(), PERIOD_CURRENT, ATR_Period, 0);
    ...
    // Step 4: Execute trades if signals are valid
    if(signal != 0 && TimeCurrent() - lastSignalTime > SignalCooldownSeconds) {
        // Evaluate entry quality based on volatility patterns and market conditions
        double entryQuality = EvaluateEntryQuality(signal);
        
        // For gold, apply stricter entry quality requirements
        double qualityThreshold = isGold ? 1.2 : 0.9; // Higher threshold for gold
        
        if(entryQuality >= qualityThreshold) {
            ...
            if(signal != 0) {
                ...
                if(DisplayDebugInfo) {
                    Print("[SMC] Trade executed with entry quality score: ", entryQuality,
                          ", threshold: ", qualityThreshold);
                }
            }
        } else {
            // Skip this entry due to poor quality
            if(DisplayDebugInfo) {
                Print("[SMC] Trade signal skipped due to low entry quality score: ", entryQuality,
    ...
    // Check for gold-specific time-based exits
    if(isGold && UseGoldTimeBasedExit) {
        CheckGoldTimeBasedExits();
    }
...
double GetATR(string symbol, ENUM_TIMEFRAMES tf, int period, int shift) {
    ...
    if(handle != INVALID_HANDLE) {
        if(CopyBuffer(handle, 0, shift, 1, atrBuffer) == 1) {
...
bool CheckSpread() {
    ...
    for(int i=SpreadAveragingPeriod-1; i>0; i--) {
        spreadHistory[i] = spreadHistory[i-1];
    }
    spreadHistory[0] = currentSpread;
    
    // Calculate average spread
    double sum = 0;
    for(int i=0; i<SpreadAveragingPeriod; i++) {
        if(spreadHistory[i] > 0) sum += spreadHistory[i];
    }
    
    int validValues = 0;
    for(int i=0; i<SpreadAveragingPeriod; i++) {
        if(spreadHistory[i] > 0) validValues++;
    }
    
    if(validValues > 0) {
        averageSpread = sum / validValues;
    } else {
        averageSpread = currentSpread; // Use current if no history
    }
    
    // Check if current spread is too high
    if(currentSpread > averageSpread * MaxSpreadMultiplier) {
        if(DisplayDebugInfo) Print("[SMC] Spread too high: Current=", currentSpread, ", Average=", averageSpread);
        return false;
    }
    
    // For gold, be even more careful with spread during volatile hours
    if(isGold && UseGoldTimeFilter) {
        int currentHour = TimeHour(TimeCurrent());
        if(currentHour >= GoldAvoidStartHour && currentHour < GoldAvoidEndHour) {
            if(currentSpread > averageSpread * 1.2) { // Be more strict during volatile hours
                if(DisplayDebugInfo) Print("[SMC] Gold spread too high during volatile hours: ", currentSpread);
                return false;
            }
        }
    }
...
//+------------------------------------------------------------------+
bool CanTrade() {
    // Check for emergency mode (stop trading after too many consecutive losses)
    if(emergencyMode) { 
        if(DisplayDebugInfo) Print("[SMC] Trading disabled due to emergency mode");
        return false; 
    }
    
    // Check if market is closed
    if(marketClosed) {
        if(DisplayDebugInfo) Print("[SMC] Market is closed, trading disabled");
        return false;
    }
    ...
    if(hour < TradingStartHour || hour >= TradingEndHour) {
        if(DisplayDebugInfo) Print("[SMC] Outside trading hours: ", hour);
        return false;
    }
    
    // Check if max trades reached
    if(PositionsTotal() >= MaxTrades) {
        if(DisplayDebugInfo) Print("[SMC] Max trades reached: ", PositionsTotal());
        return false;
    }
    
    // Check for signal cooldown
    if(TimeCurrent() - lastSignalTime < SignalCooldownSeconds) {
        if(DisplayDebugInfo) Print("[SMC] Signal cooldown in effect");
        return false;
    }
    ...
    // Gold-specific time filtering
    if(isGold && UseGoldTimeFilter) {
        int currentHour = TimeHour(TimeCurrent());
        if(currentHour >= GoldAvoidStartHour && currentHour < GoldAvoidEndHour) {
            if(DisplayDebugInfo) Print("[SMC] Avoiding Gold trades during high volatility hours: ", currentHour);
            return false;
        }
    }
...
//+------------------------------------------------------------------+
//| Check for news events and adjust trading accordingly              |
//+------------------------------------------------------------------+
bool IsHighImpactNewsTime() { 
    // This is a placeholder for checking news events
    // In a real implementation, you would check an economic calendar API
    // or a file-based calendar of news events
    
    // Simple implementation: avoid trading at typical news times
    // Major news usually happens at round hours like 8:30, 12:00, 14:00
    datetime currentTime = TimeCurrent();
    int minute = TimeMinute(currentTime);
    int hour = TimeHour(currentTime);
    
    // Common news release times (in GMT)
    if(((hour == 8 && minute >= 25 && minute <= 35) ||   // US NFP, CPI, etc. 8:30 GMT
        (hour == 12 && minute >= 0 && minute <= 10) ||   // US Fed announcements 12:00 GMT
        (hour == 14 && minute >= 0 && minute <= 10) ||   // Various releases 14:00 GMT
        (hour == 18 && minute >= 0 && minute <= 10)) &&  // FOMC minutes 18:00 GMT
       (DayOfWeek(currentTime) >= MONDAY && DayOfWeek(currentTime) <= FRIDAY)) {
        
        // Update news protection status
        inNewsProtectionPeriod = true;
        lastNewsTime = currentTime;
        
        // Store current news status for position sizing adjustment
        if(ReducePositionSizeOnNewsDay) {
            newsAdjustmentFactor = NewsPositionSizeMultiplier;
        }
        
        if(DisplayDebugInfo) Print("[SMC] News protection active: potential high impact news time");
        return true;
    }
    
    // Also protect during the minutes after a news release
    if(inNewsProtectionPeriod) {
        int secondsSinceNews = (int)(currentTime - lastNewsTime);
        if(secondsSinceNews <= NewsProtectionMinutesAfter * 60) {
            return true; // Still in protection period after news
        } else {
            inNewsProtectionPeriod = false; // Protection period ended
            newsAdjustmentFactor = 1.0; // Reset news adjustment
        }
    }
...
bool ExecuteTrade(int signal) {
    ...
    double positionSizeInLots = riskAmount / (SL_Pips * point * pointValue);
    ...
    if(UseATRBasedStops) {
        // Get current ATR for volatility-based stops
        double currentATR = GetATR(Symbol(), PERIOD_CURRENT, ATR_Period, 0);
        
        // Adjust ATR multiplier for Gold if needed
        double slMultiplier = SL_Multiplier;
        double tpMultiplier = TP_Multiplier;
        
        if(isGold && OptimizeForGold) {
            slMultiplier *= GoldVolatilityMultiplier; // Gold often requires wider stops
            if(DisplayDebugInfo) Print("[SMC] Using increased stop distance for Gold: ", slMultiplier, "x ATR");
        }
        
        // Calculate SL/TP levels based on ATR
        double slDistance = currentATR * slMultiplier;
        double tpDistance = currentATR * tpMultiplier;
        ...
        // Use fixed pip values if ATR-based stops disabled
        if(signal > 0) {
            slPrice = currentPrice - SL_Pips * _Point;
            tpPrice = currentPrice + TP_Pips * _Point;
        } else {
            slPrice = currentPrice + SL_Pips * _Point;
            tpPrice = currentPrice - TP_Pips * _Point;
        }
...
//+------------------------------------------------------------------+
//| Advanced Gold-Specific Trailing Stop Management                 |
//+------------------------------------------------------------------+
void ManageGoldTrailingStops() {
    if(!UseAdvancedGoldTrailing || !isGold || PositionsTotal() == 0) return;
    
    // Get current regime to adjust trailing behavior
    int regime = FastRegimeDetection(Symbol());
    bool isTrending = (regime == TRENDING_UP || regime == TRENDING_DOWN);
    
    // Adjust trailing factor based on market regime
    double trailingFactor = isTrending ? GoldTrendTrailingFactor : GoldChoppyTrailingFactor;
    
    // Also adjust for current market session
    if(UseSessionBasedParameters) {
        // Make trailing tighter during US session (more decisive moves)
        // Make trailing wider during Asian session (more noise)
        if(currentSession == SESSION_US) {
            trailingFactor *= 0.9; // Tighter trailing in US session
        } else if(currentSession == SESSION_ASIAN) {
            trailingFactor *= 1.2; // Wider trailing in Asian session
        }
    }
    ...
    for(int i=0; i<PositionsTotal(); i++) {
        ...
        // Calculate trailing distance based on ATR and market conditions
        double trailingDistance = atr * trailingFactor * currentATR / atr;
        ...
        if(posType == POSITION_TYPE_BUY) {
            ...
            shouldModify = (newSL > sl && newSL < currentPrice); // Only move SL up
        } else {
            newSL = currentPrice + trailingDistance;
            shouldModify = (newSL < sl && newSL > currentPrice); // Only move SL down
        }
        
        // Apply the trailing stop modification
        if(shouldModify) {
            trade.PositionModify(ticket, newSL, tp);
            if(DisplayDebugInfo) {
                Print("[SMC Gold] Advanced trailing stop adjusted to ", newSL, 
...
void ManageTrailingStops() {
    ...
    // For Gold, use specialized trailing if enabled
    if(isGold && UseAdvancedGoldTrailing) {
        ManageGoldTrailingStops();
        return;
    }
