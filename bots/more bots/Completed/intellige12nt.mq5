...
void UpdateAdaptivePairSettings() {
   ...
   if(g_pairSettings[0].performance.totalTrades < 5) return;
   
   Print("[ADAPTIVE] Updating pair settings for ", g_pairSettings[0].symbol);
   
   // PATTERN 1: Stop Loss Frequency Adaptation
   // If we're hitting stop losses too frequently, adjust SL and entry criteria
   double slHitRate = g_pairSettings[0].performance.stopLossHits / (double)g_pairSettings[0].performance.totalTrades;
   if(slHitRate > 0.3 && g_pairSettings[0].performance.totalTrades > 10) {
      // Too many stop losses - widen stops and be more selective with entries
      g_pairSettings[0].slMultiplier += 0.2;
      g_pairSettings[0].orderBlockMinStrength += 0.5;
      Print("[ADAPTIVE] High SL hit rate (", DoubleToString(slHitRate*100, 1), "%). Increased SL multiplier to ", 
            DoubleToString(g_pairSettings[0].slMultiplier, 2), " and block threshold to ", 
            DoubleToString(g_pairSettings[0].orderBlockMinStrength, 2));
   }
   
   // PATTERN 2: Win Rate Adaptation
   // If win rate is very high, we can be more aggressive
   if(g_pairSettings[0].performance.winRate > 0.65 && g_pairSettings[0].performance.totalTrades > 10) {
      g_pairSettings[0].orderBlockMinStrength = MathMax(1.0, g_pairSettings[0].orderBlockMinStrength - 0.2);
      g_pairSettings[0].spreadThreshold += 0.05;
      Print("[ADAPTIVE] High win rate (", DoubleToString(g_pairSettings[0].performance.winRate*100, 1), 
            "%). Lowered block threshold to ", DoubleToString(g_pairSettings[0].orderBlockMinStrength, 2), 
            " and increased spread tolerance to ", DoubleToString(g_pairSettings[0].spreadThreshold, 2));
   }
   
   // PATTERN 3: Multiple Consecutive Losses
   // If we have multiple consecutive losses, become more conservative
   if(g_pairSettings[0].performance.consecutiveLosses > 3) {
      g_pairSettings[0].orderBlockMinStrength += 1.0;
      g_pairSettings[0].signalQualityThreshold += 0.5;
      Print("[ADAPTIVE] ", g_pairSettings[0].performance.consecutiveLosses, " consecutive losses. Increased block threshold to ", 
            DoubleToString(g_pairSettings[0].orderBlockMinStrength, 2), " and signal quality threshold to ", 
            DoubleToString(g_pairSettings[0].signalQualityThreshold, 2));
   }
   ...
   if(g_volatilityContext.volatilityState == VOLATILITY_HIGH || g_volatilityContext.volatilityState == VOLATILITY_VERY_HIGH) {
      ...
      g_pairSettings[0].orderBlockMinStrength += 0.5;
      g_pairSettings[0].slMultiplier += 0.3;
      ...
      g_pairSettings[0].orderBlockMinStrength = MathMax(1.0, g_pairSettings[0].orderBlockMinStrength - 0.3);
      g_pairSettings[0].slMultiplier = MathMax(1.0, g_pairSettings[0].slMultiplier - 0.1);
   ...
   if(g_pairSettings[0].orderBlockSuccessRate < 0.4 && g_pairSettings[0].performance.totalTrades > 15) {
      // Poor order block performance, be more selective
      g_pairSettings[0].orderBlockMinStrength += 0.8;
      g_pairSettings[0].orderBlockAgeHours = MathMax(1.0, g_pairSettings[0].orderBlockAgeHours - 0.5);
   ...
   g_pairSettings[0].orderBlockMinStrength = MathMax(IsHighValueAsset(g_pairSettings[0].symbol) ? 0.5 : 1.0, 
                                                  g_pairSettings[0].orderBlockMinStrength);
   // Order block strength maximum cap                                               
   g_pairSettings[0].orderBlockMinStrength = MathMin(10.0, g_pairSettings[0].orderBlockMinStrength);
   // SL multiplier caps
   g_pairSettings[0].slMultiplier = MathMax(1.0, g_pairSettings[0].slMultiplier);
   g_pairSettings[0].slMultiplier = MathMin(5.0, g_pairSettings[0].slMultiplier);
   // Spread threshold caps
   g_pairSettings[0].spreadThreshold = MathMax(0.1, g_pairSettings[0].spreadThreshold);
   g_pairSettings[0].spreadThreshold = MathMin(1.0, g_pairSettings[0].spreadThreshold);
   // Age hours caps
   g_pairSettings[0].orderBlockAgeHours = MathMax(1.0, g_pairSettings[0].orderBlockAgeHours);
   g_pairSettings[0].orderBlockAgeHours = MathMin(24.0, g_pairSettings[0].orderBlockAgeHours);
   
   // Record the adaptation
   totalAdaptations++;
   g_pairSettings[0].lastUpdated = currentTime;
   
   Print("[ADAPTIVE] Completed parameter update #", totalAdaptations, " for ", g_pairSettings[0].symbol);
}

//+------------------------------------------------------------------+
//| Record trade result and update performance metrics               |
//+------------------------------------------------------------------+
void UpdateTradePerformance(bool isWin, double profit, bool wasStopLoss, bool wasTakeProfit, int holdTimeMinutes) {
   // Update total trades count
   g_pairSettings[0].performance.totalTrades++;
   
   // Update win/loss counts and streaks
   if(isWin) {
      g_pairSettings[0].performance.winningTrades++;
      g_pairSettings[0].performance.consecutiveWins++;
      g_pairSettings[0].performance.consecutiveLosses = 0;
      if(g_pairSettings[0].performance.consecutiveWins > g_pairSettings[0].performance.maxConsecutiveWins)
         g_pairSettings[0].performance.maxConsecutiveWins = g_pairSettings[0].performance.consecutiveWins;
      
      // If it was a take profit hit, record it
      if(wasTakeProfit) g_pairSettings[0].performance.takeProfitHits++;
   } else {
      g_pairSettings[0].performance.losingTrades++;
      g_pairSettings[0].performance.consecutiveLosses++;
      g_pairSettings[0].performance.consecutiveWins = 0;
      if(g_pairSettings[0].performance.consecutiveLosses > g_pairSettings[0].performance.maxConsecutiveLosses)
         g_pairSettings[0].performance.maxConsecutiveLosses = g_pairSettings[0].performance.consecutiveLosses;
      
      // If it was a stop loss hit, record it
      if(wasStopLoss) g_pairSettings[0].performance.stopLossHits++;
   }
   
   // Update win rate
   g_pairSettings[0].performance.winRate = g_pairSettings[0].performance.winningTrades / 
                                        (double)g_pairSettings[0].performance.totalTrades;
   
   // Update profit/loss metrics
   if(isWin) {
      g_pairSettings[0].performance.avgProfit = ((g_pairSettings[0].performance.avgProfit * 
                                           (g_pairSettings[0].performance.winningTrades - 1)) + profit) / 
                                           g_pairSettings[0].performance.winningTrades;
   } else {
      g_pairSettings[0].performance.avgLoss = ((g_pairSettings[0].performance.avgLoss * 
                                        (g_pairSettings[0].performance.losingTrades - 1)) + MathAbs(profit)) / 
                                        g_pairSettings[0].performance.losingTrades;
   }
   
   // Update profit factor if we have both wins and losses
   if(g_pairSettings[0].performance.losingTrades > 0) {
      g_pairSettings[0].performance.profitFactor = (g_pairSettings[0].performance.avgProfit * 
                                               g_pairSettings[0].performance.winningTrades) / 
                                              (g_pairSettings[0].performance.avgLoss * 
                                               g_pairSettings[0].performance.losingTrades);
   }
   
   // Update average hold time
   g_pairSettings[0].performance.avgTradeHoldTime = ((g_pairSettings[0].performance.avgTradeHoldTime * 
                                               (g_pairSettings[0].performance.totalTrades - 1)) + 
                                               holdTimeMinutes) / g_pairSettings[0].performance.totalTrades;
   ...
   g_pairSettings[0].orderBlockSuccessRate = g_pairSettings[0].orderBlockSuccessRate * (1-updateWeight) + 
                                          (isWin ? updateWeight : 0);
   
   Print("[ADAPTIVE] Updated performance metrics for ", g_pairSettings[0].symbol, 
         ". Win Rate: ", DoubleToString(g_pairSettings[0].performance.winRate*100, 1), "%");
...
input int MaxOpenPositions = 3;         // Maximum number of open positions allowed
...
input double DailyLossLimit = 5.0;     // Max daily loss percentage (% of balance)
input int MinMinutesBetweenTrades = 5; // Minimum time between trades (minutes)
input double RiskPerTrade = 1.0;       // Risk % per trade for position sizing
...
input bool EnableSafetyLogging = true; // Enable detailed safety logging
...
void AnalyzePairBehavior(string symbol) {
   ...
   // Find the index for this pair in our settings array
   int pairIndex = 0;
   ...
   for(int i=0; i<MAX_PAIRS; i++) {
      ...
      // If we find an empty slot, we can use it for a new pair
      if(g_pairSettings[i].symbol == "") {
         pairIndex = i;
         g_pairSettings[i].symbol = symbol;
         g_pairSettings[i].behavior = PAIR_UNKNOWN;
         ...
         // Initialize with default adaptive parameters
         g_pairSettings[i].orderBlockMinStrength = 6.0; 
         g_pairSettings[i].slMultiplier = 1.5;          
         g_pairSettings[i].tpMultiplier = 2.0;         
         g_pairSettings[i].spreadThreshold = 0.2;      
         g_pairSettings[i].orderBlockAgeHours = 3.0;   
         pairFound = true;
         break;
      }
   }
   ...
   if(!pairFound) {
      Print("[ADAPTIVE] WARNING: Could not find or create settings for pair: ", symbol);
      return;
   }
   ...
   double atrRatio = atrValue / avgATR;
...
{
   // Use adaptive stop loss multiplier from pair settings
   double slMultiplier = g_pairSettings[0].slMultiplier;
...
bool IsTradingAllowed()
{
   ...
   // Check spread using adaptive threshold
   double spreadRatio = SymbolInfoInteger(Symbol(), SYMBOL_SPREAD) * SymbolInfoDouble(Symbol(), SYMBOL_POINT) / GetATR(Symbol(), PERIOD_CURRENT, 14, 0);
   if(spreadRatio > g_pairSettings[0].spreadThreshold) {
      Print("[FILTER] Spread too high: ", DoubleToString(spreadRatio, 2), " > ", DoubleToString(g_pairSettings[0].spreadThreshold, 2));
...
{
   // Enhanced order block detection improvements
   double minBlockStrength = 2.0; // Default minimum strength
   if(IsHighValueAsset() && EnableSpecialHandlingForBTC) {
      minBlockStrength = 1.0; // Reduced minimum block strength for high-value assets
      Print("[BLOCK] Using reduced block strength threshold (1.0) for high-value asset");
   }
   double requiredBodySize = atrValue * 0.5;
   if(IsHighValueAsset() && EnableSpecialHandlingForBTC) {
      requiredBodySize *= 0.5; // 50% reduction in size requirements for high-value assets
      Print("[BLOCK] Using reduced block size requirements for high-value asset");
   }
   // ... (rest of your block detection logic follows, using minBlockStrength and requiredBodySize as needed)
...
{
   // Get pair-specific adaptive settings
   double minBlockStrength = g_pairSettings[0].orderBlockMinStrength;
   double maxBlockAgeHours = g_pairSettings[0].orderBlockAgeHours;
...
bool RetryTradeExecutionWithErrorHandler(string symbol, datetime currentTime, int signal = 1)
{
   ...
   // If we've reached the maximum, don't place another trade
   if(openPositions >= MaximumOpenTrades) {
      Print("[REAL-EXECUTION] Maximum open trades (", MaximumOpenTrades, ") already reached. Skipping trade execution.");
      return false;
   }
   
   Print("[REAL-EXECUTION] Executing real trade for ", symbol, ". Current open positions: ", openPositions, "/", MaximumOpenTrades);
   ...
   // Make sure we have a valid trade volume
   if(tradeVolume <= 0) {
      Print("[REAL-EXECUTION] Invalid trade volume calculated: ", DoubleToString(tradeVolume, 2), ". Using minimum allowed.");
      tradeVolume = 0.01; // Use minimum trade size as fallback
   }
   
   // Apply trade direction based on signal
   while(retryCount < maxRetries && !success)
   {
      // Place order based on signal direction
      if(tradeSignal > 0) {
         // Place a buy order with proper stop loss and take profit
         if(trade.Buy(tradeVolume, symbol, 0, localStopLoss, localTakeProfit, "REAL")) {
            Print("[REAL-EXECUTION] BUY trade placed successfully with SL=", 
                  DoubleToString(localStopLoss, localDigits), ", TP=", 
                  DoubleToString(localTakeProfit, localDigits));
            success = true;
         }
      } else {
         // Place a sell order with proper stop loss and take profit
         if(trade.Sell(tradeVolume, symbol, 0, localStopLoss, localTakeProfit, "REAL")) {
            Print("[REAL-EXECUTION] SELL trade placed successfully with SL=", 
                  DoubleToString(localStopLoss, localDigits), ", TP=", 
                  DoubleToString(localTakeProfit, localDigits));
            success = true;
         }
      ...
      if(!success) {
         ...
         Print("[REAL-EXECUTION] Failed to place trade: ", IntegerToString(lastError));
      }
      ...
      if(!success && retryCount < maxRetries) {
         Print("[REAL-EXECUTION-RETRY] Attempt ", retryCount, " failed with error code: ", lastError, 
               " (", IntegerToString(lastError), "). Retrying...");
   ...
   // Final result report
   if(success) {
      Print("[REAL-EXECUTION-SUCCESS] Trade successfully executed after ", retryCount, " attempts.");
      ...
      Print("[REAL-EXECUTION-FAILED] Failed to execute trade after ", maxRetries, " attempts. Last error: ", lastError);
      return false;
   }
}
                       ...
                       const MqlTradeResult& result)
{
   ...
   // Find the pair index in our settings array
   int pairIndex = -1;
   for(int i=0; i<MAX_PAIRS; i++) {
      if(g_pairSettings[i].symbol == symbol) {
         pairIndex = i;
   ...
   // If we don't have this pair in our settings yet, initialize it
   if(pairIndex == -1) {
      for(int i=0; i<MAX_PAIRS; i++) {
         if(g_pairSettings[i].symbol == "") {
            pairIndex = i;
      ...
      // If we still couldn't find a slot, use index 0 (current symbol)
...
{
   // Don't process if no valid CHOCHs or no open positions
   if(PositionsTotal() == 0) return;
   
   // Get current symbol - CHOCH patterns are symbol-specific
   ...
   // Only use recent CHOCHs (within last 4 hours)
   if(currentTime - recentCHOCHs[latestCHOCHIndex].time > 4 * 3600) return;
   
   Print("[CHOCH_STOPS] Processing stop modifications based on recent CHOCH pattern for ", currentSymbol);
   ...
   for(int i=PositionsTotal()-1; i>=0; i--) {
      ...
      if(posType == POSITION_TYPE_BUY && !recentCHOCHs[latestCHOCHIndex].isBullish) {
         double newSL = recentCHOCHs[latestCHOCHIndex].price - (::atrValue * 0.5); // Buffer below CHOCH
         
         // Only modify if new SL is higher (better) than current SL
         if(newSL > currentSL) {
      ...
      // For short positions, use bullish CHOCH to adjust stops
         ...
         double newSL = recentCHOCHs[latestCHOCHIndex].price + (::atrValue * 0.5); // Buffer above CHOCH
         
         // Only modify if new SL is lower (better) than current SL
...
void UpdateVolatilityContext(string symbol, ENUM_TIMEFRAMES timeframe) {
    ...
    if(higherTf != PERIOD_CURRENT) {
        double higherTfATR = GetATR(symbol, higherTf, 14, 0);
...
int GetPairSettingsIndex(string symbol)
{
   ...
   // First try to find an exact match for the symbol
   for(int i=0; i<MAX_PAIRS; i++) {
      if(g_pairSettings[i].symbol == symbol) {
         return i;
      }
   ...
   // If we get here, the symbol wasn't found in the settings array
   // Log this unusual situation
   Print("[WARNING] No settings found for symbol: ", symbol, ". Using default settings.");
   return defaultIndex;
}
