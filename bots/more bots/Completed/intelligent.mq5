...
input int VolatilityLookback = 7;    // Reduced periods for volatility calculation (from 20)
input int TrendStrengthLookback = 5; // Reduced periods for trend strength calculation (from 25)
...
ENUM_MARKET_REGIME DetectMarketRegime()
{
   ...
   if(CopyClose(Symbol(), PERIOD_CURRENT, 0, lookback + 5, close) <= 0) return REGIME_RANGING;
   if(CopyHigh(Symbol(), PERIOD_CURRENT, 0, lookback + 5, high) <= 0) return REGIME_RANGING;
   if(CopyLow(Symbol(), PERIOD_CURRENT, 0, lookback + 5, low) <= 0) return REGIME_RANGING;
...
bool ExecuteTradeWithSignal(int signal, double entryPrice, double stopLoss)
{
   ...
   CTrade trade;
   trade.SetExpertMagicNumber(MagicNumber);
   trade.SetDeviationInPoints((ulong)AdaptiveSlippagePoints); // Apply slippage settings
   bool result = false;
...
void GetLatestPriceData()
{
   ...
   double atrBuffer[];
   int atrHandleLocal = iATR(Symbol(), PERIOD_CURRENT, 7); // Reduced from 14 to match shorter analysis period
   ArraySetAsSeries(atrBuffer, true);
   ...
   double avgVolume = totalVolume / MathMin(10, copied);
   ...
   // Swing detection - reduced window size to analyze fewer candles
   int swingWindow = 1; // Smaller window to analyze just a few candles
   ...
   for(int i=1; i<copied-1 && validBlocks < MAX_BLOCKS; i++) {
...
void ManageATRTrailingStops()
{
    ...
    for(int i = 0; i < PositionsTotal(); i++) {
        ...
        if(profitPips >= activationThreshold) {
            ...
            double trailingDistance = atrValue * TrailingStopATRMultiplier;
                    ...
                    if(trade.PositionModify(ticket, newStopLoss, posTakeProfit)) {
                        if(EnableTrailingDebug) {
                            Print("[TRAILING] Updated BUY stop loss from ", posStopLoss, " to ", newStopLoss, 
                                  " (profit: ", profitPips, " pips, ATR: ", atrValue / pipSize, " pips)");
                        }
                    } else {
