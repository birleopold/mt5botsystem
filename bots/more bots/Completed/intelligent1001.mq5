...
{
   // Get current symbol spread in points
   ...
   // Base acceptable spread threshold (in points)
   double maxAcceptableSpread = 20.0; // Default
   
   // If we have ATR, make it dynamic based on volatility
   if(atr > 0) {
      // Convert ATR to points (ATR is in price units)
      double atrPoints = atr / Point();
      maxAcceptableSpread = atrPoints * 0.25; // 25% of ATR by default
      
      // Special handling for high-value assets like BTC/XAU
      string symbolName = Symbol();
      if(EnableSpecialHandlingForBTC && 
         (StringFind(symbolName, "BTC") >= 0 || 
          StringFind(symbolName, "XBT") >= 0 || 
          StringFind(symbolName, "XAU") >= 0)) {
         // Use the special multiplier for high-value assets (250% of normal)
         maxAcceptableSpread = atrPoints * 0.25 * BTC_SpreadMultiplier;
         Print("[SPREAD] Using more permissive spread threshold for ", symbolName, 
               ": ", DoubleToString(maxAcceptableSpread, 1), " points (vs standard ", 
               DoubleToString(atrPoints * 0.25, 1), " points)");
      }
   }
   
   // Ensure we have a minimum threshold regardless of ATR
   maxAcceptableSpread = MathMax(maxAcceptableSpread, 10.0);
   
   // Compare current spread with our acceptable threshold
   bool result = (currentSpread <= maxAcceptableSpread);
   
   if(!result) {
      Print("[SPREAD] Current spread ", currentSpread, " exceeds maximum acceptable ", 
            maxAcceptableSpread, " - skipping trade");
   }
...
input group "Advanced Trailing Stop Settings"
input bool EnableAdvancedTrailing = true;     // Enable advanced trailing stops
input double TrailingActivationPct = 0.5;     // Activate trailing at this % of risk (0.5 = 50%)
input double TrailingStopMultiplier = 1.2;    // Trailing stop multiplier of ATR
input bool UseDynamicTakeProfit = true;       // Dynamically extend take profit in trending markets
input double TakeProfitExtensionMultiplier = 0.5; // Extend TP by this multiplier of current distance
...
int MaxConsecutiveLosses = 3; // Auto-pause after this many consecutive losses
...
int OnInit()
{
   ...
   if(ShowDashboard) {
      // Create unique filename for dashboard position
      DashboardFile = "SMC_Dashboard_" + Symbol() + "_" + IntegerToString(MagicNumber) + ".dat";
      
      // Set initial position
      if(SaveDashPosition) {
         // Try to load saved position
         if(!LoadDashboardPosition()) {
            // If no saved position, use defaults
            CurrentDashX = DashX;
            CurrentDashY = DashY;
...
double CalculateOptimalStopLoss(int signal, double entryPrice)
{
   ...
   int atrHandleH4 = iATR(Symbol(), PERIOD_H4, 14);
   
   if(atrHandleCurrent == INVALID_HANDLE || atrHandleH1 == INVALID_HANDLE || atrHandleH4 == INVALID_HANDLE) {
      Print("[ERROR] Failed to create ATR indicator handles");
      return 0.0;
   }
   ...
   if(CopyBuffer(atrHandleCurrent, 0, 0, 3, atrCurrent) <= 0 ||
      CopyBuffer(atrHandleH1, 0, 0, 3, atrH1) <= 0 ||
      CopyBuffer(atrHandleH4, 0, 0, 3, atrH4) <= 0) {
      Print("[ERROR] Failed to copy ATR data");
      return 0.0;
   }
   
   // Calculate weighted ATR value (more weight to higher timeframes for stability)
   double weightedATR = (atrCurrent[0] * 0.2) + (atrH1[0] * 0.4) + (atrH4[0] * 0.4);
   
   // IMPROVED: Wider stops for all assets to prevent premature stop-outs
   double atrValue = weightedATR * VolatilityMultiplier * PatternQualityMultiplier;
   
   // NEW: Stop loss width adaptations based on time of day and market conditions
   MqlDateTime timeStruct;
   TimeToStruct(TimeCurrent(), timeStruct);
   double volatilityMultiplier = 1.0;
   
   // Widen stops during high-volatility hours (Asian session close, European/US opens)
   if((timeStruct.hour >= 1 && timeStruct.hour <= 3) || // Asian session volatility
      (timeStruct.hour >= 8 && timeStruct.hour <= 10) || // European open volatility
      (timeStruct.hour >= 14 && timeStruct.hour <= 16)) { // US open volatility
      volatilityMultiplier = 1.5; // 50% wider stops during volatile session transitions
      Print("[SL] Using wider stops during volatile session transition: ", 
            TimeToString(TimeCurrent(), TIME_MINUTES), ", ATR multiplier: 1.5");
   }
   
   // Special handling for crypto and high-value assets
   bool isHighValueAsset = false;
   string symbolName = Symbol();
   
   // Check if this is a high-value asset that needs special handling
   if(EnableSpecialHandlingForBTC && 
      (StringFind(symbolName, "BTC") >= 0 || 
       StringFind(symbolName, "XBT") >= 0 || 
       StringFind(symbolName, "XAU") >= 0)) {
      isHighValueAsset = true;
      volatilityMultiplier *= 1.8; // 80% wider stops for high-value assets
      Print("[SL] Using extra-wide stops for high-value asset: ", symbolName, ", ATR multiplier: 1.8");
   }
   
   // Apply the volatility multiplier to the ATR value
   atrValue *= volatilityMultiplier;
   
   // Calculate stop distance based on the optimized ATR value
   double stopDistance = atrValue * (isHighValueAsset ? BTC_SpreadMultiplier : 1.0);
   ...
   Print("[SL] Calculated stop loss: ", stopLoss, " (ATR: ", atrValue, ", distance: ", stopDistance, ")");
   return stopLoss;
}
...
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| Check if an order block is still valid based on its age        |
//+------------------------------------------------------------------+
bool IsOrderBlockValid(datetime blockTime, bool isHighValueAsset = false)
{
   // Current time for age calculation
   datetime currentTime = TimeCurrent();
   
   // Calculate block age in seconds
   int blockAgeSeconds = (int)(currentTime - blockTime);
   
   // Convert to hours for easier comparison
   double blockAgeHours = blockAgeSeconds / 3600.0;
   
   // Default max age (3 hours for standard assets)
   double maxAgeHours = 3.0;
   
   // Special handling for high-value assets like BTC/XAU
   if(isHighValueAsset && EnableSpecialHandlingForBTC) {
      // Use extended block age validity from input parameters
      maxAgeHours = BTC_BlockAgeHours; // 8 hours by default
      Print("[BLOCK-AGE] Using extended validity period for high-value asset: ", 
            DoubleToString(maxAgeHours, 1), " hours");
   }
   
   // Additional 50% age extension during low volatility periods
   if(CurrentRegime == REGIME_RANGING || CurrentRegime == REGIME_CHOPPY) {
      maxAgeHours *= 1.5;
      Print("[BLOCK-AGE] Extended block validity by 50% due to low volatility market regime");
   }
   
   // Check if block is still valid based on age
   bool isValid = (blockAgeHours <= maxAgeHours);
   
   if(!isValid) {
      Print("[BLOCK-AGE] Order block from ", TimeToString(blockTime), " invalidated due to age: ", 
            DoubleToString(blockAgeHours, 1), " hours old (max allowed: ", 
            DoubleToString(maxAgeHours, 1), " hours)");
   }
...
void DetectOrderBlocks()
{
   ...
   // Apply age validation to blocks
   for(int i=0; i<MAX_BLOCKS; i++) {
      if(!recentBlocks[i].valid) continue;
      
      // Check if this is a high-value asset
      string symbolName = Symbol();
      bool isHighValueAsset = (EnableSpecialHandlingForBTC && 
                             (StringFind(symbolName, "BTC") >= 0 || 
                              StringFind(symbolName, "XBT") >= 0 || 
                              StringFind(symbolName, "XAU") >= 0));
      
      // Special handling for high-value assets - reduced minimum strength
      if(isHighValueAsset && recentBlocks[i].strength < BTC_MinBlockStrength) {
         recentBlocks[i].strength = (int)BTC_MinBlockStrength;
         Print("[BLOCK] Boosted block strength for high-value asset to minimum threshold");
      }
      
      // Validate block age with proper handling for high-value assets
      if(!IsOrderBlockValid(recentBlocks[i].time, isHighValueAsset)) {
         recentBlocks[i].valid = false;
         Print("[BLOCK] Invalidated block due to age");
      }
   }
...
void OnTick()
{
   ...
   if(ConsecutiveLosses >= MaxConsecutiveLosses) {
      canTradeNow = false;
      Print("[PROTECTION] Trading paused due to ", ConsecutiveLosses, 
            " consecutive losses (limit: ", MaxConsecutiveLosses, ")");
   }
...
void ManageOpenTrade()
{
   ...
   MqlRates rates[];
   ArraySetAsSeries(rates, true);
   int copied = CopyRates(Symbol(), PERIOD_CURRENT, 0, 10, rates); // Increased lookback for swing point detection
   ...
   for(int i=0; i<PositionsTotal(); i++) {
      ...
      if(EnableAdvancedTrailing && !partialTaken2) { // Allow trailing until second partial
         // Calculate how far we've moved from entry as percentage of initial risk
         double profitRiskRatio = profitDistance / riskDistance;
         
         // Only start trailing after we've reached activation threshold
         if(profitRiskRatio >= TrailingActivationPct) {
            // Calculate trailing amount based on ATR and the multiplier
            double trailAmount = atrValue * TrailingStopMultiplier;
            
            // Adjust trailing based on market regime
            if(EnableRegimeFilters && CurrentRegime != REGIME_RANGING) {
               switch(CurrentRegime) {
                  case REGIME_TRENDING_BULL:
                  case REGIME_TRENDING_BEAR:
                     // More aggressive trailing in trends (allow more room)
                     trailAmount *= 1.5; 
                     Print("[TRAIL] Increased trailing distance in trending market");
                     break;
                     
                  case REGIME_VOLATILE:
                  case REGIME_CHOPPY:
                     // Tighter trailing in volatile/choppy conditions
                     trailAmount *= 0.8; 
                     Print("[TRAIL] Reduced trailing distance in volatile/choppy market");
                     break;
               }
            }
            
            // Minimum trailing amount to avoid noise
            ...
            if(posType == 1) { // Buy position
               // Basic trailing stop: current price minus trail amount
               double potentialSL = currentBid - trailAmount;
               ...
               if(swingBar >= 0) {
                  // Adjust swing point with a small buffer
                  swingSL = rates[swingBar].low - (3 * _Point);
                  
                  // Use swing low only if it's higher than current SL and below current price
                  if(swingSL > posStopLoss && swingSL < currentBid - (5 * _Point)) {
                     potentialSL = MathMax(potentialSL, swingSL);
                     Print("[TRAIL] Using swing low at bar ", swingBar, " price ", swingSL, " for trailing stop");
                  }
               }
               
               // Only update if new SL is higher than current SL
               if(potentialSL > posStopLoss + (2 * _Point)) {
                  newSL = potentialSL;
               }
               
               // Dynamic take profit extension in trending markets
               if(UseDynamicTakeProfit && profitRiskRatio >= 0.8 && 
                  CurrentRegime == REGIME_TRENDING_BULL && posTakeProfit > 0) {
                  double tpDistance = posTakeProfit - posOpenPrice;
                  double newTP = posTakeProfit + (tpDistance * TakeProfitExtensionMultiplier);
                  
                  // Only update if the extension is significant
                  if(newTP > posTakeProfit + (10 * _Point)) {
                     // Normalize TP
                     int digits = (int)SymbolInfoInteger(Symbol(), SYMBOL_DIGITS);
                     newTP = NormalizeDouble(newTP, digits);
                     
                     // Update position with new TP and SL
                     if(trade.PositionModify(posTicket, newSL > 0 ? newSL : posStopLoss, newTP)) {
                        Print("[TRAIL] Extended take profit for ticket ", posTicket, 
                              " to ", newTP, " in trending bullish market");
                        continue; // Skip regular SL update as we've modified both
                     }
                  }
               }
            } else { // Sell position
               // Basic trailing stop: current price plus trail amount
               double potentialSL = currentAsk + trailAmount;
               ...
               if(swingBar >= 0) {
                  // Adjust swing point with a small buffer
                  swingSL = rates[swingBar].high + (3 * _Point);
                  
                  // Use swing high only if it's lower than current SL and above current price
                  if((posStopLoss == 0 || swingSL < posStopLoss) && swingSL > currentAsk + (5 * _Point)) {
                     potentialSL = MathMin(potentialSL, swingSL);
                     Print("[TRAIL] Using swing high at bar ", swingBar, " price ", swingSL, " for trailing stop");
                  }
               }
               
               // Only update if new SL is lower than current SL
               if(posStopLoss == 0 || potentialSL < posStopLoss - (2 * _Point)) {
                  newSL = potentialSL;
               }
               
               // Dynamic take profit extension in trending markets
               if(UseDynamicTakeProfit && profitRiskRatio >= 0.8 && 
                  CurrentRegime == REGIME_TRENDING_BEAR && posTakeProfit > 0) {
                  double tpDistance = posOpenPrice - posTakeProfit;
                  double newTP = posTakeProfit - (tpDistance * TakeProfitExtensionMultiplier);
                  ...
                  if(newTP < posTakeProfit - (10 * _Point)) {
                     ...
                     if(trade.PositionModify(posTicket, newSL > 0 ? newSL : posStopLoss, newTP)) {
                        Print("[TRAIL] Extended take profit for ticket ", posTicket, 
                              " to ", newTP, " in trending bearish market");
                        continue; // Skip regular SL update as we've modified both
                     }
                  }
               }
            ...
            if(newSL != 0 && MathAbs(newSL - posStopLoss) > (3 * _Point)) {
               // Normalize SL
               int digits = (int)SymbolInfoInteger(Symbol(), SYMBOL_DIGITS);
               newSL = NormalizeDouble(newSL, digits);
               
               if(trade.PositionModify(posTicket, newSL, posTakeProfit)) {
                  Print("[TRAIL] Updated trailing stop for ticket ", posTicket, 
                        " to ", newSL, " (profit ratio: ", NormalizeDouble(profitRiskRatio, 2), ")");
               }
            }
         }
      }
...
//+------------------------------------------------------------------+
//| Find recent swing high/low for better stop placement              |
//+------------------------------------------------------------------+
int FindRecentSwingPoint(bool isBuy, int startBar, int lookbackBars)
{
   // For BUY positions, we look for swing LOWS as potential support levels
   // For SELL positions, we look for swing HIGHS as potential resistance levels
   
   MqlRates rates[];
   ArraySetAsSeries(rates, true);
   int copied = CopyRates(Symbol(), PERIOD_CURRENT, 0, lookbackBars + 2, rates);
   
   if(copied <= 0) {
      Print("[SWING] Failed to copy rates data for swing point detection: ", GetLastError());
      return -1;
   }
   ...
   if(isBuy) { // For BUY positions, find swing LOW
      // Look for local minima (low is lower than previous and next bar)
      for(int i = startBar + 1; i < lookbackBars - 1; i++) {
         if(rates[i].low < rates[i-1].low && rates[i].low < rates[i+1].low) {
            // If we haven't found a swing point yet, or this is lower than the current one
            if(swingPointBar == -1 || rates[i].low < swingValue) {
               swingPointBar = i;
               swingValue = rates[i].low;
               
               // Extra check for a stronger swing point (lower than 2 bars on each side)
               bool isStrongSwing = true;
               if(i > 1 && i < lookbackBars - 2) {
                  isStrongSwing = rates[i].low < rates[i-2].low && rates[i].low < rates[i+2].low;
               }
               
               // Prefer strong swing points
               if(isStrongSwing) {
                  break; // Use this strong swing point
               }
            }
         }
      }
   } else { // For SELL positions, find swing HIGH
      // Look for local maxima (high is higher than previous and next bar)
      for(int i = startBar + 1; i < lookbackBars - 1; i++) {
         if(rates[i].high > rates[i-1].high && rates[i].high > rates[i+1].high) {
            // If we haven't found a swing point yet, or this is higher than the current one
            if(swingPointBar == -1 || rates[i].high > swingValue) {
               swingPointBar = i;
               swingValue = rates[i].high;
               
               // Extra check for a stronger swing point (higher than 2 bars on each side)
               bool isStrongSwing = true;
               if(i > 1 && i < lookbackBars - 2) {
                  isStrongSwing = rates[i].high > rates[i-2].high && rates[i].high > rates[i+2].high;
               }
   ...
   if(swingPointBar >= 0) {
      Print("[SWING] Found swing point at bar ", swingPointBar, " value: ", 
            isBuy ? rates[swingPointBar].low : rates[swingPointBar].high);
   }
...
void OnTradeTransaction(const MqlTradeTransaction &trans, const MqlTradeRequest &request, const MqlTradeResult &result)
{
   ...
   if(HistoryDealSelect(dealTicket)) {
      // Make sure it's our EA's deal by checking magic number
      ...
      if(magic == MagicNumber && symbol == Symbol()) {
         ...
         if(dealEntry == DEAL_ENTRY_OUT) { // Position closed
            ...
            // Update performance stats for this EA
            UpdatePerformanceStats(profit, isWin, 5); // Standard quality for now
            ...
            Print("[TRADE] Position closed with P/L: ", profit, 
                  ", Deal Type: ", EnumToString((ENUM_DEAL_TYPE)dealType),
...
double CalculateCorrelation(string symbol1, string symbol2, int period = 100)
{
   ...
   for(int i=0; i<period; i++) {
      sum_x += prices1[i];
      sum_y += prices2[i];
      sum_xy += prices1[i] * prices2[i];
      sum_x2 += prices1[i] * prices1[i];
      sum_y2 += prices2[i] * prices2[i];
   }
   ...
   if(denominator != 0) {
      correlation = (period * sum_xy - sum_x * sum_y) / denominator;
   }
   
   return correlation;
}

// This duplicate function has been removed - using the implementation from earlier in the code
