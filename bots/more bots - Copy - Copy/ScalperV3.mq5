...
input int minSecondsBetweenTrades = 2;   // Minimum seconds between trades (avoid overtrading)
input double maxSpreadFactor = 0.4;      // Maximum spread as a factor of ATR
...
input double slMultiplier = 0.3;      // SL multiplier for ATR (reduced for tight scalping)
input double tpMultiplier = 0.6;      // TP multiplier for ATR (reduced for quick profits)
input double riskRewardRatio = 1.5;  // Target risk-reward ratio (TP/SL)
input double minStopLoss = 5.0;      // Minimum stop loss in points
input double maxStopLoss = 15.0;     // Maximum stop loss in points for scalping
input double minTakeProfit = 8.0;    // Minimum take profit in points
input double maxTakeProfit = 25.0;   // Maximum take profit in points for scalping
...
double dynamicBuyThresh = 0.55;  // Lowered from 0.65 to make buying easier
double dynamicSellThresh = 0.45;  // Raised from 0.35 to make selling easier
...
bool IsSafeToTrade() {
    ...
    // SAFETY: Check for drawdown limit
    ...
    // SAFETY: Check for daily loss limit
    if(dailyTradeCount > 0 && dailyProfit < 0 && MathAbs(dailyProfit)/balance*100 > maxDailyLossPct) {
        Print("[SAFETY] Maximum daily loss reached: ", MathAbs(dailyProfit)/balance*100, "%. Trading paused.");
        tradingStopped = true;
        return false;
    }
    
    // SAFETY: Check trading frequency
    if(TimeCurrent() - lastTradeTime < minSecondsBetweenTrades) {
        // Not an error, just skipping this tick
        return false;
    }
    
    // SAFETY: Calculate and monitor spread
    ...
    // SAFETY: Check if spread is too high
    if(enableSpreadProtection && avgSpread > 0) {
        double maxSpread = avgSpread * maxSpreadFactor;
    ...
    // SAFETY: Check for high volatility
    double currentATR = GetATR(_Symbol, PERIOD_M5, 14, 0);
    double dayATR = GetATR(_Symbol, PERIOD_D1, 14, 0);
    if(currentATR > dayATR * 2) {
        Print("[SAFETY] Current volatility too high (", currentATR, " > ", dayATR * 2, "). Skipping trade.");
        return false;
    }
...
double ValidateStopLevel(double stopLevel, bool isBuy) {
    ...
    if(distance < minStopDistance) {
        Print("[WARNING] Stop level too close to current price (", distance, " < ", minStopDistance, "). Adjusting.");
        return isBuy ? currentPrice - minStopDistance * 1.1 : currentPrice + minStopDistance * 1.1;
    }
...
void OnTick() {
    ...
    Print("[SL/TP] SL: ", slPoints, " points, TP: ", tpPoints, " points");
    ...
    if(canOpenNewPosition) {
        ...
        switch(currentRegime) {
            case TRENDING_UP:
                ...
                buyThreshold = 0.50;  // Lowered from 0.55
            ...
            case TRENDING_DOWN:
                ...
                sellThreshold = 0.50;  // Raised from 0.45
                rsiUpperLimit = 70;
                rsiLowerLimit = 20;
                break;
            ...
            case RANGING_WIDE:
                // In wide range, be more selective
            ...
            case BREAKOUT:
                ...
                buyThreshold = 0.50;  // Lowered from 0.52
                sellThreshold = 0.50;  // Raised from 0.48
                rsiUpperLimit = 85;
                rsiLowerLimit = 15;
                break;
            ...
            case CHOPPY:
                ...
                buyThreshold = 0.65;  // Lowered from 0.70
                sellThreshold = 0.35;  // Raised from 0.30
                rsiUpperLimit = 70;
                rsiLowerLimit = 30;
                break;
        }
        ...
        bool strongBuySignal = prediction > buyThreshold && rsi < rsiUpperLimit &&
                              (momentum > -0.0001 || pattern == PATTERN_BULL || imbalance > 0.5) && // Only one confirmation needed
                              !(rsi > 85 && currentRegime == TRENDING_DOWN); // Only avoid extreme overbought in downtrend
        
        bool strongSellSignal = prediction < sellThreshold && rsi > rsiLowerLimit &&
                                (momentum < 0.0001 || pattern == PATTERN_BEAR || imbalance < 0.5) && // Only one confirmation needed
                                !(rsi < 15 && currentRegime == TRENDING_UP); // Only avoid extreme oversold in uptrend
            ...
            for(int attempt=1; attempt<=3; attempt++) { // Allow up to 3 attempts
                ...
                if(result) {
                    ...
                    if(errorCode == ERR_INVALID_STOPS || errorCode == ERR_INVALID_TRADE_VOLUME) {
                        ...
                        double minStopDist = SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL) * _Point;
                        if(errorCode == ERR_INVALID_STOPS && minStopDist > 0) {
                            sellSLLevel = currentAsk + (minStopDist * 1.2); // Add 20% margin
                            sellTPLevel = currentAsk - (minStopDist * 1.2); // Add 20% margin
                        }
                        else if(errorCode == ERR_INVALID_TRADE_VOLUME || errorCode == 4756) {
                            // Get the exact symbol volume constraints
                            double minLot = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
                    ...
                    // Other errors - log and continue
                    Print("[ERROR] Failed to place SELL order. Error ", errorCode, ": ", ErrorDescription(errorCode));
                }
            }
    ...
    if(trainSampleCount % 50 == 0 && trainSampleCount > 0 && GetPredictionAccuracy() < 0.5) {
        // RetrainNeuralNet(); // Uncomment if implemented
...
void ManageOpenPositionsAggressivelyImpl() {
    ...
    for(int i = 0; i < posCount; i++) {
        if(PositionSelectByTicket(tickets[i])) {
            ...
            bool firstTPHit = (StringFind(posComment, "TP1Hit") >= 0);
            bool secondTPHit = (StringFind(posComment, "TP2Hit") >= 0);
            
            // Calculate TP levels based on original TP
            double firstTPLevel = originalTP * firstTakeProfitPct;
            double secondTPLevel = originalTP * (firstTakeProfitPct + secondTakeProfitPct);
            double finalTPLevel = originalTP;
            
            // Check if we need to take partial profits
            if(!firstTPHit && currentProfit >= firstTPLevel) {
                // First take profit hit - close part of position and update SL
                double volumeToClose = NormalizeDouble(posVolume * firstTakeProfitPct, 2);
                
                // Ensure minimum volume
                double minVolume = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
                if(volumeToClose >= minVolume && (posVolume - volumeToClose) >= minVolume) {
                    // Close partial position
                    if(trade.PositionClosePartial(posTicket, volumeToClose)) {
                        Print("[Partial TP1] Closed ", firstTakeProfitPct*100, "% of position ", posTicket, 
                              " profit: ", currentProfit, " points");
                        
                        // Move stop loss to break-even plus small buffer
                        double newSL = 0.0;
                        if(posType == POSITION_TYPE_BUY) {
                            newSL = posOpenPrice + breakEvenLevel * pointSize;
                        } else {
                            newSL = posOpenPrice - breakEvenLevel * pointSize;
                        }
                        
                        // Update position with new SL and comment
                        string newComment = posComment;
                        if(StringFind(newComment, "OrigTP=") < 0) {
                            newComment = StringFormat("OrigTP=%.1f;TP1Hit", originalTP);
                        } else {
                            newComment = StringFormat("%s;TP1Hit", newComment);
                        }
                        
                        // Modify position with new SL/TP and store comment in a global variable or array
                        // since MQL5 doesn't allow modifying position comments directly
                        trade.PositionModify(posTicket, newSL, posTakeProfit);
                        Print("[TP1] New comment for position ", posTicket, ": ", newComment);
                    }
                }
            }
            // Check for second take profit level
            else if(firstTPHit && !secondTPHit && currentProfit >= secondTPLevel) {
                // Second take profit hit - close another part and tighten SL
                double remainingVolume = PositionGetDouble(POSITION_VOLUME);
                double volumeToClose = NormalizeDouble(remainingVolume * secondTakeProfitPct / 
                                                     (secondTakeProfitPct + finalTakeProfitPct), 2);
                
                // Ensure minimum volume
                double minVolume = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
                if(volumeToClose >= minVolume && (remainingVolume - volumeToClose) >= minVolume) {
                    // Close partial position
                    if(trade.PositionClosePartial(posTicket, volumeToClose)) {
                        Print("[Partial TP2] Closed ", secondTakeProfitPct*100, "% of position ", posTicket, 
                              " profit: ", currentProfit, " points");
                        
                        // Move stop loss to first TP level
                        double newSL = 0.0;
                        if(posType == POSITION_TYPE_BUY) {
                            newSL = posOpenPrice + firstTPLevel * pointSize * 0.8; // 80% of first TP level
                        } else {
                            newSL = posOpenPrice - firstTPLevel * pointSize * 0.8;
                        }
                        
                        // Calculate new take profit for remaining position
                        double newTP = 0.0;
                        if(posType == POSITION_TYPE_BUY) {
                            newTP = posOpenPrice + finalTPLevel * pointSize * 1.2; // Extend final TP by 20%
                        } else {
                            newTP = posOpenPrice - finalTPLevel * pointSize * 1.2;
                        }
                        
                        // Update position with new SL, TP and comment
                        string newComment = posComment;
                        if(StringFind(newComment, "TP2Hit") < 0) {
                            newComment = StringFormat("%s;TP2Hit", newComment);
                        }
                        
                        // Modify position with new SL/TP and store comment in a global variable or array
                        // since MQL5 doesn't allow modifying position comments directly
                        trade.PositionModify(posTicket, newSL, newTP);
                        Print("[TP2] New comment for position ", posTicket, ": ", newComment);
                    }
                }
            }
            // Apply trailing stop if activated
            else if(currentProfit >= originalTP * trailingStopActivationPct) {
                ...
                if(posType == POSITION_TYPE_BUY) {
                    ...
                    if(newSL > posStopLoss + pointSize) {
                        trade.PositionModify(posTicket, newSL, posTakeProfit);
                        Print("[Trailing] Updated SL for position ", posTicket, " to ", newSL);
                    }
                } else { // POSITION_TYPE_SELL
            ...
            // Move to break-even if profit exceeds threshold
            else if(currentProfit >= breakEvenLevel && 
                ...
                trade.PositionModify(posTicket, newSL, posTakeProfit);
                Print("[Break-Even] Updated SL for position ", posTicket, " to ", newSL);
            }
        }
    }
}
...
// Stub for InitializeMetricArrays
void InitializeMetricArrays() {
    ArrayResize(tradeProfits, METRIC_WINDOW);
    for(int i=0; i<METRIC_WINDOW; i++) tradeProfits[i] = 0.0;
...
// Stub for CheckButtonClicks
void CheckButtonClicks() {
    // Implementation would go here
}

// End of ScalperV3 implementation