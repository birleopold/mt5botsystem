...
// Usage: double atr = GetATR(Symbol(), PERIOD_CURRENT, 14, 0);
...
ENUM_MARKET_REGIME DetectMarketRegime()
{
   ...
   double atr[] = {0};
   // Use the global /* atrHandle removed: use GetATR utility instead */ instead of creating a local one
   if(CopyBuffer(::/* atrHandle removed: use GetATR utility instead */, 0, 0, lookback, atr) <= 0) {
...
int OnInit()
{
   ...
   /* atrHandle removed: use GetATR utility instead */ = localAtrHandle;
...
void OnDeinit(const int reason)
{
   ...
   if(/* atrHandle removed: use GetATR utility instead */ != INVALID_HANDLE) IndicatorRelease(/* atrHandle removed: use GetATR utility instead */);
}
...
void AnalyzePairBehavior(string symbol) {
   ...
   if(atrRatio > 1.5 || movementRatio > 0.7 || reversalRatio < 0.3 || avgSpread > GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 0.25) {
...
void CalculateVolumeProfile()
{
   ...
   // Use a local name to avoid hiding the global GetATR(Symbol(), PERIOD_CURRENT, 14, 0)
   ...
   if(atrCopied > 0) {
      ...
      // Also update the global GetATR(Symbol(), PERIOD_CURRENT, 14, 0) for use elsewhere
      ::GetATR(Symbol(), PERIOD_CURRENT, 14, 0) = localAtrValue;
...
double DetermineOptimalStopLoss(int signal, double entryPrice)
{
   ...
   // Also update the global GetATR(Symbol(), PERIOD_CURRENT, 14, 0) for consistency
   ::GetATR(Symbol(), PERIOD_CURRENT, 14, 0) = (atrCurrent[0] + atrH1[0] * 2 + atrH4[0] * 3) / 6;
   ...
   // We already calculated GetATR(Symbol(), PERIOD_CURRENT, 14, 0) from multiple timeframes above
   
   // Calculate ATR-based stop (standard approach)
   double stopLoss = 0.0;
   if(signal > 0) {
      stopLoss = entryPrice - (GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * atrMultiplier * slMultiplier);
   } else {
      stopLoss = entryPrice + (GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * atrMultiplier * slMultiplier);
   }
   ...
   Print("[SL] ATR Current: ", DoubleToString(atrCurrent[0], 5),
         ...
         " Weighted: ", DoubleToString(GetATR(Symbol(), PERIOD_CURRENT, 14, 0), 5));
   ...
   for(int i=0; i<MAX_CHOCHS; i++) {
      ...
      if(signal > 0 && !recentCHOCHs[i].isBullish) {
         chochStop = recentCHOCHs[i].price - (GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 0.5 * slMultiplier); // Add buffer below CHOCH
         foundChochStop = true;
      ...
      // For SELL positions, use bullish CHOCH as potential stop
         ...
         chochStop = recentCHOCHs[i].price + (GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 0.5 * slMultiplier); // Add buffer above CHOCH
         foundChochStop = true;
   ...
   if(signal > 0) { // BUY position, look for recent swing low
      ...
      if(isSwingLow) {
         swingStop = lowestLow - (GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 0.3 * slMultiplier); // Add buffer below swing
         foundSwingStop = true;
      ...
      if(isSwingHigh) {
         swingStop = highestHigh + (GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 0.3 * slMultiplier); // Add buffer above swing
         foundSwingStop = true;
   ...
   double minStopDistance = 3 * GetATR(Symbol(), PERIOD_CURRENT, 14, 0);
...
double CalculatePositionSize(double entryPrice, double stopLoss, double riskPercent=1.0, int setupQuality=5)
{
   ...
   if(RiskManagementEnabled) {
      ...
      CopyBuffer(/* atrHandle removed: use GetATR utility instead */, 0, 0, 20, atrHistory);
   ...
   for(int i=0; i<MAX_BLOCKS; i++) {
      if(recentBlocks[i].valid) {
         if((signal > 0 && recentBlocks[i].isBuy) || 
            ...
            double threshold = GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 0.5; // Within half ATR of block
                     ...
                     double posSize, int setupQuality, bool executed)
{
   ...
   logEntry += "\nATR Value: " + DoubleToString(GetATR(Symbol(), PERIOD_CURRENT, 14, 0), _Digits);
...
void GetLatestPriceData()
{
   ...
   int /* atrHandle removed: use GetATR utility instead */Local = iATR(Symbol(), PERIOD_CURRENT, 14);
   ArraySetAsSeries(atrBuffer, true);
   bool atrValid = CopyBuffer(/* atrHandle removed: use GetATR utility instead */Local, 0, 0, 1, atrBuffer) > 0;
...
void ManageATRTrailingStops()
{
    ...
    double GetATR(Symbol(), PERIOD_CURRENT, 14, 0) = 0.0;
    double atrBuffer[];
    ArraySetAsSeries(atrBuffer, true);
    ...
    if(/* atrHandle removed: use GetATR utility instead */ != INVALID_HANDLE) {
        ...
        if(ArraySize(atrBuffer) > 0) {
            GetATR(Symbol(), PERIOD_CURRENT, 14, 0) = atrBuffer[0];
        }
        IndicatorRelease(/* atrHandle removed: use GetATR utility instead */);
    }
    
    if(GetATR(Symbol(), PERIOD_CURRENT, 14, 0) <= 0) {
    ...
    for(int i = 0; i < PositionsTotal(); i++) {
        ...
        if(posStopLoss > 0) {
            ...
            riskPips = GetATR(Symbol(), PERIOD_CURRENT, 14, 0) / pipSize;
        }
        ...
        if(profitPips >= activationThreshold) {
            ...
            double trailingDistance = GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * TrailingStopATRMultiplier;
                    ...
                    if(trade.PositionModify(ticket, newStopLoss, posTakeProfit)) {
                        if(EnableTrailingDebug) {
                            Print("[TRAILING] Updated BUY stop loss from ", posStopLoss, " to ", newStopLoss, 
                                  " (profit: ", profitPips, " pips, ATR: ", GetATR(Symbol(), PERIOD_CURRENT, 14, 0) / pipSize, " pips)");
                        }
                    } else {
...
void UpdateAdaptivePairSettings()
{
    ...
    for(int i=0; i<g_pairCount; i++) {
        ...
        int /* atrHandle removed: use GetATR utility instead */ = iATR(symbol, PERIOD_H1, 14);
        if(/* atrHandle removed: use GetATR utility instead */ != INVALID_HANDLE) {
            double GetATR(Symbol(), PERIOD_CURRENT, 14, 0)s[];
            CopyBuffer(/* atrHandle removed: use GetATR utility instead */, 0, 0, 1, GetATR(Symbol(), PERIOD_CURRENT, 14, 0)s);
            if(ArraySize(GetATR(Symbol(), PERIOD_CURRENT, 14, 0)s) > 0) {
                double atr = GetATR(Symbol(), PERIOD_CURRENT, 14, 0)s[0];
...
double GetATR(string symbol, ENUM_TIMEFRAMES timeframe, int period, int shift=0) {
   ...
   if(CopyBuffer(/* atrHandle removed: use GetATR utility instead */, 0, shift, 1, atrBuffer) <= 0) {
      Print("Error copying ATR values: ", GetLastError());
      IndicatorRelease(/* atrHandle removed: use GetATR utility instead */); // Clean up the handle
      return 0.0;
   }
...
bool RetryTrade(int signal, double price, double sl, double tp, double size, int maxRetries=3)
{
   ...
   double GetATR(Symbol(), PERIOD_CURRENT, 14, 0) = 0;
   double atrBuffer[];
   int /* atrHandle removed: use GetATR utility instead */ = iATR(symbol, 0, 14);
   if(/* atrHandle removed: use GetATR utility instead */ != INVALID_HANDLE) {
   ...
   // If we couldn't get ATR for some reason, use a reasonable default
   if(GetATR(Symbol(), PERIOD_CURRENT, 14, 0) <= 0) {
       GetATR(Symbol(), PERIOD_CURRENT, 14, 0) = 0.0010; // Default to roughly 10 pips for major pairs
   }
   ...
   // For wider spreads, increase the breathing room
   if(spread > GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 0.3) { // If spread is more than 30% of ATR
   ...
   double newSL, newTP;
   if(signal > 0) { // Buy
      newSL = adjustedPrice - ((GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * slMultiplier) * spreadMultiplier);
      newTP = adjustedPrice + ((GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * tpMultiplier) * spreadMultiplier);
   } else { // Sell
      newSL = adjustedPrice + ((GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * slMultiplier) * spreadMultiplier);
      newTP = adjustedPrice - ((GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * tpMultiplier) * spreadMultiplier);
   }
   ...
   Print("[REAL-TRADE] Using ATR-based SL/TP: ATR=", DoubleToString(GetATR(Symbol(), PERIOD_CURRENT, 14, 0)/_Point, 1), 
...
void ModifyStopsOnCHOCH(string symbol = NULL)
{
   ...
   for(int i=PositionsTotal()-1; i>=0; i--) {
      ...
      double bufferSize = GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * bufferMultiplier;
      ...
      if(posType == POSITION_TYPE_BUY && !recentCHOCHs[latestCHOCHIndex].isBullish) {
         ...
         if(newSL > currentSL && MathAbs(newSL - currentSL) > GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 0.2) {
      ...
      // For short positions, use bullish CHOCH to adjust stops
         ...
         if((newSL < currentSL || currentSL == 0) && (currentSL == 0 || MathAbs(newSL - currentSL) > GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 0.2)) {
...
void DetectSupplyDemandZones()
{
   ...
   for(int i=3; i<copied-3; i++) {
      ...
      if(!inDemandZone && 
         rates[i].close > rates[i].open && 
         rates[i].close > rates[i+1].high && 
         rates[i].close - rates[i].open > GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 0.3 * blockSizeMultiplier && 
         ...
         for(int l=0; l<levelCount; l++) {
            if(MathAbs(demandLower - potentialLevels[l]) < GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 0.5) {
               strength += 2;
               break;
            }
         }
      ...
      if(!inSupplyZone && 
         ...
         rates[i].open - rates[i].close > GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 0.3 * blockSizeMultiplier && 
         ...
         // Increase strength if near a potential level
         for(int l=0; l<levelCount; l++) {
            if(MathAbs(supplyUpper - potentialLevels[l]) < GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 0.5) {
               strength += 2;
               break;
            }
         }
...
void DetectFairValueGaps()
{
   ...
   for(int i=2; i<copied-1; i++) {
      ...
      if(rates[i].low > rates[i+1].high) {
         ...
         if(gapSize >= GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 0.5) {
            // Find a free slot
            int fvgIndex = -1;
...
double GetATR(string symbol, ENUM_TIMEFRAMES timeframe, int period, int shift = 0, int averagePeriods = 1) {
    ...
    double GetATR(Symbol(), PERIOD_CURRENT, 14, 0)s[];
    ArraySetAsSeries(GetATR(Symbol(), PERIOD_CURRENT, 14, 0)s, true);
    
    // If averagePeriods > 1, we'll calculate an average of ATR values
    int copyCount = shift + averagePeriods;
    int copied = CopyBuffer(/* atrHandle removed: use GetATR utility instead */, 0, shift, copyCount, GetATR(Symbol(), PERIOD_CURRENT, 14, 0)s);
    ...
    if(averagePeriods > 1) {
        ...
        for(int i=0; i<averagePeriods; i++) {
            sum += GetATR(Symbol(), PERIOD_CURRENT, 14, 0)s[i];
        }
        return sum / averagePeriods;
    } else {
        // Return single value
        return GetATR(Symbol(), PERIOD_CURRENT, 14, 0)s[0];
    }
}
         ...
         for(int j=1; j<highCount; j++) {
            ...
            if(swingHighTimes[j] < swingLowTimes[i] && j+1 < highCount) {
               ...
               if(swingHighTimes[j+1] > swingLowTimes[i] && swingHighs[j+1] > swingHighs[j]) {
                  ...
                  if(breakStrength > GetATR(Symbol(), PERIOD_CURRENT, 14, 0)) strength += 2;
                  ...
                  breakerBlocks[breakerIndex].stopLevel = swingLows[i] - (GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 0.5);
         ...
         for(int j=1; j<lowCount; j++) {
            ...
            if(swingLowTimes[j] < swingHighTimes[i] && j+1 < lowCount) {
               ...
               if(swingLowTimes[j+1] > swingHighTimes[i] && swingLows[j+1] < swingLows[j]) {
                  ...
                  breakerBlocks[breakerIndex].entryLevel = breakerLevel;
                  breakerBlocks[breakerIndex].stopLevel = swingHighs[i] + (GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 0.5);
                  breakerBlocks[breakerIndex].strength = strength;
...
bool IsOrderBlockValid(string symbol, double price, datetime blockTime, int strength, bool isBullish) {
    ...
    bool priceBreached = isBullish ? 
        (currentPrice < price - (GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 0.5)) : 
        (currentPrice > price + (GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 0.5));
   ...
   if(canTradeNow) {
      ...
      if(bestBuyBlockIndex >= 0) {
         ...
         if(MathAbs(currentPrice - blockPrice) < 5 * _Point) {
            ...
            double stopLoss = blockPrice - (GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 1.5);
...
void ManageOpenTrade(void)
{
   ...
   ArraySetAsSeries(atrBuffer, true);
   CopyBuffer(/* atrHandle removed: use GetATR utility instead */, 0, 0, 1, atrBuffer);
   double GetATR(Symbol(), PERIOD_CURRENT, 14, 0) = (ArraySize(atrBuffer) > 0) ? atrBuffer[0] : 0.01;
...
void StorePositionForReentry(ulong positionId, double stopPrice, ENUM_POSITION_TYPE posType)
{
   ...
   if(posType == POSITION_TYPE_BUY) {
      ReentryStops[idx] = stopPrice - GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * atrMultiplier;
      ReentryTargets[idx] = stopPrice + GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 2 * atrMultiplier;
   } else {
      ReentryStops[idx] = stopPrice + GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * atrMultiplier;
      ReentryTargets[idx] = stopPrice - GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 2 * atrMultiplier;
   }
...
bool IsSpreadAcceptable()
{
   ...
   double GetATR(Symbol(), PERIOD_CURRENT, 14, 0) = atrBuffer[0];
   
   // Set dynamic threshold based on ATR
   double maxAcceptableSpread = GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 0.25; // Default 25% of ATR
   ...
   if(isHighValueAsset) {
      ...
      maxAcceptableSpread = GetATR(Symbol(), PERIOD_CURRENT, 14, 0) * 0.625; // 2.5x the normal threshold
