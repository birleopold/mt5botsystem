...
#define PARAM_POP 50
#define PARAM_WINDOW 100
#define K_CLUSTERS 5
#define PATTERN_CLUSTER_SIZE 10

// Indicator Periods
#define RSI_PERIOD 14
#define MOMENTUM_PERIOD 12
...
#define PATTERN_BEAR 2

// Add missing input parameters
input int InpOrderDeviation = 10; // Order deviation in points
input bool InpNormalizeFeatures = true;
input int InpFeatureLookback = 50;
input int InpRegimeLookback = 100;
input int minSecondsBetweenTrades = 30; // Increased from 15 to reduce overtrading
input double maxSpreadFactor = 0.25;
input bool enableSpreadProtection = true;
input double maxSlippagePips = 2.0;
input int maxConsecutiveLosses = 3;
input int maxPositionsPerSymbol = 1;
input double minStopLoss = 18.0;  // Increased from 15.0 for better protection
input double maxStopLoss = 25.0;
input double minTakeProfit = 36.0;  // Increased from 30.0 for better RR ratio
input double maxTakeProfit = 55.0;  // Increased from 50.0 for better RR ratio
input double riskRewardRatio = 2.0;  // Increased from 1.67 for better trade quality
input double scalingFactor = 0.8;
input double maxDrawdownPct = 5.0;
input double maxDailyLossPct = 2.0;
input bool useKellySizing = true;
input double autoTPFactor = 1.5;
input double autoSLFactor = 1.0;
input bool useProfitProtection = true;
input double profitProtectionThreshold = 0.3;
input double profitLockPercentage = 0.6; // Increased from 0.5 to lock in more profits
input int profitCheckSeconds = 5;
input bool useTrailingTakeProfit = true;
input double tpExtensionFactor = 1.5; // Increased from 1.3 for more profit potential
input int maxTPTrailCount = 0;
input bool useVolumeSpike = true; // Enabled to use additional confirmation
input bool useImbalance = true; // Enabled to use additional confirmation
input bool useOrderFlow = true; // Enabled to use additional confirmation
input bool useAggressiveTrailing = true;
...
struct MarketFeatures {
    ...
    double correlationStructure;
    double trendStrength;
};

// Global variables
MarketFeatures g_marketFeatures;
...
struct FeatureSet
{
    double rawFeatures[MAX_FEATURES];
    double normalizedFeatures[MAX_FEATURES];
    double featureMin[MAX_FEATURES];
    double featureMax[MAX_FEATURES];
};

// Global feature set
FeatureSet g_features;

// Function to initialize feature arrays
void InitializeFeatureArrays() {
    for(int i=0; i<MAX_FEATURES; i++)
    {
        g_features.rawFeatures[i] = 0.0;
        g_features.normalizedFeatures[i] = 0.0;
        g_features.featureMin[i] = DBL_MAX;
        g_features.featureMax[i] = -DBL_MAX;
    }
}

// Fix constant declarations - change from input to variables
double tpMultiplier = 2.0;
double slMultiplier = 1.0;
double trailingActivationPct = 0.5;
double volatilityThreshold = 0.5;
double maxKellyFraction = 0.25;

//+------------------------------------------------------------------+
//| ScalperV3 - Consolidated Advanced Algorithmic Trading System    |
//| Features:                                                       |
...
//+------------------------------------------------------------------+
// GP hyperparameters structure
struct GPParameters {
   double lengthScale;    // How quickly correlation falls off with distance
   double signalVariance; // Overall variance in the function
   double noiseVariance;  // Observation noise
};

// Parameter set for optimization
struct ParameterSet {
   double values[8];     // Parameter values
   double performance;   // Performance metric
   double uncertainty;   // Uncertainty estimate
   double acquisition;   // Acquisition function value
};
...
ParameterSet paramSetsBayes[PARAM_POP];
GPParameters gpParams;
ParameterBounds paramBounds[8];
int bayesianCounter = 0;
bool useBayesianOptimization = true;
datetime lastOptimizationTime = 0;
int optimizationInterval = 3600; // Seconds between optimizations (1 hour)

// Initialize GP hyperparameters
void InitializeGaussianProcess(GPParameters &params)
{
   params.lengthScale = 1.0;
   params.signalVariance = 1.0;
   params.noiseVariance = 0.01;
   
   Print("[BAYES] GP initialized with L=", params.lengthScale, 
         " sigma_f=", params.signalVariance, " sigma_n=", params.noiseVariance);
}

// Initialize parameter bounds
void InitializeParameterBounds() {
   // Buy threshold (0.5-0.6)
   paramBounds[0].min = 0.5;
   paramBounds[0].max = 0.6;
   paramBounds[0].name = "BuyThreshold";
   
   // Sell threshold (0.4-0.5)
   paramBounds[1].min = 0.4;
   paramBounds[1].max = 0.5;
   paramBounds[1].name = "SellThreshold";
   
   // Risk percentage (0.01-0.05)
   paramBounds[2].min = 0.01;
   paramBounds[2].max = 0.05;
   paramBounds[2].name = "RiskPercentage";
   
   // TP multiplier (1.0-2.0)
   paramBounds[3].min = 1.0;
   paramBounds[3].max = 2.0;
   paramBounds[3].name = "TPMultiplier";
   
   // SL multiplier (0.5-1.0)
   paramBounds[4].min = 0.5;
   paramBounds[4].max = 1.0;
   paramBounds[4].name = "SLMultiplier";
   
   // Trailing activation (0.5-0.9)
   paramBounds[5].min = 0.5;
   paramBounds[5].max = 0.9;
   paramBounds[5].name = "TrailingActivation";
   
   // Volatility threshold (1.0-2.0)
   paramBounds[6].min = 1.0;
   paramBounds[6].max = 2.0;
   paramBounds[6].name = "VolatilityThreshold";
   
   // Kelly fraction (0.05-0.3)
   paramBounds[7].min = 0.05;
   paramBounds[7].max = 0.3;
   paramBounds[7].name = "KellyFraction";
}

// Sample a new parameter set within bounds
void SampleParameterSet(ParameterSet &params) {
   for(int i=0; i<8; i++) {
      double range = paramBounds[i].max - paramBounds[i].min;
      params.values[i] = paramBounds[i].min + (MathRand()/32767.0) * range;
   }
   params.performance = 0.0;
   params.uncertainty = 1.0;
   params.acquisition = 0.0;
}

// Calculate squared distance between parameter sets
double SquaredDistance(ParameterSet &a, ParameterSet &b) {
   double sum = 0.0;
   for(int i=0; i<8; i++) {
      // Normalize by parameter range
      double diff = (a.values[i] - b.values[i]) / (paramBounds[i].max - paramBounds[i].min);
      sum += diff * diff;
   }
   return sum;
}

// Calculate RBF kernel between parameter sets
double RBFKernel(ParameterSet &a, ParameterSet &b) {
   double dist2 = SquaredDistance(a, b);
   return gpParams.signalVariance * MathExp(-0.5 * dist2 / (gpParams.lengthScale * gpParams.lengthScale));
}

// Calculate acquisition function (Upper Confidence Bound)
double CalculateAcquisition(ParameterSet &params, double explorationWeight=2.0) {
   // UCB acquisition function: mean + exploration_weight * std_dev
   return params.performance + explorationWeight * MathSqrt(params.uncertainty);
}

// Evaluate performance of a parameter set
double EvaluateParameterPerformance(ParameterSet &params) {
   ...
   if(predictionCount >= ACCURACY_WINDOW) {
      // Calculate profit factor
      double totalProfit = 0.0, totalLoss = 0.0;
      for(int i=0; i<ACCURACY_WINDOW; i++) {
         if(tradeProfits[i] > 0)
            totalProfit += tradeProfits[i];
         else
            totalLoss += MathAbs(tradeProfits[i]);
      }
      
      if(totalLoss > 0)
         profitFactor = totalProfit / totalLoss;
      else
         profitFactor = totalProfit > 0 ? 10.0 : 1.0; // Default values
      
      // Calculate drawdown
      double peak = 0.0, equity = 0.0;
      for(int i=0; i<ACCURACY_WINDOW; i++) {
         equity += tradeProfits[i];
         peak = MathMax(peak, equity);
         if(peak > 0)
            maxDrawdown = MathMax(maxDrawdown, (peak - equity) / peak);
      }
   }
   else {
   ...
   double perfMetric = accuracy * accuracyWeight + 
                      profitFactor * profitWeight - 
                      maxDrawdown * drawdownWeight;
                      
   return perfMetric;
}

// Apply optimized parameters to trading algorithm
void ApplyParameterSet(ParameterSet &params) {
   // Apply parameters to global trading variables
   dynamicBuyThresh = params.values[0];
   dynamicSellThresh = params.values[1];
   runtimeRisk = params.values[2];
   tpMultiplier = params.values[3];
   slMultiplier = params.values[4];
   trailingActivationPct = params.values[5];
   volatilityThreshold = params.values[6];
   maxKellyFraction = params.values[7];
   
   // Update regime-specific parameters if needed
   for(int i=0; i<REGIME_COUNT; i++) {
      // Just initialize if not set yet
      if(regimeDynamicBuyThresh[i] == 0)
         regimeDynamicBuyThresh[i] = dynamicBuyThresh;
      if(regimeDynamicSellThresh[i] == 0)
         regimeDynamicSellThresh[i] = dynamicSellThresh;
      if(regimeDynamicRisk[i] == 0)
         regimeDynamicRisk[i] = runtimeRisk;
   }
   
   Print("[BAYES] Applied optimized parameters: BuyThresh=", dynamicBuyThresh, 
         " SellThresh=", dynamicSellThresh, " Risk=", runtimeRisk,
         " TPMult=", tpMultiplier, " SLMult=", slMultiplier);
}

// Find best parameter set based on acquisition function
int FindBestParameterSet() {
   int bestIdx = 0;
   double bestAcq = -DBL_MAX;
   
   for(int i=0; i<PARAM_POP; i++) {
      double acq = CalculateAcquisition(paramSetsBayes[i]);
      if(acq > bestAcq) {
         bestAcq = acq;
         bestIdx = i;
      }
...
void InitializeBayesianOptimization() {
   // Initialize GP
   InitializeGaussianProcess(gpParams);
   
   // Initialize parameter bounds
   InitializeParameterBounds();
   
   // Initialize parameter sets with random values
   for(int i=0; i<PARAM_POP; i++) {
      SampleParameterSet(paramSetsBayes[i]);
   }
   
   // Set initial optimization time
   lastOptimizationTime = TimeCurrent();
   
   Print("[BAYES] Bayesian optimization framework initialized with ", PARAM_POP, " parameter sets");
}

// Run one iteration of Bayesian optimization
void RunBayesianOptimization() {
   if(!useBayesianOptimization) return;
   
   // Check if it's time to run optimization
   datetime currentTime = TimeCurrent();
   if(currentTime - lastOptimizationTime < optimizationInterval) return;
   
   Print("[BAYES] Starting optimization cycle #", ++bayesianCounter);
   
   // Evaluate all parameter sets
   for(int i=0; i<PARAM_POP; i++) {
      paramSetsBayes[i].performance = EvaluateParameterPerformance(paramSetsBayes[i]);
      
      // Calculate uncertainty (simple version - in a full implementation this would use GP)
      // Distance-based uncertainty - farther from evaluated points means higher uncertainty
      double minDist = DBL_MAX;
      for(int j=0; j<PARAM_POP; j++) {
         if(i != j) {
            double dist = MathSqrt(SquaredDistance(paramSetsBayes[i], paramSetsBayes[j]));
            if(dist < minDist) minDist = dist;
         }
      }
      paramSetsBayes[i].uncertainty = MathMin(1.0, MathMax(0.1, minDist * 5.0));
      
      // Calculate acquisition function
      paramSetsBayes[i].acquisition = CalculateAcquisition(paramSetsBayes[i]);
      
      Print("[BAYES] Set ", i, ": Performance=", paramSetsBayes[i].performance, 
            " Uncertainty=", paramSetsBayes[i].uncertainty,
            " Acq=", paramSetsBayes[i].acquisition);
   }
   
   // Find best parameter set
   int bestIdx = FindBestParameterSet();
   
   // Apply best parameter set
   Print("[BAYES] Applying best parameter set (index ", bestIdx, ")");
   ApplyParameterSet(paramSetsBayes[bestIdx]);
   
   // Replace worst performing set with new random sample
   int worstIdx = 0;
   double worstPerf = paramSetsBayes[0].performance;
   
   for(int i=1; i<PARAM_POP; i++) {
      if(paramSetsBayes[i].performance < worstPerf) {
         worstPerf = paramSetsBayes[i].performance;
         worstIdx = i;
      }
   }
   
   // Replace worst with new sample
   SampleParameterSet(paramSetsBayes[worstIdx]);
   Print("[BAYES] Replaced worst parameter set (index ", worstIdx, ")");
   
   // Update last optimization time
   lastOptimizationTime = currentTime;
   
   Print("[BAYES] Optimization cycle complete");
}

//+------------------------------------------------------------------+
//| Bayesian Optimization Structures                                |
//+------------------------------------------------------------------+
struct GPR_Parameters
{
    double length_scale;
    double noise_level;
...
double CalculateDynamicSL() {
    double currentAtr = iATR(_Symbol, PERIOD_CURRENT, ATR_PERIOD);
    return CalculateDynamicSL(currentRegime, currentAtr, autoSLFactor);
}
...
double CalculateDynamicTP() { 
    ...
    double volatilityRatio = currentAtr / (GetATR(_Symbol, PERIOD_D1, 14, 0) + 0.00001);
    return autoTPFactor * (1 + volatilityRatio) * currentAtr;
}
...
double FastPrediction(string symbol) {
    ...
    switch(currentRegime) {
        ...
        case BREAKOUT:
            // In breakout, be more selective with better confirmation
            prediction = 0.10*rsiNorm + 0.40*momentumNorm + 0.40*volumeImbalance + 0.10*maCross;
            break;
        case REVERSAL:
            // In reversal, be more selective with better confirmation
            prediction = 0.25*rsiNorm + 0.40*momentumNorm + 0.25*volumeImbalance + 0.10*maCross;
            break;
            
        case CHOPPY:
            // In choppy markets, be very selective
...
void OnTick() {
    ...
    // Apply profit protection mechanism to capture small profits
    ...
    if(spread > currentAtr * maxSpreadFactor) {
        return;
    }
    ...
    double dynamicSL = currentAtr * slMultiplier;
    double dynamicTP = currentAtr * tpMultiplier;
    ...
    switch(currentRegime) {
        ...
        case TRENDING_DOWN:
            ...
            dynamicTP = currentAtr * (tpMultiplier * 1.2);
            break;
            
        case HIGH_VOLATILITY:
            // In high volatility, use slightly wider SL but also wider TP
            dynamicSL = currentAtr * (slMultiplier * 1.2);
            dynamicTP = currentAtr * (tpMultiplier * 1.3);
            break;
            
        case LOW_VOLATILITY:
            // In low volatility, can use tighter SL/TP
            dynamicSL = currentAtr * (slMultiplier * 0.8);
            dynamicTP = currentAtr * (tpMultiplier * 0.9);
            break;
            
        case RANGING_NARROW:
            // In narrow range, use very tight SL/TP
            dynamicSL = currentAtr * (slMultiplier * 0.7);
            dynamicTP = currentAtr * (tpMultiplier * 0.8);
            break;
            
        case RANGING_WIDE:
            // In wide range, use more selective filters
            dynamicSL = currentAtr * (slMultiplier * 1.0);
            dynamicTP = currentAtr * (tpMultiplier * 1.1);
            break;
            
        case BREAKOUT:
            // In breakout, use tight SL and wider TP
            dynamicSL = currentAtr * (slMultiplier * 0.7);
            dynamicTP = currentAtr * (tpMultiplier * 1.5);
            break;
        ...
        case CHOPPY:
            ...
            dynamicSL = currentAtr * (slMultiplier * 1.1);
            dynamicTP = currentAtr * tpMultiplier;
            break;
            
        default:
            // Default parameters
            dynamicSL = currentAtr * 1.0;
            dynamicTP = currentAtr * 1.2;
    }
    ...
    if(canOpenNewPosition) {
        ...
        switch(currentRegime) {
            case TRENDING_UP:
                ...
                buyThreshold = 0.52;  // Raised from 0.48 - more selective
                sellThreshold = 0.40;  // Lowered from 0.45 - more selective
                rsiUpperLimit = 75;    // Lowered from 80
                rsiLowerLimit = 35;    // Raised from 30
                break;
                
            case TRENDING_DOWN:
                // In downtrend, easier to go short, harder to go long
                buyThreshold = 0.60;  // Raised from 0.55
                sellThreshold = 0.48;  // Lowered from 0.52
                rsiUpperLimit = 65;    // Lowered from 70
                rsiLowerLimit = 25;    // Raised from 20
                break;
                
            case HIGH_VOLATILITY:
                // In high volatility, be more selective
                buyThreshold = 0.62;  // Raised from 0.55 for more selectivity
                sellThreshold = 0.38;  // Lowered from 0.45 for more selectivity
                rsiUpperLimit = 70;  // Lowered from 75
                rsiLowerLimit = 30;  // Raised from 25
                break;
                
            case LOW_VOLATILITY:
                // In low volatility, be more selective
                buyThreshold = 0.55;  // Raised from 0.48
                sellThreshold = 0.45;  // Lowered from 0.52
                rsiUpperLimit = 65;    // Lowered from 70
                rsiLowerLimit = 35;    // Raised from 30
                break;
                
            case RANGING_NARROW:
                // In narrow range, focus on mean reversion with stricter filters
                buyThreshold = 0.55;  // Raised from 0.48
                sellThreshold = 0.45;  // Lowered from 0.52
                rsiUpperLimit = 65;    // Lowered from 70
                rsiLowerLimit = 35;    // Raised from 30
            ...
            case RANGING_WIDE:
                // In wide range, use more selective filters
                buyThreshold = 0.58;  // Raised from 0.52
                sellThreshold = 0.42;  // Lowered from 0.48
                rsiUpperLimit = 70;    // Lowered from 75
                rsiLowerLimit = 30;    // Raised from 25
                break;
            ...
            case BREAKOUT:
                ...
                rsiUpperLimit = 85;    // Raised from 80
                rsiLowerLimit = 15;    // Lowered from 20
                break;
            ...
            case REVERSAL:
                ...
                buyThreshold = 0.48;  // Lowered from 0.50
                sellThreshold = 0.52;  // Raised from 0.50
                rsiUpperLimit = 80;    // Lowered from 85
                rsiLowerLimit = 20;    // Raised from 15
                break;
        ...
        bool strongBuySignal = prediction > buyThreshold && rsi < rsiUpperLimit &&
                               momentum > 0.0005 && pattern == PATTERN_BULL && imbalance > 0.6 &&
                               efficiencyRatio > 0.6 && // Only trade in clearer trends
                               !(rsi > 70 && currentRegime == TRENDING_DOWN); // Only avoid extreme overbought in downtrend
        
        bool strongSellSignal = prediction < sellThreshold && rsi > rsiLowerLimit &&
                                momentum < -0.0005 && pattern == PATTERN_BEAR && imbalance < 0.4 &&
                                efficiencyRatio > 0.6 && // Only trade in clearer trends
                                !(rsi < 30 && currentRegime == TRENDING_UP); // Only avoid extreme oversold in uptrend
        // Enhanced buy execution with dynamic parameters
        if(strongBuySignal) {
            // Calculate optimal stop loss and take profit based on volatility and regime
            double sl = dynamicSL * 1.2; // Increase stop loss distance by 20%
            double tp = dynamicTP * 1.3; // Increase take profit distance by 30%
...
void ManageOpenPositionsAggressivelyImpl() {
    ...
    for(int i = 0; i < posCount; i++) {
        if(PositionSelectByTicket(tickets[i])) {
            ...
            // Get original TP distance in points from comment
            ...
            if(useTrailingTakeProfit && !maxTrailsReached && currentProfit >= currentTPLevel * 0.75) {
                ...
                if(trade.PositionModify(posTicket, newSL, newTP)) {
                    ...
                    string logEntry = StringFormat("%s: Position %d - Trailing TP activated at %.1f points, SL=%.5f, TP=%.5f", 
...
void ApplyProfitProtection() {
    ...
    // Static variable to track the last time we checked
    static datetime lastProfitCheckTime = 0;
    datetime currentTime = TimeCurrent();
    
    // Only check at specified intervals to avoid constant modifications
    if(currentTime - lastProfitCheckTime < profitCheckSeconds) {
        return;
    }
    lastProfitCheckTime = currentTime;
    ...
    for(int i = 0; i < posCount; i++) {
        if(PositionSelectByTicket(tickets[i])) {
            ...
            // Check if position is in profit and exceeds our minimum threshold
            if(currentProfit > 0 && currentProfit >= originalTP * profitProtectionThreshold) {
                // Check if current SL is at breakeven or worse
                bool slAtBreakeven = false;
                if(posType == POSITION_TYPE_BUY) {
                    slAtBreakeven = (posStopLoss <= posOpenPrice);
                } else {
                    slAtBreakeven = (posStopLoss >= posOpenPrice);
                }
                
                // Only proceed if SL is at breakeven and we have some profit to protect
                if(slAtBreakeven) {
                    // Calculate new stop loss to lock in some profit
                    double newSL = 0.0;
                    if(posType == POSITION_TYPE_BUY) {
                        newSL = posOpenPrice + (currentProfit * profitLockPercentage * pointSize);
                        // Only update if new SL is better than current
                        if(newSL > posStopLoss) {
                            CTrade trade;
                            trade.SetExpertMagicNumber(InpMagicNumber);
                            if(trade.PositionModify(posTicket, newSL, posTakeProfit)) {
                                Print("[Profit Protection] Updated BUY SL from ", DoubleToString(posStopLoss, digits), 
                                      " to ", DoubleToString(newSL, digits), " (locking ", 
                                      DoubleToString(profitLockPercentage * 100, 1), "% of ", 
                                      DoubleToString(currentProfit, 1), " points profit)");
                                
                                // Log to file
                                string logEntry = StringFormat("%s: Position %d - Profit protection activated at %.1f points, SL=%.5f", 
                                                            TimeToString(currentTime), posTicket, currentProfit, newSL);
                                int fileHandle = FileOpen("ScalperV3_Profit_Protection_Log.txt", FILE_WRITE|FILE_READ|FILE_SHARE_READ|FILE_SHARE_WRITE);
                                if(fileHandle != INVALID_HANDLE) {
                                    FileSeek(fileHandle, 0, SEEK_END);
                                    FileWriteString(fileHandle, logEntry + "\r\n");
                                    FileClose(fileHandle);
                        ...
                        newSL = posOpenPrice - (currentProfit * profitLockPercentage * pointSize);
                        // Only update if new SL is better than current
                        if(newSL < posStopLoss || posStopLoss == 0) {
                            CTrade trade;
                            trade.SetExpertMagicNumber(InpMagicNumber);
                            if(trade.PositionModify(posTicket, newSL, posTakeProfit)) {
                                Print("[Profit Protection] Updated SELL SL from ", DoubleToString(posStopLoss, digits), 
                                      " to ", DoubleToString(newSL, digits), " (locking ", 
                                      DoubleToString(profitLockPercentage * 100, 1), "% of ", 
                                      DoubleToString(currentProfit, 1), " points profit)");
...
double CalculateHurstExponent(int lookback=100)
{
    ...
    int copied = CopyClose(_Symbol, PERIOD_CURRENT, 0, lookback, prices);
    
    if(copied < lookback) {
        Print("[ERROR] Failed to copy prices for Hurst calculation");
        return 0.5; // Neutral value
    }
    
    // Calculate mean
    double mean = 0.0;
    for(int i=0; i<lookback; i++) mean += prices[i];
    mean /= lookback;
    
    // Calculate cumulative deviation and range
    double cumulativeDeviation = 0.0;
    double maxDev = -DBL_MAX, minDev = DBL_MAX;
    
    for(int i=0; i<lookback; i++) {
        cumulativeDeviation += (prices[i] - mean);
        if(cumulativeDeviation > maxDev) maxDev = cumulativeDeviation;
        if(cumulativeDeviation < minDev) minDev = cumulativeDeviation;
    }
    ...
    double stdDev = 0.0;
    for(int i=0; i<lookback; i++) stdDev += MathPow(prices[i] - mean, 2);
    stdDev = MathSqrt(stdDev/lookback);
    
    // Apply formula for Hurst exponent
    if(range <= 0 || stdDev <= 0 || lookback <= 1) return 0.5;
    return MathLog(range/stdDev)/MathLog(lookback);
}

// Calculate volatility clustering (autocorrelation of squared returns)
double CalculateVolatilityClustering(int lookback=20)
{
    double returns[], squaredReturns[];
    ArrayResize(returns, lookback);
    ArrayResize(squaredReturns, lookback);
    
    // Get closing prices
    ArraySetAsSeries(returns, true);
    int copied = CopyClose(_Symbol, PERIOD_CURRENT, 0, lookback, returns);
    
    if(copied < lookback) {
        Print("[ERROR] Failed to copy prices for volatility clustering calculation");
        return 0.0;
    }
    
    // Calculate returns and squared returns
    for(int i=0; i<lookback; i++) {
        returns[i] = (returns[i] - returns[i+1]) / returns[i+1]; // Simple returns
        squaredReturns[i] = MathPow(returns[i], 2);
    }
    ...
    // Calculate mean of squared returns
    for(int i=0; i<lookback; i++) {
        meanSqr += squaredReturns[i];
    }
    meanSqr /= lookback;
    
    // Calculate autocorrelation
    for(int i=0; i<lookback-1; i++) {
        sumProduct += (squaredReturns[i] - meanSqr) * (squaredReturns[i+1] - meanSqr);
        sumSquares += MathPow(squaredReturns[i] - meanSqr, 2);
    }
    
    if(sumSquares <= 0.0) return 0.0;
    return sumProduct / sumSquares;
}

// Calculate volume imbalance (buy vs sell pressure)
double CalculateVolumeImbalance(int lookback=14)
{
    double closes[], opens[], volumes[];
    ArrayResize(closes, lookback);
    ArrayResize(opens, lookback);
    ...
    int copiedClose = CopyClose(_Symbol, PERIOD_CURRENT, 0, lookback, closes);
    int copiedOpen = CopyOpen(_Symbol, PERIOD_CURRENT, 0, lookback, opens);
    // Fix volume handling
    long volumesLong[];
    ArrayResize(volumesLong, lookback);
    ArrayResize(volumes, lookback);
    int copiedVolume = CopyTickVolume(_Symbol, PERIOD_CURRENT, 0, lookback, volumesLong);
    
    // Convert long array to double array
    for(int i=0; i<lookback; i++) {
        volumes[i] = (double)volumesLong[i];
    }      
    
    if(copiedClose < lookback || copiedOpen < lookback || copiedVolume < lookback) {
        Print("[ERROR] Failed to copy data for volume imbalance calculation");
        return 0.0;
    }
    ...
    for(int i=0; i<lookback; i++) {
        // If close > open, consider buying pressure
        if(closes[i] > opens[i]) {
            buyVol += volumes[i];
        } else if(closes[i] < opens[i]) {
            sellVol += volumes[i];
        } else {
            // If equal, split the volume
            buyVol += volumes[i] / 2.0;
            sellVol += volumes[i] / 2.0;
        }
    }
    ...
    // Return normalized imbalance between -1 and 1
    return (buyVol - sellVol) / totalVol;
}

// Calculate correlation structure of recent price changes
double CalculateCorrelationStructure(int lookback=10)
{
    double prices[], changes[];
    ArrayResize(prices, lookback+1);
    ArrayResize(changes, lookback);
    
    // Get closing prices
    ArraySetAsSeries(prices, true);
    int copied = CopyClose(_Symbol, PERIOD_CURRENT, 0, lookback+1, prices);
    
    if(copied < lookback+1) {
        Print("Error getting prices for correlation calculation");
        return 0.0;
    }
...
// Enhanced regime detection function using statistical features
int EnhancedRegimeDetection() {
    // Calculate market features
    MarketFeatures features;
    
    // Statistical features
    features.hurstExponent = CalculateHurstExponent(InpRegimeLookback);
    features.volatilityClustering = CalculateVolatilityClustering(InpRegimeLookback/2);
    features.volumeImbalance = CalculateVolumeImbalance(InpRegimeLookback);
    features.correlationStructure = CalculateCorrelationStructure(InpRegimeLookback/5);
    
    // Get ADX for trend strength - normalize to 0-1
    double adx = GetADXValue(14);
    features.trendStrength = adx / 100.0;
    
    // Classify regime based on features
    if(features.hurstExponent > 0.65 && features.trendStrength > 0.6) {
        if(features.volumeImbalance > 0.3) {
            Print("[REGIME] Strong uptrend detected (Hurst=", DoubleToString(features.hurstExponent, 2), 
                  ", Trend=", DoubleToString(features.trendStrength, 2), ")");
            return TRENDING_UP;
        }
        if(features.volumeImbalance < -0.3) {
            Print("[REGIME] Strong downtrend detected (Hurst=", DoubleToString(features.hurstExponent, 2), 
                  ", Trend=", DoubleToString(features.trendStrength, 2), ")");
            return TRENDING_DOWN;
        }
    }
    ...
    if(features.volatilityClustering > 0.6) {
        if(features.hurstExponent < 0.45) {
            Print("[REGIME] High volatility detected (Vol.Cluster=", DoubleToString(features.volatilityClustering, 2), 
                  ", Hurst=", DoubleToString(features.hurstExponent, 2), ")");
            return HIGH_VOLATILITY;
        }
    }
    
    // Ranging market detection
    if(features.hurstExponent < 0.45 && features.trendStrength < 0.3) {
        if(features.volatilityClustering < 0.3) {
            Print("[REGIME] Narrow range detected (Hurst=", DoubleToString(features.hurstExponent, 2), 
                  ", Vol.Cluster=", DoubleToString(features.volatilityClustering, 2), ")");
            return RANGING_NARROW;
        } else {
            Print("[REGIME] Wide range detected (Hurst=", DoubleToString(features.hurstExponent, 2), 
                  ", Vol.Cluster=", DoubleToString(features.volatilityClustering, 2), ")");
            return RANGING_WIDE;
        }
    }
    ...
    // Breakout detection
    if(features.correlationStructure < -0.5 && features.hurstExponent > 0.55) {
        Print("[REGIME] Breakout detected (Corr=", DoubleToString(features.correlationStructure, 2), 
              ", Hurst=", DoubleToString(features.hurstExponent, 2), ")");
        return BREAKOUT;
    }
    
    // Reversal detection
    if(features.correlationStructure < -0.4 && features.hurstExponent < 0.45) {
        Print("[REGIME] Reversal detected (Corr=", DoubleToString(features.correlationStructure, 2), 
              ", Hurst=", DoubleToString(features.hurstExponent, 2), ")");
        return REVERSAL;
    }
    
    // Choppy market detection
    if(features.hurstExponent < 0.4 && features.volatilityClustering > 0.4 && 
       MathAbs(features.correlationStructure) < 0.2) {
        Print("[REGIME] Choppy market detected (Hurst=", DoubleToString(features.hurstExponent, 2), 
              ", Vol.Cluster=", DoubleToString(features.volatilityClustering, 2), 
              ", Corr=", DoubleToString(features.correlationStructure, 2), ")");
        return CHOPPY;
    }
    
    // Default - low volatility
    Print("[REGIME] Low volatility market detected (default)");
    return LOW_VOLATILITY;
}
...
//+------------------------------------------------------------------+
void ExtractAdvancedFeatures(int regime)
{
   // 1. Statistical features
   // Get Hurst Exponent
   g_features.rawFeatures[0] = CalculateHurstExponent(InpFeatureLookback);
   g_features.rawFeatures[1] = CalculateVolatilityClustering(InpFeatureLookback/2);
   g_features.rawFeatures[2] = CalculateVolumeImbalance(InpFeatureLookback);
   ...
   if(CopyBuffer(rsiHandle, 0, 0, 1, rsiBuffer) == 1) {
       g_features.rawFeatures[3] = rsiBuffer[0];
   }
   ...
   if(CopyBuffer(atrHandle, 0, 0, 1, atrBuffer) == 1) {
       g_features.rawFeatures[4] = atrBuffer[0];
   }
   IndicatorRelease(atrHandle);
   
   // 3. Market structure features
   g_features.rawFeatures[5] = g_marketFeatures.trendStrength;
   g_features.rawFeatures[6] = g_marketFeatures.volumeImbalance;
...
// Update feature min/max for normalization
void UpdateFeatureMinMax()
{
   for(int i=0; i<MAX_FEATURES; i++) {
      if(g_features.rawFeatures[i] < g_features.featureMin[i])
         g_features.featureMin[i] = g_features.rawFeatures[i];
         
      if(g_features.rawFeatures[i] > g_features.featureMax[i])
         g_features.featureMax[i] = g_features.rawFeatures[i];
   }
}
...
{
   for(int i=0; i<MAX_FEATURES; i++) {
      double range = g_features.featureMax[i] - g_features.featureMin[i];
      
      // Avoid division by zero
      if(range > 0.0001) {
         g_features.normalizedFeatures[i] = (g_features.rawFeatures[i] - g_features.featureMin[i]) / range;
      } else {
         g_features.normalizedFeatures[i] = 0.5; // Default to middle value
      }
      
      // Safety bounds check
      if(g_features.normalizedFeatures[i] < 0.0) g_features.normalizedFeatures[i] = 0.0;
      if(g_features.normalizedFeatures[i] > 1.0) g_features.normalizedFeatures[i] = 1.0;
   }
}

//+------------------------------------------------------------------+
//| Helper function for ADX                                          |
//+------------------------------------------------------------------+
double GetADXValue(int period=14)
{
   ...
   // Don't forget to release the indicator handle
