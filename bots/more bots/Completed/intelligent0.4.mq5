...
double BaseRiskPercent = 1.0;      // Base risk percentage (1% of account)
double MaxRiskPercent = 2.0;       // Maximum risk per trade
double MaxDailyRiskPercent = 5.0;  // Maximum daily risk
double MaxExposurePercent = 15.0;  // Maximum total exposure
double MaxCorrelatedRisk = 3.0;    // Maximum risk across correlated pairs
double MaxDrawdownPercent = 20.0;  // Maximum allowed drawdown before pausing trading
double DrawdownPauseLevel = 10.0;  // Pause trading at this drawdown level
double DrawdownStopLevel = 20.0;   // Stop trading completely at this drawdown
int MaxOpenPositions = 5;         // Maximum number of open positions allowed
double VolatilityMultiplier = 1.0; // Adjusts position size based on volatility
double PatternQualityMultiplier = 1.0; // Adjusts position size based on pattern quality
bool AdaptToVolatility = true;   // Whether to adapt to volatility
bool RiskManagementEnabled = true; // Toggle for risk management features
bool DrawdownProtectionEnabled = true; // Toggle for drawdown protection
bool CorrelationRiskEnabled = true; // Toggle for correlation risk management
bool MaxPositionsLimitEnabled = true; // Toggle for maximum positions limit

// Market Regime settings
bool EnableRegimeFilters = true;      // Whether to use market regime filters
bool EnableNewsFilter = true;         // Whether to avoid trading during news events
int VolatilityLookback = 20;          // Periods to look back for volatility regime
int TrendStrengthLookback = 50;       // Periods to look back for trend strength
double NewsAvoidanceMinutes = 30;     // Minutes to avoid trading before/after news
...
bool EnablePerformanceStats = true;   // Whether to track performance stats
...
void UpdatePerformanceStats(double profit, int setupQuality)
{
   ...
   if(profit > 0) {
      WinningTrades++;
      GrossProfit += profit;
      LargestWin = MathMax(LargestWin, profit);
      AverageWin = (AverageWin * (WinningTrades - 1) + profit) / WinningTrades;
   } else {
      LosingTrades++;
      double loss = MathAbs(profit);
      GrossLoss += loss;
      LargestLoss = MathMax(LargestLoss, loss);
      AverageLoss = (AverageLoss * (LosingTrades - 1) + loss) / LosingTrades;
   }
   
   // Update quality-based stats
   if(setupQuality >= 8) {
      HighQualityTrades++;
   } else if(setupQuality >= 5) {
      MediumQualityTrades++;
   } else {
      LowQualityTrades++;
   }
