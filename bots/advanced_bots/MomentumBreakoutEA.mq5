...
//|   Donchian Channel Breakout with SMC & Risk Management          |
...
input bool   InpUseSmcFilters   = true;     // Use SMC to filter breakout signals
...
int OnInit()
{
   ...
   Print("[INFO] MomentumBreakoutEA v2.00 initialized with SMC features");
...
int TradeSignal()
{
   ...
   // Check buy signal
   if(close > ExtDonchianUpper[0]) return 1;
   
   // Check sell signal
   if(close < ExtDonchianLower[0]) return -1;
...
// Apply SMC filters to the breakout trade signal
int ApplySmcFilters(int signal)
{
   ...
   if(g_choch_count > 0) {
      ...
      // BUY signal aligns with bullish CHoCH
      ...
      // SELL signal aligns with bearish CHoCH
