...
#include "error_description.mqh"  // Include error descriptions for better error handling
...
string ErrorDescription(int error_code); // Function to convert error codes to messages
...
bool ExecuteTradeWithSignal(int signal, double entryPrice, double stopLoss)
{
   ...
   // Strict enforcement of position limit
   if(openPositions >= MaxOpenPositions) {
       Print("[TRADE] STRICT LIMIT: Maximum open positions (", MaxOpenPositions, ") reached. Trade not executed.");
       Print("[TRADE] Current open positions: ", openPositions, ", Max allowed: ", MaxOpenPositions);
       return false;
   }
   
   // Double-check to ensure we're not exceeding the limit
   if(PositionsTotal() > 0) {
       // Re-count just to be extra safe (could be positions opened from other EAs)
       int doubleCheckCount = CountOpenPositions();
       if(doubleCheckCount >= MaxOpenPositions) {
   ...
   if(signal > 0) { // Buy
      result = trade.Buy(posSize, Symbol(), 0, stopLoss, takeProfit, comment);
   } else if(signal < 0) { // Sell
      result = trade.Sell(posSize, Symbol(), 0, stopLoss, takeProfit, comment);
   }
...
void ManageTrailingStops() 
{
   ...
   for(int i=0; i<PositionsTotal(); i++) {
      ...
      double trailAmount = atr * TrailingStopMultiplier;
       ...
       // For runner positions (after partial close), activate trailing even earlier
       if(StringFind(PositionGetString(POSITION_COMMENT), "Runner") >= 0 || 
          StringFind(PositionGetString(POSITION_COMMENT), "Partial") >= 0) {
          activationThreshold = TrailingActivationPct * 0.7; // 30% earlier for runners
       }
      ...
      if(profitPct >= activationThreshold) {
         ...
         if(posType == POSITION_TYPE_BUY) {
            ...
            if(potentialSL < currentSL || MathAbs(currentSL) < _Point) {
               newSL = potentialSL;
...
void OnTick()
{
   ...
   // After DetectOrderBlocks() and DetectCHOCH(), call the stop modification function
   DetectOrderBlocks();
   DetectCHOCH();
   ModifyStopsOnCHOCH();
   ...
   if(ConsecutiveTrades >= MaxConsecutiveTrades) {
      canTradeNow = false;
      Print("[PROTECTION] Trading paused due to ", ConsecutiveTrades, 
            " consecutive trades (limit: ", MaxConsecutiveTrades, ")");
   }
   ...
   // Test trade execution every 5 minutes
    ...
    // Reuse the existing currentTime variable from above
    if(TimeCurrent() - lastTestTime > 300) { // 5 minutes
