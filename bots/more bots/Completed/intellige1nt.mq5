...
void ModifyStopsOnCHOCH()
{
   ...
   Print("[CHOCH_STOPS] Processing stop modifications based on recent CHOCH pattern");
   ...
   for(int i=PositionsTotal()-1; i>=0; i--) {
      ...
      if(posType == POSITION_TYPE_BUY && !recentCHOCHs[latestCHOCHIndex].isBullish) {
         ...
         if(newSL > currentSL) {
            Print("[CHOCH_STOPS] Adjusting BUY stop based on bearish CHOCH: ", 
      ...
      // For short positions, use bullish CHOCH to adjust stops
         ...
         if(newSL < currentSL || currentSL == 0) {
            Print("[CHOCH_STOPS] Adjusting SELL stop based on bullish CHOCH: ", 
                  DoubleToString(currentSL, _Digits), " -> ", 
                  DoubleToString(newSL, _Digits));
            trade.PositionModify(ticket, newSL, PositionGetDouble(POSITION_TP));
         }
