...
input double TrailingActivationPct = 0.3;     // When to activate trailing (% of TP reached)
...
void ManageTrailingStops() 
{
   ...
   // Using reverse iteration to prevent race conditions when positions change
   for(int i=PositionsTotal()-1; i>=0; i--) {
      ulong ticket = PositionGetTicket(i);
      if(ticket <= 0) continue;
      if(!PositionSelectByTicket(ticket)) continue;
   ...
   for(int i=PositionsTotal()-1; i>=0; i--) {
      ...
      if(initialRisk <= 0) initialRisk = 10 * _Point; // Use minimum risk
      double profitPct = priceMovement / initialRisk;
