...
struct OrderBlock {
    ...
    double high;           // High price of the block
    double low;            // Low price of the block
    double open;           // Open price of the block
    double close;          // Close price of the block
    double price;          // Price level of the block (from second definition)
    bool isBuy;            // Direction of the block (true for buy, false for sell)
    int strength;          // Strength/confidence in this block (0-10)
    double volume;         // Volume of the block
    bool valid;            // Whether this block meets criteria
    int barIndex;          // Bar index when detected
    bool tested;           // Whether it has been tested since formation (alias for touched)
    bool touched;          // Has the block been touched/tested?
    int score;             // Quality score (higher is better)
    ...
    for(int i = 0; i < PositionsTotal(); i++) {
        ...
        if(MathAbs(newSL - currentSL) > Point()) {
            ...
            if(trade_local.PositionModify(ticket, newSL, currentTP)) {
                ...
                LogError("[CHOCH] Failed to modify stop: " + ErrorDescription(GetLastError()));
            }
        }
    }
}
...
bool CheckForDivergence(int signal, double &qualityDescription) {
    ...
    if(regularDivergence || hiddenDivergence) {
        qualityDescription = qualityDescription + "\nDivergence: " + 
                            (regularDivergence ? "Regular" : "Hidden") + 
                            " (" + DoubleToString(divergenceQuality, 2) + ")"; 
        
        LogInfo(StringFormat("Divergence found - Type: %s, Strength: %.2f", 
               (regularDivergence ? "Regular" : "Hidden"), 
               divergenceQuality));
...
// EA logging and debugging - COMMENTED OUT DUE TO DUPLICATE DECLARATIONS
/* These parameters are already defined earlier in the code
...
// These parameters are already defined elsewhere
...
double CalculateSignalQuality(int signal) {
    ...
    divInfo.found = false;
    double qualityDescription = 0.0;
    CheckForDivergence(signal, qualityDescription); // Using the overloaded version with double parameter
    if(divInfo.found) {
        if(divInfo.type == DIVERGENCE_REGULAR_BULL || divInfo.type == DIVERGENCE_REGULAR_BEAR) {
            quality += 0.15 * 1.0; // Regular divergence gets full points
        } else if(divInfo.type == DIVERGENCE_HIDDEN_BULL || divInfo.type == DIVERGENCE_HIDDEN_BEAR) {
            quality += 0.15 * 0.5; // Hidden divergence gets half points
        }
        string divergenceType = (divInfo.type > 2) ? "Hidden" : "Regular";
        qualityDescription = StringFormat("%s\nDivergence: %s", qualityDescription, divergenceType);
    }
...
bool RetryTrade(int signal, double price, double sl, double tp, double size, int maxRetries=3) {
    ...
    for(int attempts = 0; attempts < maxRetries; attempts++) {
        ...
        if(signal > 0) {
            if(tradeMgr.Buy(retrySize, Symbol(), 0, sl, tp, "SMC Buy Retry "+IntegerToString(attempts+1)))
                return true;
        } else {
            if(tradeMgr.Sell(retrySize, Symbol(), 0, sl, tp, "SMC Sell Retry "+IntegerToString(attempts+1)))
                return true;
        }
...
void DetectOrderBlocks() {
    ...
    int lookback = 30; // Increased from 20 to 30 to catch more potential setups
    ...
    if(currentRegime == REGIME_TRENDING_UP || currentRegime == REGIME_TRENDING_DOWN) {
        ...
        Print("[ADAPTIVE BLOCK] Relaxing block requirements in trending market");
    }
    ...
    for(int i=0; i<MAX_BLOCKS; i++) {
        ...
        if(recentBlocks[i].valid) {
            ...
            // Only invalidate blocks if price has clearly broken through them
            // This is a more conservative approach to keep blocks valid longer
            if((recentBlocks[i].isBuy && close[0] < (recentBlocks[i].price - buffer*2)) ||
            ...
            else if(DisplayDebugInfo && ageInMinutes % 5 == 0) { // Log age every 5 minutes to reduce spam
    ...
    // Look for new blocks - slightly relaxed criteria
    for(int i=2; i<lookback-2; i++) {
        // Check for bullish order block (bearish reversal followed by upward movement)
        // Modified to be more lenient with one of the conditions
        bool cond1 = close[i] < close[i+1];
        bool cond2 = close[i] < close[i+2];
        bool cond3 = close[i-1] > close[i];
        bool cond4 = close[i-2] > close[i-1];
        
        // Relaxed condition: needs at least 3 of 4 conditions plus the critical reversal condition
        if((cond1 && cond2 && cond3) || (cond1 && cond2 && cond4) || (cond1 && cond3 && cond4) || (cond2 && cond3 && cond4)) {
            // Require at least the critical reversal condition
            if(cond3) { // Price must have reversed upward after the low
                ...
                if(DisplayDebugInfo) Print("[ORDER BLOCK] Bullish block detected at ", recentBlocks[localBlockIdx].price);
            }
        }
            ...
            if(DisplayDebugInfo) Print("[ORDER BLOCK] Strong bullish block detected at ", recentBlocks[localBlockIdx].price);
        }
        ...
        // Check for bearish order block (bullish reversal followed by downward movement)
        // Modified to be more lenient with one of the conditions
        cond1 = close[i] > close[i+1];
        cond2 = close[i] > close[i+2];
        cond3 = close[i-1] < close[i];
        cond4 = close[i-2] < close[i-1];
        
        // Relaxed condition: needs at least 3 of 4 conditions plus the critical reversal condition
        if((cond1 && cond2 && cond3) || (cond1 && cond2 && cond4) || (cond1 && cond3 && cond4) || (cond2 && cond3 && cond4)) {
            // Require at least the critical reversal condition
            if(cond3) { // Price must have reversed downward after the high
                ...
                recentBlocks[localBlockIdx].touched = false;
                recentBlocks[localBlockIdx].score = 5; // Basic score
                if(DisplayDebugInfo) Print("[ORDER BLOCK] Bearish block detected at ", recentBlocks[localBlockIdx].price);
            }
        }
        else if(cond1 && cond2 && cond3 && cond4) {
            // Original stricter criteria still works too
            ...
            recentBlocks[localBlockIdx].touched = false;
            recentBlocks[localBlockIdx].score = 7; // Higher score for perfect match
            if(DisplayDebugInfo) Print("[ORDER BLOCK] Strong bearish block detected at ", recentBlocks[localBlockIdx].price);
        }
        else if(DisplayDebugInfo && (cond1 + cond2 + cond3 + cond4) >= 2) {
            // Log near-misses for diagnostic purposes
...
//+------------------------------------------------------------------+
//| Calculate ATR value for the specified period                     |
//+------------------------------------------------------------------+
...
void OnDeinit(const int reason)
{
    ...
    if(LogPerformanceStats) {
        // Calculate performance metrics - sum up values from regime arrays
        double totalWins = 0, totalLosses = 0, totalProfit = 0;
        // Make sure we have the arrays defined and properly sized
        if(ArraySize(regimeWins) > 0 && ArraySize(regimeLosses) > 0 && ArraySize(regimeProfit) > 0) {
            for(int i=0; i<ArraySize(regimeWins); i++) {
                totalWins += regimeWins[i];
                totalLosses += regimeLosses[i];
                totalProfit += regimeProfit[i];
            }
...
bool ModifyStopsOnCHOCHImproved(ulong ticket, double& chochLevel, double& newSL) {
    ...
    if(posType == POSITION_TYPE_BUY) {
        ...
        localChochDetected = DetectBearishCHOCH(chochLevel);
...
//+------------------------------------------------------------------+
bool DetectBearishCHOCH(double& level) { // Fixed parameter to avoid hiding global
    // Initialize the level
    level = 0;
...
//+------------------------------------------------------------------+
// This duplicate ExecuteTradeWithRetry function was removed to avoid compilation errors
// Using the implementation at line 3650 instead.
...
void OnTick() {
    ...
    if(DisplayDebugInfo) Print("[ORDER BLOCKS] Valid buy blocks: ", validBuyBlocks, ", Valid sell blocks: ", validSellBlocks);
   ...
   // TrailingActivationPct is already defined as an input parameter
    ...
    if(validBuyBlocks > 0 && ((int)regime == (int)REGIME_TRENDING_UP || (int)regime == (int)REGIME_BREAKOUT)) {
    ...
    else if(validSellBlocks > 0 && ((int)regime == (int)REGIME_TRENDING_DOWN || (int)regime == (int)REGIME_REVERSAL)) {
    ...
    // Calculate adaptive signal quality threshold based on market conditions
    double baseSignalQuality = 0.5; // Base quality threshold (reduced from 0.6 to be less selective)
    double adaptiveSignalQuality = baseSignalQuality;
    ...
    double minSignalQuality = MathMax(minQualityThreshold, MathMin(maxQualityThreshold, adaptiveSignalQuality));
    ...
    if(signal != 0 && signalQuality >= minSignalQuality) {
        // Find the optimal stop loss level based on swing points and ATR
        double stopLoss = DetermineOptimalStopLoss(signal, atr);
        ...
        if(stopLoss > 0 && posSize > 0) {
            ...
            bool tradeResult = ExecuteTradeWithRetry(signal, entryPrice, stopLoss, takeProfit, posSize, comment);
...
double CalculateDynamicSize(double riskPercent, double stopDistance) {
    ...
    if(tickValue != 0 && stopDistance != 0) {
        lotSize = riskAmount / (stopDistance * tickValue / tickSize);
    }
    ...
    lotSize = MathFloor(lotSize / lotStep) * lotStep;
            ...
            double maxLot = SymbolInfoDouble(Symbol(), SYMBOL_VOLUME_MAX); // Use Symbol() instead of undeclared symbol
    
    lotSize = MathMax(minLot, MathMin(maxLot, lotSize));
    
    return lotSize;
}
// Note: No additional closing brace needed here
...
double CalculateDynamicTakeProfit(int signal, double entryPrice, double stopLossPrice) {
    ...
    int localRegime = currentRegime; // Use a local copy to avoid ambiguity
    if(EnableMarketRegimeFiltering && localRegime >= 0 && localRegime < REGIME_COUNT) {
        switch((int)localRegime) {
            case 4: // REGIME_TRENDING_UP
                rrMultiplier = 3.0; // Higher targets in trending markets
                break;
                
            case 5: // REGIME_TRENDING_DOWN
                rrMultiplier = 3.0; // Higher targets in trending markets
                break;
            ...
            case 2: // REGIME_HIGH_VOLATILITY
                rrMultiplier = 2.25; // Adjust for volatility
                break;
                
            case 3: // REGIME_CHOPPY
...
bool ExecuteTrade(int signal, double price, double sl, double tp) {
    ...
    bool success = false;
    if(signal > 0) { // BUY
        success = trade_local.Buy(lotSize, Symbol(), 0, sl, tp, "SMC Hybrid");
    } else { // SELL
        success = trade_local.Sell(lotSize, Symbol(), 0, sl, tp, "SMC Hybrid");
    }
...
double GetBands(string symbol, ENUM_TIMEFRAMES timeframe, int period, double deviation, int shift, ENUM_APPLIED_PRICE price_type, int mode, int buffer_index) {
    ...
    // Cast explicitly to avoid data loss warning
    if((long)handle == INVALID_HANDLE) {
    ...
    if(CopyBuffer(handle, mode, shift, 1, bands) <= 0) { // Removed unnecessary cast to double
    ...
    if(isBuy) {
        ...
        if(CopyLow(Symbol(), PERIOD_CURRENT, 0, 20, low) > 0 && 
           CopyHigh(Symbol(), PERIOD_CURRENT, 0, 20, high) > 0) {
            ...
            int swingLowBar = FindRecentSwingPoint(true); // Fixed parameter list
...
void FindOrderBlocks(string symbol, ENUM_TIMEFRAMES timeframe) {
    ...
    int copied = CopyRates(symbol, timeframe, 0, 30, rates);
    ...
    double atr = CalculateATR(14); // Get ATR for volatility context
    ...
    for(int i = 5; i < copied - 1; i++) {
        ...
        if(rates[i].close < rates[i].open && // Bearish candle
           ...
           rates[i-1].close - rates[i-1].open > atr * 0.5) { // Significant move
            ...
            recentBlocks[blockIndex].volume = (double)rates[i].tick_volume;
            ...
            double avgVolume = 0;
            for(int v = i; v < i + 5; v++) {
                if(v < copied) avgVolume += (double)rates[v].tick_volume;
            }
            avgVolume /= 5;
            
            if((double)rates[i].tick_volume > avgVolume * 1.5) score += 3;
            else if((double)rates[i].tick_volume > avgVolume) score += 2;
            else score += 1;
...
void AdjustTradeFrequency(int phase) {
    switch(phase) {
        case 1: // PHASE_TRENDING_UP
            ActualSignalCooldownSeconds = 1;
            break;
        case 2: // PHASE_TRENDING_DOWN
            ActualSignalCooldownSeconds = 1;
            break;
...
// Using the implementation defined at line 5488 instead
...
double PlaceholderForCalculateDynamicSize(double riskPercent, double stopDistance) {
    ...
    return CalculateDynamicSize(riskPercent, stopDistance);
}
...
// Define a proper implementation of OrderCheck that will replace the one that was removed
