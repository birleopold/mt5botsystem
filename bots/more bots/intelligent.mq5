...
input bool EnableDebugLogging = false;    // Enable debug logging
...
input double g_correlationRiskMultiplier = 0.5; // Correlation risk multiplier
...
enum ENUM_MARKET_REGIME {
   REGIME_UNKNOWN,     // Unknown or not yet classified
   REGIME_NORMAL,      // Normal market conditions
   REGIME_TRENDING,    // Market is trending (general)
   REGIME_TRENDING_BULL, // Strong bullish trend
   REGIME_TRENDING_BEAR, // Strong bearish trend
   REGIME_RANGING,     // Market is ranging
   REGIME_CHOPPY,      // Choppy/ranging market (synonym for RANGING)
   REGIME_VOLATILE,    // Market is volatile/highly volatile
   REGIME_BREAKOUT,    // Market is in breakout mode
   REGIME_STRONG_TREND,// Strong trend across multiple timeframes
   REGIME_MIXED,       // Mixed signals across timeframes
   REGIME_REVERSAL     // Potential reversal forming
};

// Utility function to get ATR value for a symbol/timeframe/period
// Usage: double atr = GetATR(Symbol(), Period(), 14, 0);
double GetATR(string symbol, ENUM_TIMEFRAMES timeframe, int period, int shift = 0) {
    ...
    if (atrHandle == INVALID_HANDLE) {
        Print("Failed to create ATR indicator handle for ", symbol);
        return 0.0;
    }
    double atrBuffer[];
    ArraySetAsSeries(atrBuffer, true);
    if (CopyBuffer(atrHandle, 0, shift, 1, atrBuffer) <= 0) {
        Print("Failed to copy ATR buffer for ", symbol);
...
enum ENUM_PATTERN_TYPE {
   ...
   PATTERN_CHOCH,       // Change of character pattern
   PATTERN_ORDER_BLOCK, // Order block pattern
   PATTERN_BREAKER,     // Breaker block pattern
   PATTERN_BOS,         // Break of structure pattern
   PATTERN_FVG,         // Fair value gap pattern
   PATTERN_LIQUIDITY    // Liquidity grab pattern
};
...
struct PairSettings {
    ...
    // Pattern weights structure for adaptive pattern selection
    struct PatternWeights {
        double orderBlockWeight;    // Weight for order block patterns
        double chochWeight;         // Weight for CHOCH patterns
        double bosWeight;           // Weight for BOS (break of structure) patterns
        double fvgWeight;           // Weight for FVG patterns
    };
    
    PatternWeights patternWeights;  // Pattern weights for this pair
    
    // Pair ranking and allocation metrics
    double rankScore;              // Overall ranking score for this pair
    double capitalAllocation;      // Percentage of capital allocated to this pair
    int validBlocksCount;          // Count of valid order blocks
    
    // Correlation data
    double correlationMatrix[MAX_PAIRS]; // Correlation with other pairs
    double averageSpread;          // Average spread for this pair
    double profitFactor;           // Profit factor for this pair
    
    // Correlation tracking
    double correlations[MAX_PAIRS]; // Correlation with other pairs
    double correlationExposure;    // Current correlation exposure
    ...
    struct ClosedTrade {
        datetime closeTime;         // When the trade was closed
        double profit;             // Profit/loss in pips
        ENUM_PATTERN_TYPE patternType; // Pattern that generated this trade
    ...
    ClosedTrade closedTrades[50];  // Store last 50 closed trades
    int closedTradesCount;         // Count of closed trades in the array
};
...
//+------------------------------------------------------------------+
//| Get adaptive order block strength threshold based on multiple factors |
//+------------------------------------------------------------------+
double GetAdaptiveBlockStrengthThreshold(string symbol) {
   int pairIndex = GetPairSettingsIndex(symbol);
   double baseThreshold = 6.0; // Default threshold
   
   // Adjust based on historical success rate
   if(g_pairSettings[pairIndex].orderBlockSuccessRate > 0.6) {
      // If order blocks have been very successful, we can be more selective
      baseThreshold *= 1.1;
      Print("[ADAPTIVE] Increasing threshold due to high success rate: ", 
            DoubleToString(g_pairSettings[pairIndex].orderBlockSuccessRate, 2));
   } else if(g_pairSettings[pairIndex].orderBlockSuccessRate < 0.4 && 
             g_pairSettings[pairIndex].performance.totalTrades > 5) {
      // If order blocks haven't been successful, be more permissive
      baseThreshold *= 0.9;
      Print("[ADAPTIVE] Decreasing threshold due to low success rate: ", 
            DoubleToString(g_pairSettings[pairIndex].orderBlockSuccessRate, 2));
   }
   
   // Adjust for pair-specific characteristics
   if(IsHighValueAsset(symbol)) {
      // More permissive for high-value assets like crypto and gold
      baseThreshold *= 0.75;
      Print("[ADAPTIVE] Applying high-value asset adjustment for ", symbol);
   }
   
   // Adjust for recent valid blocks count
   if(g_pairSettings[pairIndex].performance.totalTrades > 0) {
      // If we're not finding enough valid blocks, gradually reduce threshold
      int validBlocksCount = 0; // This would ideally be tracked in the pair settings
      
      // Use a proxy for valid blocks count based on recent trading activity
      datetime lastDay = TimeCurrent() - 86400; // Last 24 hours
      if(g_pairSettings[pairIndex].lastUpdated > lastDay) {
         validBlocksCount = 3; // Assume moderate activity if recently updated
      }
      
      if(validBlocksCount < 3) {
         baseThreshold *= 0.9;
         Print("[ADAPTIVE] Reducing threshold due to low valid block count");
      }
   }
   
   // Adjust for market volatility using ATR
   double currentAtr = GetATR(symbol, PERIOD_CURRENT, 14, 0);
   double prevAtr1 = GetATR(symbol, PERIOD_CURRENT, 14, 1);
   double prevAtr2 = GetATR(symbol, PERIOD_CURRENT, 14, 2);
   double avgAtr = (currentAtr + prevAtr1 + prevAtr2) / 3.0;
   ...
   if(isLowVolatility) {
      // In low volatility, be more permissive with thresholds
      baseThreshold *= 0.85;
      Print("[ADAPTIVE] Reducing threshold due to low volatility");
   } else if(isHighVolatility) {
      // In high volatility, be more selective to avoid false signals
      baseThreshold *= 1.15;
      Print("[ADAPTIVE] Increasing threshold due to high volatility");
   }
   
   // Floor at minimum threshold
   double minThreshold = IsHighValueAsset(symbol) ? 3.0 : 4.0;
   double result = MathMax(baseThreshold, minThreshold);
   
   Print("[ADAPTIVE] Final block strength threshold for ", symbol, ": ", DoubleToString(result, 2));
   return result;
}
...
void UpdateAdaptivePairSettings(string symbol) {
   ...
   if(slHitRate > 0.3 && g_pairSettings[pairIndex].performance.totalTrades > 10) {
       ...
       // Update success rate to influence adaptive threshold calculation
       g_pairSettings[pairIndex].orderBlockSuccessRate = MathMax(0.1, g_pairSettings[pairIndex].orderBlockSuccessRate - 0.05);
       Print("[ADAPTIVE] High SL hit rate (", DoubleToString(slHitRate*100, 1), "%). Increased SL multiplier to ", 
             DoubleToString(g_pairSettings[pairIndex].slMultiplier, 2), " and reduced order block success rate to ", 
             DoubleToString(g_pairSettings[pairIndex].orderBlockSuccessRate, 2));
   }
   ...
   if(g_pairSettings[pairIndex].performance.winRate > 0.65 && g_pairSettings[pairIndex].performance.totalTrades > 10) {
       // Increase success rate to make adaptive threshold more permissive
       g_pairSettings[pairIndex].orderBlockSuccessRate = MathMin(0.9, g_pairSettings[pairIndex].orderBlockSuccessRate + 0.05);
       ...
       Print("[ADAPTIVE] High win rate (", DoubleToString(g_pairSettings[pairIndex].performance.winRate*100, 1), 
             "%). Increased order block success rate to ", DoubleToString(g_pairSettings[pairIndex].orderBlockSuccessRate, 2), 
   ...
   if(g_pairSettings[pairIndex].performance.consecutiveLosses > 3) {
      // Decrease success rate to make adaptive threshold more selective
      g_pairSettings[pairIndex].orderBlockSuccessRate = MathMax(0.1, g_pairSettings[pairIndex].orderBlockSuccessRate - 0.1);
      g_pairSettings[pairIndex].signalQualityThreshold += 0.5;
      Print("[ADAPTIVE] ", g_pairSettings[pairIndex].performance.consecutiveLosses, " consecutive losses. Decreased order block success rate to ", 
            DoubleToString(g_pairSettings[pairIndex].orderBlockSuccessRate, 2), " and increased signal quality threshold to ", 
   ...
   // PATTERN 4: Volatility Adjustment
    ...
    if(g_volatilityContext.volatilityState == VOLATILITY_HIGH || g_volatilityContext.volatilityState == VOLATILITY_VERY_HIGH) {
       ...
       Print("[ADAPTIVE] High volatility detected. Decreased order block success rate and increased SL multiplier.");
    }
       ...
       Print("[ADAPTIVE] Low volatility detected. Increased order block success rate and decreased SL multiplier.");
    }
   ...
   if(g_pairSettings[pairIndex].orderBlockSuccessRate < 0.4 && g_pairSettings[pairIndex].performance.totalTrades > 15) {
       ...
       // We already use orderBlockSuccessRate in the adaptive threshold calculation
       // Just update the age hours parameter
       g_pairSettings[pairIndex].orderBlockAgeHours = MathMax(1.0, g_pairSettings[pairIndex].orderBlockAgeHours - 0.5);
       Print("[ADAPTIVE] Poor order block performance. Reduced age limit to ", 
             DoubleToString(g_pairSettings[pairIndex].orderBlockAgeHours, 1), " hours.");
   }
...
// Global variables
int blockIndex = 0;
// Global ATR value for use in multiple functions
...
input double DailyLossLimit = 15.0;     // Max daily loss percentage (% of balance) - INCREASED TO ALLOW TRADES
...
ENUM_MARKET_REGIME DetectMarketRegime()
{
   ...
   double atr[] = {0};
   // Use GetATR function to get ATR values
   for(int i = 0; i < lookback; i++) {
      atr[i] = GetATR(Symbol(), PERIOD_CURRENT, 14, i);
      if(atr[i] == 0) {
         Print("Error getting ATR data at index ", i);
   ...
   if(trendStrength > 0.65) {
      ...
      if(maAligned) {
         ...
         return isBullish ? REGIME_TRENDING : REGIME_TRENDING;
      }
   }
...
int OnInit()
{
   ...
   // ATR handle management now handled inside the GetATR function
   // localAtrHandle no longer needed
...
void OnDeinit(const int reason)
{
   ...
   // ATR handle management now handled inside GetATR function
}
...
void AnalyzePairBehavior(string symbol) {
   ...
   double localAtrValue = GetATR(symbol, PERIOD_CURRENT, 14, 0); // Fixed parameter count
   double avgATR = GetATR(symbol, PERIOD_CURRENT, 14, 0); // Fixed parameter count - would need to implement period averaging separately
   ...
   if(atrRatio > 1.5 || movementRatio > 0.7 || reversalRatio < 0.3 || avgSpread > GetATR(Symbol(), Period(), 14, 0) * 0.25) {
...
void CalculateVolumeProfile()
{
   ...
   // Use a local name to avoid hiding the global GetATR(Symbol(), Period(), 14, 0)
   double localAtrValue = 0;
   double atrBuffer[];
   ArraySetAsSeries(atrBuffer, true);
   // Use GetATR function directly instead of CopyBuffer
   localAtrValue = GetATR(Symbol(), Period(), 14, 0);
   
   if(localAtrValue > 0) {
      // ATR value successfully retrieved
      // The GetATR function now handles everything internally
      // localAtrValue is already set from GetATR function call
...
double DetermineOptimalStopLoss(int signal, double entryPrice)
{
   ...
   int /* atrHandle removed: use GetATR utility instead */Current = iATR(Symbol(), PERIOD_CURRENT, 14);
   int /* atrHandle removed: use GetATR utility instead */H1 = iATR(Symbol(), PERIOD_H1, 14);
   int /* atrHandle removed: use GetATR utility instead */H4 = iATR(Symbol(), PERIOD_H4, 14); // Added H4 for wider perspective
   
   if(/* atrHandle removed: use GetATR utility instead */Current == INVALID_HANDLE || /* atrHandle removed: use GetATR utility instead */H1 == INVALID_HANDLE || /* atrHandle removed: use GetATR utility instead */H4 == INVALID_HANDLE) {
   ...
   int copiedCurrent = CopyBuffer(/* atrHandle removed: use GetATR utility instead */Current, 0, 0, 3, atrCurrent);
   int copiedH1 = CopyBuffer(/* atrHandle removed: use GetATR utility instead */H1, 0, 0, 3, atrH1);
   int copiedH4 = CopyBuffer(/* atrHandle removed: use GetATR utility instead */H4, 0, 0, 3, atrH4);
   ...
   // Create a weighted ATR value instead of trying to assign to the function
   double weightedAtr = (atrCurrent[0] + atrH1[0] * 2 + atrH4[0] * 3) / 6;
   ...
   // We already calculated GetATR(Symbol(), Period(), 14, 0) from multiple timeframes above
   
   // Calculate ATR-based stop (standard approach)
   double stopLoss = 0.0;
   if(signal > 0) {
      stopLoss = entryPrice - (GetATR(Symbol(), Period(), 14, 0) * atrMultiplier * slMultiplier);
   } else {
      stopLoss = entryPrice + (GetATR(Symbol(), Period(), 14, 0) * atrMultiplier * slMultiplier);
   }
   ...
   Print("[SL] ATR Current: ", DoubleToString(atrCurrent[0], 5),
         ...
         " Weighted: ", DoubleToString(GetATR(Symbol(), Period(), 14, 0), 5));
   ...
   for(int i=0; i<MAX_CHOCHS; i++) {
      ...
      if(signal > 0 && !recentCHOCHs[i].isBullish) {
         chochStop = recentCHOCHs[i].price - (GetATR(Symbol(), Period(), 14, 0) * 0.5 * slMultiplier); // Add buffer below CHOCH
         foundChochStop = true;
      ...
      // For SELL positions, use bullish CHOCH as potential stop
         ...
         chochStop = recentCHOCHs[i].price + (GetATR(Symbol(), Period(), 14, 0) * 0.5 * slMultiplier); // Add buffer above CHOCH
         foundChochStop = true;
   ...
   if(signal > 0) { // BUY position, look for recent swing low
      ...
      if(isSwingLow) {
         swingStop = lowestLow - (GetATR(Symbol(), Period(), 14, 0) * 0.3 * slMultiplier); // Add buffer below swing
         foundSwingStop = true;
      ...
      if(isSwingHigh) {
         swingStop = highestHigh + (GetATR(Symbol(), Period(), 14, 0) * 0.3 * slMultiplier); // Add buffer above swing
         foundSwingStop = true;
   ...
   double minStopDistance = 3 * GetATR(Symbol(), Period(), 14, 0);
...
double CalculatePositionSize(double entryPrice, double stopLoss, double riskPercent=1.0, int setupQuality=5)
{
   ...
   if(RiskManagementEnabled) {
      // Get current ATR value using GetATR function
      double currentATR = GetATR(Symbol(), Period(), 14, 0);
      
      // Get historical ATR for comparison
      double atrHistory[20];
      // Fill with historical ATR values
      for(int i=0; i<20; i++) {
         atrHistory[i] = GetATR(Symbol(), PERIOD_CURRENT, 14, i);
      }
      ...
      if(avgHistoricalATR > 0) {
         double volatilityRatio = currentATR / avgHistoricalATR;
   ...
   for(int i=0; i<MAX_BLOCKS; i++) {
      if(recentBlocks[i].valid) {
         if((signal > 0 && recentBlocks[i].isBuy) || 
            ...
            double threshold = GetATR(Symbol(), Period(), 14, 0) * 0.5; // Within half ATR of block
                     ...
                     double posSize, int setupQuality, bool executed)
{
   ...
   logEntry += "\nATR Value: " + DoubleToString(GetATR(Symbol(), Period(), 14, 0), _Digits);
...
bool CanTrade()
{
   ...
   int /* atrHandle removed: use GetATR utility instead */Local = iATR(Symbol(), PERIOD_CURRENT, 14);
   double atrBuffer[];
   ArraySetAsSeries(atrBuffer, true);
   double atr = 0.001; // Default value
   
   if(CopyBuffer(/* atrHandle removed: use GetATR utility instead */Local, 0, 0, 1, atrBuffer) > 0) {
      atr = atrBuffer[0];
   } else {
   ...
   IndicatorRelease(/* atrHandle removed: use GetATR utility instead */Local);
...
void DetectOrderBlocks()
{
   ...
   // Get pair-specific settings for block age
   double maxBlockAgeHours = g_pairSettings[pairIndex].orderBlockAgeHours;
   
   // Get adaptive block strength threshold based on multiple factors
   double minBlockStrength = GetAdaptiveBlockStrengthThreshold(symbol);
   ...
   if(IsHighValueAsset(symbol)) {
      ...
      Print("[HIGH-VALUE] Using extended age for order blocks: ", maxBlockAgeHours, " hours");
   }
   
   // Check for low volatility periods and extend block lifetime if needed
   // Compute volatility ratio as current ATR / 20-period moving average
   double currentATR = GetATR(symbol, PERIOD_CURRENT, 14, 0);
   // Calculate a simple 20-period average manually
   double totalATR = 0.0;
   for(int i=0; i<20; i++) {
      totalATR += GetATR(symbol, PERIOD_CURRENT, 14, i);
   }
   double avgATR = totalATR / 20; // 20-period average
   ...
   for(int i=0; i<MAX_BLOCKS; i++) {
      ...
      // Check block strength using adaptive threshold
      double adaptiveThreshold = GetAdaptiveBlockStrengthThreshold(symbol);
      if(recentBlocks[i].strength < adaptiveThreshold) {
         recentBlocks[i].valid = false;
         Print("[BLOCK] Invalidated block #", i, " due to insufficient strength: ", 
               recentBlocks[i].strength, " < ", adaptiveThreshold);
         continue;
      }
...
void GetLatestPriceData()
{
   ...
   // Calculate volatility metrics for adaptive thresholds using GetATR function
   double atr = GetATR(Symbol(), Period(), 14, 0);
...
bool RetryTradeExecutionWithErrorHandler(string symbol, datetime currentTime, int signal = 1)
{
   ...
   // Apply correlation risk management to adjust position size
   double correlationRiskMultiplier = CheckCorrelationRisk(symbol, tradeSignal);
   ...
   // Calculate SL/TP based on trade direction
    if(tradeSignal > 0) { // BUY
        ...
        Print("[REAL-EXECUTION] Using ATR-based SL/TP for BUY: ATR=", DoubleToString(localAtrValue/_Point, 1), " pips, SL=", 
        ...
        Print("[REAL-EXECUTION] Using ATR-based SL/TP for SELL: ATR=", DoubleToString(localAtrValue/_Point, 1), " pips, SL=", 
              DoubleToString((localStopLoss-currentPrice)/_Point, 1), " pips, TP=", 
              DoubleToString((currentPrice-localTakeProfit)/_Point, 1), " pips");
    }
   ...
   // Ensure stop loss respects minimum distance from current price
    if(tradeSignal > 0) { // BUY
        ...
        if(localBid - localStopLoss < localMinDistance) {
            Print("[REAL-EXECUTION] BUY stop loss too close to current price. Adjusting to minimum distance.");
            localStopLoss = localBid - localMinDistance - (localPoint * 5); // Add small buffer
        }
    } else { // SELL
        double localAsk = SymbolInfoDouble(symbol, SYMBOL_ASK);
        if(localStopLoss - localAsk < localMinDistance) {
            Print("[REAL-EXECUTION] SELL stop loss too close to current price. Adjusting to minimum distance.");
            localStopLoss = localAsk + localMinDistance + (localPoint * 5); // Add small buffer
        }
    }
   ...
   // Get pair settings index for capital allocation
   int pairIndex = GetPairSettingsIndex(symbol);
   
   // Apply pair ranking capital allocation
   double capitalAllocationMultiplier = g_pairSettings[pairIndex].scalingFactor;
   
   // Calculate position size based on risk parameters with adaptive allocation
   double riskAmount = AccountInfoDouble(ACCOUNT_BALANCE) * (RiskPercent / 100.0) * capitalAllocationMultiplier;
   
   // Apply correlation risk adjustment to position size
   riskAmount *= correlationRiskMultiplier;
   
   // Calculate position size based on risk and stop loss distance
   double pipValue = SymbolInfoDouble(symbol, SYMBOL_TRADE_TICK_VALUE) * (SymbolInfoDouble(symbol, SYMBOL_POINT) / SymbolInfoDouble(symbol, SYMBOL_TRADE_TICK_SIZE));
   double pipDistance = MathAbs(currentPrice - localStopLoss) / SymbolInfoDouble(symbol, SYMBOL_POINT);
   double tradeVolume = 0.0;
   
   if(pipValue > 0 && pipDistance > 0) {
      tradeVolume = riskAmount / (pipDistance * pipValue);
   } else {
      tradeVolume = CalculatePositionSize(currentPrice, localStopLoss); // Fallback to standard calculation
   }
   ...
   double maxLot = SymbolInfoDouble(symbol, SYMBOL_VOLUME_MAX);
   double lotStep = SymbolInfoDouble(symbol, SYMBOL_VOLUME_STEP);
   tradeVolume = MathFloor(tradeVolume / lotStep) * lotStep;
   tradeVolume = MathMax(minLot, MathMin(maxLot, tradeVolume));
   
   Print("[POSITION-SIZING] Base risk: ", DoubleToString(RiskPercent, 2), "%, Capital allocation: ", 
         DoubleToString(capitalAllocationMultiplier, 2), "x, Correlation adjustment: ", 
         DoubleToString(correlationRiskMultiplier, 2), "x, Final volume: ", DoubleToString(tradeVolume, 2));
...
//+------------------------------------------------------------------+
//| Manage ATR-based trailing stops for all open positions           |
//+------------------------------------------------------------------+
void ManageATRTrailingStops()
{
    // Create trade object for position modifications
    CTrade trade;
    trade.SetExpertMagicNumber(MagicNumber);
    
    // Get current ATR value for adaptive trailing
    double atr = GetATR(Symbol(), Period(), 14, 0);
    
    if(atr <= 0) {
        Print("[TRAILING] Warning: Could not get ATR value for trailing stop calculation");
        return;
    }
    ...
    for(int i = 0; i < PositionsTotal(); i++) {
        ...
        // Skip if position selection fails
        ...
        if(posType == POSITION_TYPE_BUY) {
            profitPips = (currentBid - posOpenPrice) / pipSize;
        } else {
            profitPips = (posOpenPrice - currentAsk) / pipSize;
        }
        
        // Calculate risk in pips (distance from open price to stop loss)
        double riskPips = 0.0;
        if(posStopLoss > 0) {
            if(posType == POSITION_TYPE_BUY) {
                riskPips = (posOpenPrice - posStopLoss) / pipSize;
            } else {
                riskPips = (posStopLoss - posOpenPrice) / pipSize;
            }
        } else {
            // If no stop loss, use ATR as default risk
            riskPips = GetATR(Symbol(), Period(), 14, 0) / pipSize;
        }
        
        // Only activate trailing when profit exceeds activation threshold
        double activationThreshold = riskPips * TrailingActivationPct;
        
        if(profitPips >= activationThreshold) {
            // Calculate new stop loss based on ATR
            double trailingDistance = GetATR(Symbol(), Period(), 14, 0) * TrailingStopATRMultiplier;
            double newStopLoss = 0.0;
            
            // Apply aggressive trailing if enabled
            if(EnableAggressiveTrailing && profitPips >= riskPips) {
                // More aggressive trailing once profit exceeds initial risk
                trailingDistance *= 0.7; // Tighter trailing stop
            }
            
            // Calculate new stop loss position
            if(posType == POSITION_TYPE_BUY) {
                newStopLoss = currentBid - trailingDistance;
                
                // Only move stop loss if it would move it higher (don't move it lower)
                if(newStopLoss > posStopLoss) {
                    // Normalize the stop loss value
                    ...
                    if(trade.PositionModify(ticket, newStopLoss, posTakeProfit)) {
                        if(EnableTrailingDebug) {
                            Print("[TRAILING] Updated BUY stop loss from ", posStopLoss, " to ", newStopLoss, 
                                  " (profit: ", profitPips, " pips, ATR: ", GetATR(Symbol(), Period(), 14, 0) / pipSize, " pips)");
                        }
                    } else {
                        Print("[TRAILING] Failed to update stop loss. Error: ", GetLastError());
                    }
                }
            } else { // SELL position
                newStopLoss = currentAsk + trailingDistance;
                
                // Only move stop loss if it would move it lower (don't move it higher)
                if(newStopLoss < posStopLoss || posStopLoss == 0) {
                    // Normalize the stop loss value
                    newStopLoss = NormalizeDouble(newStopLoss, (int)SymbolInfoInteger(posSymbol, SYMBOL_DIGITS));
                    ...
                    if(trade.PositionModify(ticket, newStopLoss, posTakeProfit)) {
                        if(EnableTrailingDebug) {
                            Print("[TRAILING] Updated SELL stop loss from ", posStopLoss, " to ", newStopLoss, 
                                  " (profit: ", profitPips, " pips, ATR: ", GetATR(Symbol(), Period(), 14, 0) / pipSize, " pips)");
                        }
                    } else {
                        Print("[TRAILING] Failed to update stop loss. Error: ", GetLastError());
        ...
        } else if(EnableTrailingDebug) {
            Print("[TRAILING] Position #", ticket, " profit (", profitPips, " pips) below activation threshold (", 
                  activationThreshold, " pips)");
        }
    }
}
...
void UpdateAdaptivePairSettings()
{
    ...
    for(int i=0; i<g_pairCount; i++) {
        ...
        double atr = GetATR(g_pairSettings[i].symbol, PERIOD_H1, 14, 0);
        g_pairSettings[i].adaptiveAtr = atr; // Make sure adaptiveAtr exists in PairSettings struct
        
        if(VerboseLogging) {
            string symbol = g_pairSettings[i].symbol;
            Print("[ADAPTIVE] Updated settings for ", symbol, ": WinRate=N/A", ", VolMultiplier=", DoubleToString(g_pairSettings[i].adaptiveVolatilityMultiplier,2));
        }
    }
}
...
bool RetryTrade(int signal, double price, double sl, double tp, double size, int maxRetries=3)
{
   ...
   double GetATR(Symbol(), Period(), 14, 0) = 0;
   double atrBuffer[];
   int /* atrHandle removed: use GetATR utility instead */ = iATR(symbol, 0, 14);
   if(/* atrHandle removed: use GetATR utility instead */ != INVALID_HANDLE) {
       int copied = CopyBuffer(/* atrHandle removed: use GetATR utility instead */, 0, 0, 1, atrBuffer);
       if(copied > 0) {
           GetATR(Symbol(), Period(), 14, 0) = atrBuffer[0];
       }
       IndicatorRelease(/* atrHandle removed: use GetATR utility instead */);
   }
   
   // If we couldn't get ATR for some reason, use a reasonable default
   if(GetATR(Symbol(), Period(), 14, 0) <= 0) {
       GetATR(Symbol(), Period(), 14, 0) = 0.0010; // Default to roughly 10 pips for major pairs
   }
   ...
   // For wider spreads, increase the breathing room
   if(spread > GetATR(Symbol(), Period(), 14, 0) * 0.3) { // If spread is more than 30% of ATR
   ...
   double newSL, newTP;
   if(signal > 0) { // Buy
      newSL = adjustedPrice - ((GetATR(Symbol(), Period(), 14, 0) * slMultiplier) * spreadMultiplier);
      newTP = adjustedPrice + ((GetATR(Symbol(), Period(), 14, 0) * tpMultiplier) * spreadMultiplier);
   } else { // Sell
      newSL = adjustedPrice + ((GetATR(Symbol(), Period(), 14, 0) * slMultiplier) * spreadMultiplier);
      newTP = adjustedPrice - ((GetATR(Symbol(), Period(), 14, 0) * tpMultiplier) * spreadMultiplier);
   }
   ...
   Print("[REAL-TRADE] Using ATR-based SL/TP: ATR=", DoubleToString(GetATR(Symbol(), Period(), 14, 0)/_Point, 1), 
                       ...
                       const MqlTradeResult& result)
{
   ...
   if(g_pairSettings[pairIndex].performance.losingTrades > 0 && g_pairSettings[pairIndex].performance.avgLoss != 0) {
    ...
    // Determine pattern type from deal comment
    ENUM_PATTERN_TYPE patternType = PATTERN_UNKNOWN;
    if(StringFind(dealComment, "OB") >= 0) patternType = PATTERN_ORDER_BLOCK;
    else if(StringFind(dealComment, "CHOCH") >= 0) patternType = PATTERN_CHOCH;
    else if(StringFind(dealComment, "BOS") >= 0) patternType = PATTERN_BOS;
    else if(StringFind(dealComment, "FVG") >= 0) patternType = PATTERN_FVG;
    
    // Update pattern weights
    UpdatePatternWeights(symbol, patternType, isWin);
...
void ModifyStopsOnCHOCH(string symbol = NULL)
{
   ...
   double atrValueForChoch = GetATR(Symbol(), Period(), 14, 0);
   // If we couldn't get ATR for some reason, use a reasonable default
   if(atrValueForChoch <= 0) {
      atrValueForChoch = 0.0010; // Default to roughly 10 pips for major pairs
   }
   ...
   for(int i=PositionsTotal()-1; i>=0; i--) {
      ...
      double bufferSize = atrValueForChoch * bufferMultiplier;
      ...
      if(posType == POSITION_TYPE_BUY && !recentCHOCHs[latestCHOCHIndex].isBullish) {
         ...
         if(newSL > currentSL && MathAbs(newSL - currentSL) > atrValueForChoch * 0.2) {
      ...
      // For short positions, use bullish CHOCH to adjust stops
         ...
         if((newSL < currentSL || currentSL == 0) && (currentSL == 0 || MathAbs(newSL - currentSL) > atrValueForChoch * 0.2)) {
...
} // End of CheckDrawdownProtection()
...
void DetectSupplyDemandZones()
{
   ...
   double atrBuffer[];
   ArraySetAsSeries(atrBuffer, true);
   int atrCopied = CopyBuffer(/* atrHandle removed: use GetATR utility instead */, 0, 0, 1, atrBuffer);
   ...
   for(int i=3; i<copied-3; i++) {
      ...
      if(!inDemandZone && 
         ...
         for(int l=0; l<levelCount; l++) {
            if(MathAbs(demandLower - potentialLevels[l]) < GetATR(Symbol(), Period(), 14, 0) * 0.5) {
               strength += 2;
               break;
            }
         }
      ...
      if(!inSupplyZone && 
         ...
         // Increase strength if near a potential level
         for(int l=0; l<levelCount; l++) {
            if(MathAbs(supplyUpper - potentialLevels[l]) < GetATR(Symbol(), Period(), 14, 0) * 0.5) {
               strength += 2;
               break;
            }
         }
...
void DetectFairValueGaps()
{
   ...
   for(int i=2; i<copied-1; i++) {
      ...
      if(rates[i].low > rates[i+1].high) {
         ...
         if(gapSize >= GetATR(Symbol(), Period(), 14, 0) * 0.5) {
            // Find a free slot
            int fvgIndex = -1;
...
//+------------------------------------------------------------------+
/*
// The following GetATR function block (lines ~6306-6332) is obsolete and commented out.
// Its original definition also contained syntax errors (e.g., invalid array declarations).
// double GetATR(string symbol, ENUM_TIMEFRAMES timeframe, int period, int shift = 0, int averagePeriods = 1) {
    ...
    int /* atrHandle removed: use GetATR utility instead */ = iATR(symbol, timeframe, period);
    if(/* atrHandle removed: use GetATR utility instead */ == INVALID_HANDLE) {
    ...
    double GetATR(Symbol(), Period(), 14, 0)s[];
    ArraySetAsSeries(GetATR(Symbol(), Period(), 14, 0)s, true);
    
    // If averagePeriods > 1, we'll calculate an average of ATR values
    int copyCount = shift + averagePeriods;
    int copied = CopyBuffer(/* atrHandle removed: use GetATR utility instead */, 0, shift, copyCount, GetATR(Symbol(), Period(), 14, 0)s);
    ...
    if(averagePeriods > 1) {
        ...
        for(int i=0; i<averagePeriods; i++) {
            sum += GetATR(Symbol(), Period(), 14, 0)s[i];
        }
        return sum / averagePeriods;
    } else {
        // Return single value
        return GetATR(Symbol(), Period(), 14, 0)s[0];
    }
}
...
void UpdateVolatilityContext(string symbol, ENUM_TIMEFRAMES timeframe) {
    // Calculate volatility metrics
    double currentATR = GetATR(symbol, timeframe, 14, 0);
    
    // Calculate moving averages manually since GetATR doesn't have period averaging
    double avg20ATR = 0.0;
    double avg50ATR = 0.0;
    
    // Calculate 20-period average
    for(int i=0; i<20; i++) {
        avg20ATR += GetATR(symbol, timeframe, 14, i);
    }
    avg20ATR /= 20.0;
    
    // Calculate 50-period average
    for(int i=0; i<50; i++) {
        avg50ATR += GetATR(symbol, timeframe, 14, i);
    }
    avg50ATR /= 50.0;
    
    // Previous period ATR
    double prevATR = GetATR(symbol, timeframe, 14, 1);
         ...
         for(int j=1; j<highCount; j++) {
            ...
            if(swingHighTimes[j] < swingLowTimes[i] && j+1 < highCount) {
               ...
               if(swingHighTimes[j+1] > swingLowTimes[i] && swingHighs[j+1] > swingHighs[j]) {
                  ...
                  if(breakStrength > GetATR(Symbol(), Period(), 14, 0)) strength += 2;
                  ...
                  breakerBlocks[breakerIndex].stopLevel = swingLows[i] - (GetATR(Symbol(), Period(), 14, 0) * 0.5);
         ...
         for(int j=1; j<lowCount; j++) {
            ...
            if(swingLowTimes[j] < swingHighTimes[i] && j+1 < lowCount) {
               ...
               if(swingLowTimes[j+1] > swingHighTimes[i] && swingLows[j+1] < swingLows[j]) {
                  ...
                  breakerBlocks[breakerIndex].entryLevel = breakerLevel;
                  breakerBlocks[breakerIndex].stopLevel = swingHighs[i] + (GetATR(Symbol(), Period(), 14, 0) * 0.5);
                  breakerBlocks[breakerIndex].strength = strength;
        ...
        if(EnableRegimeFilters && CurrentRegime >= 0) {
           switch(CurrentRegime) {
              ...
              case REGIME_VOLATILE:
                 trailAmount *= 0.7; // Even tighter trailing in highly volatile markets
                 break;
           }
       }
...
//+------------------------------------------------------------------+
//| Rank and select pairs based on performance metrics                |
//+------------------------------------------------------------------+
void RankAndSelectPairs() {
    // Create array to hold pair scores
    double pairScores[];
    ArrayResize(pairScores, g_pairCount);
    
    // Score each pair
    for(int i = 0; i < g_pairCount; i++) {
        // Base score on performance metrics
        double winRateScore = g_pairSettings[i].performance.winRate * 40;
        double profitFactorScore = g_pairSettings[i].profitFactor * 20;
        double validBlocksScore = g_pairSettings[i].validBlocksCount * 5;
        double chochSuccessScore = g_pairSettings[i].chochSuccessRate * 20;
        
        // Penalize for high spread
        double spreadPenalty = g_pairSettings[i].averageSpread * 10;
        
        // Adjust based on market regime
        double regimeBonus = 0;
        switch(g_pairSettings[i].marketRegime) {
            case REGIME_STRONG_TREND:
                regimeBonus = 15; // Strong bonus for clear trending markets
                break;
            case REGIME_TRENDING:
                regimeBonus = 10; // Good bonus for trending markets
                break;
            case REGIME_RANGING:
                regimeBonus = 5;  // Small bonus for ranging markets (good for order blocks)
                break;
            case REGIME_MIXED:
                regimeBonus = 0;  // No bonus for mixed signals
                break;
            case REGIME_VOLATILE:
                regimeBonus = -5; // Penalty for volatile markets
                break;
        }
        
        // Calculate final score
        pairScores[i] = winRateScore + profitFactorScore + validBlocksScore + chochSuccessScore + regimeBonus - spreadPenalty;
        g_pairSettings[i].rankScore = pairScores[i]; // Store for future reference
        
        // Log the scores
        Print("[RANKING] Pair: ", g_pairSettings[i].symbol, " Score: ", DoubleToString(pairScores[i], 2), 
              " (WinRate: ", DoubleToString(winRateScore, 1),
              ", ProfitFactor: ", DoubleToString(profitFactorScore, 1),
              ", ValidBlocks: ", DoubleToString(validBlocksScore, 1),
              ", CHOCH: ", DoubleToString(chochSuccessScore, 1),
              ", Regime: ", DoubleToString(regimeBonus, 1),
              ", Spread: -", DoubleToString(spreadPenalty, 1), ")");
    }
    
    // Adjust position sizing based on scores
    AdjustPositionSizingByRank(pairScores);
}

//+------------------------------------------------------------------+
//| Adjust position sizing based on pair ranking                      |
//+------------------------------------------------------------------+
void AdjustPositionSizingByRank(double &pairScores[]) {
    // Find the highest and lowest scores
    double maxScore = 0, minScore = DBL_MAX, totalScore = 0;
    
    for(int i = 0; i < g_pairCount; i++) {
        if(pairScores[i] > maxScore) maxScore = pairScores[i];
        if(pairScores[i] < minScore) minScore = pairScores[i];
        totalScore += MathMax(0, pairScores[i]); // Only count positive scores
    }
    
    // Normalize scores and allocate capital
    for(int i = 0; i < g_pairCount; i++) {
        // Ensure we don't divide by zero
        if(maxScore == minScore || totalScore <= 0) {
            g_pairSettings[i].capitalAllocation = 1.0 / g_pairCount; // Equal allocation
        } else {
            // Normalize to 0-1 range and ensure no negative allocations
            double normalizedScore = MathMax(0, pairScores[i]) / totalScore;
            
            // Allocate capital based on normalized score
            // Use a weighted approach: 50% equal allocation, 50% performance-based
            g_pairSettings[i].capitalAllocation = 0.5 * (1.0 / g_pairCount) + 0.5 * normalizedScore;
            
            // Cap maximum allocation at 25% for diversification
            g_pairSettings[i].capitalAllocation = MathMin(g_pairSettings[i].capitalAllocation, 0.25);
        }
        
        // Apply the allocation to the scaling factor for position sizing
        g_pairSettings[i].scalingFactor = 1.0 + (g_pairSettings[i].capitalAllocation * 2.0 - 1.0);
        
        Print("[ALLOCATION] Pair: ", g_pairSettings[i].symbol, 
              " Capital: ", DoubleToString(g_pairSettings[i].capitalAllocation * 100, 1), "%", 
              " Scaling: ", DoubleToString(g_pairSettings[i].scalingFactor, 2));
    }
}

//+------------------------------------------------------------------+
//| Update pattern weights based on trade outcomes                    |
//+------------------------------------------------------------------+
void UpdatePatternWeights(string symbol, ENUM_PATTERN_TYPE patternType, bool wasProfit) {
    // Get pair settings index
    int pairIndex = GetPairSettingsIndex(symbol);
    
    // Learning rate - how quickly we adjust weights
    double learningRate = 0.05; // 5% adjustment per trade
    
    // Update the specific pattern weight
    switch(patternType) {
        case PATTERN_ORDER_BLOCK:
            if(wasProfit) {
                // Increase weight for successful pattern
                g_pairSettings[pairIndex].patternWeights.orderBlockWeight += 
                    learningRate * (1.0 - g_pairSettings[pairIndex].patternWeights.orderBlockWeight);
            } else {
                // Decrease weight for unsuccessful pattern
                g_pairSettings[pairIndex].patternWeights.orderBlockWeight -= 
                    learningRate * g_pairSettings[pairIndex].patternWeights.orderBlockWeight;
            }
            // Ensure weight stays in valid range
            g_pairSettings[pairIndex].patternWeights.orderBlockWeight = 
                MathMax(0.1, MathMin(1.0, g_pairSettings[pairIndex].patternWeights.orderBlockWeight));
            break;
            
        case PATTERN_CHOCH:
            if(wasProfit) {
                g_pairSettings[pairIndex].patternWeights.chochWeight += 
                    learningRate * (1.0 - g_pairSettings[pairIndex].patternWeights.chochWeight);
            } else {
                g_pairSettings[pairIndex].patternWeights.chochWeight -= 
                    learningRate * g_pairSettings[pairIndex].patternWeights.chochWeight;
            }
            g_pairSettings[pairIndex].patternWeights.chochWeight = 
                MathMax(0.1, MathMin(1.0, g_pairSettings[pairIndex].patternWeights.chochWeight));
            break;
            
        case PATTERN_BOS:
            if(wasProfit) {
                g_pairSettings[pairIndex].patternWeights.bosWeight += 
                    learningRate * (1.0 - g_pairSettings[pairIndex].patternWeights.bosWeight);
            } else {
                g_pairSettings[pairIndex].patternWeights.bosWeight -= 
                    learningRate * g_pairSettings[pairIndex].patternWeights.bosWeight;
            }
            g_pairSettings[pairIndex].patternWeights.bosWeight = 
                MathMax(0.1, MathMin(1.0, g_pairSettings[pairIndex].patternWeights.bosWeight));
            break;
            
        case PATTERN_FVG:
            if(wasProfit) {
                g_pairSettings[pairIndex].patternWeights.fvgWeight += 
                    learningRate * (1.0 - g_pairSettings[pairIndex].patternWeights.fvgWeight);
            } else {
                g_pairSettings[pairIndex].patternWeights.fvgWeight -= 
                    learningRate * g_pairSettings[pairIndex].patternWeights.fvgWeight;
            }
            g_pairSettings[pairIndex].patternWeights.fvgWeight = 
                MathMax(0.1, MathMin(1.0, g_pairSettings[pairIndex].patternWeights.fvgWeight));
            break;
    }
    
    // Log the updated weights
    Print("[PATTERN-LEARNING] Updated weights for ", symbol, ": ",
          "OB=", DoubleToString(g_pairSettings[pairIndex].patternWeights.orderBlockWeight, 2), ", ",
          "CHOCH=", DoubleToString(g_pairSettings[pairIndex].patternWeights.chochWeight, 2), ", ",
          "BOS=", DoubleToString(g_pairSettings[pairIndex].patternWeights.bosWeight, 2), ", ",
          "FVG=", DoubleToString(g_pairSettings[pairIndex].patternWeights.fvgWeight, 2));
}

//+------------------------------------------------------------------+
//| Check correlation risk between pairs                              |
//+------------------------------------------------------------------+
double CheckCorrelationRisk(string symbol, int direction) {
    // Get pair settings index
    int pairIndex = GetPairSettingsIndex(symbol);
    
    // Default position size multiplier (no reduction)
    double positionSizeMultiplier = 1.0;
    
    // Count positions in same and opposite direction
    int sameDirectionCount = 0;
    int oppositeDirectionCount = 0;
    double sameDirectionExposure = 0.0;
    double oppositeDirectionExposure = 0.0;
    
    // Track correlated pairs with existing positions
    int correlatedPairsCount = 0;
    ...
    for(int i = 0; i < PositionsTotal(); i++) {
        ...
        string posSymbol = PositionGetString(POSITION_SYMBOL);
        int posDirection = (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) ? 1 : -1;
        double posVolume = PositionGetDouble(POSITION_VOLUME);
        
        // Skip if it's the same symbol
        if(posSymbol == symbol) continue;
        
        // Get correlation between this pair and the position symbol
        int posPairIndex = GetPairSettingsIndex(posSymbol);
        double correlation = g_pairSettings[pairIndex].correlationMatrix[posPairIndex];
        
        // Only consider significant correlations (positive or negative)
        if(MathAbs(correlation) >= 0.5) {
            correlatedPairsCount++;
            totalCorrelation += MathAbs(correlation);
            
            // Determine effective direction based on correlation
            int effectiveDirection = posDirection;
            if(correlation < 0) effectiveDirection *= -1; // Negative correlation inverts direction
            
            // Count positions in same or opposite direction
            if(effectiveDirection == direction) {
                sameDirectionCount++;
                sameDirectionExposure += posVolume * MathAbs(correlation);
            } else {
                oppositeDirectionCount++;
                oppositeDirectionExposure += posVolume * MathAbs(correlation);
            }
        }
    }
    
    // Calculate risk adjustment based on correlation
    double localCorrelationRiskMultiplier = 1.0;
    if(correlatedPairsCount > 0) {
        // Average correlation of correlated pairs
        double avgCorrelation = totalCorrelation / correlatedPairsCount;
        
        // Reduce risk based on correlation and same direction exposure
        localCorrelationRiskMultiplier = 1.0 - (avgCorrelation * g_correlationRiskMultiplier * sameDirectionExposure);
    }
    
    // Reduce position size based on correlated exposure in same direction
    if(sameDirectionCount > 0 && sameDirectionExposure > 0) {
        // Reduce position size more aggressively as correlated exposure increases
        double reductionFactor = MathMin(0.8, sameDirectionExposure * avgCorrelation * 0.2);
        positionSizeMultiplier *= (1.0 - reductionFactor);
        
        Print("[CORRELATION] Reducing position size by ", DoubleToString(reductionFactor * 100, 1), 
              "% due to ", sameDirectionCount, " correlated positions in same direction");
    }
    
    // Increase position size slightly if we have opposite correlated exposure (hedging effect)
    if(oppositeDirectionCount > 0 && oppositeDirectionExposure > 0) {
        // Small bonus for natural hedging, max 20% increase
        double bonusFactor = MathMin(0.2, oppositeDirectionExposure * avgCorrelation * 0.1);
        positionSizeMultiplier *= (1.0 + bonusFactor);
        
        Print("[CORRELATION] Increasing position size by ", DoubleToString(bonusFactor * 100, 1), 
              "% due to ", oppositeDirectionCount, " correlated positions in opposite direction");
    }
    
    // Ensure multiplier stays within reasonable bounds
    positionSizeMultiplier = MathMax(0.2, MathMin(1.2, positionSizeMultiplier));
    
    return positionSizeMultiplier;
}

//+------------------------------------------------------------------+
//| Validate order block with nuanced criteria                        |
//+------------------------------------------------------------------+
bool IsOrderBlockValid(string symbol, double price, datetime blockTime, int strength, bool isBullish) {
    // Get pair settings index
    int pairIndex = GetPairSettingsIndex(symbol);
    ...
    // Get current time and calculate block age
    datetime currentTime = TimeCurrent();
    double blockAgeHours = (currentTime - blockTime) / 3600.0;
    
    // Get ATR value for volatility context
    double GetATR(Symbol(), Period(), 14, 0) = 0.0;
    int /* atrHandle removed: use GetATR utility instead */ = iATR(symbol, PERIOD_CURRENT, 14);
    if(/* atrHandle removed: use GetATR utility instead */ != INVALID_HANDLE) {
        double atrBuffer[];
        ArraySetAsSeries(atrBuffer, true);
        if(CopyBuffer(/* atrHandle removed: use GetATR utility instead */, 0, 0, 1, atrBuffer) > 0) {
            GetATR(Symbol(), Period(), 14, 0) = atrBuffer[0];
        }
        IndicatorRelease(/* atrHandle removed: use GetATR utility instead */);
    }
    ...
    // 1. Check block strength against adaptive threshold
    if(strength < adaptiveThreshold) {
        Print("[BLOCK-INVALID] ", symbol, " Block strength ", IntegerToString(strength), 
              " below threshold ", DoubleToString(adaptiveThreshold, 1));
        return false;
    }
    
    // 2. Check block age - more permissive for high-value assets
    double maxAgeHours = g_pairSettings[pairIndex].orderBlockAgeHours;
    
    // Adjust max age based on volatility
    if(g_volatilityContext.volatilityState == VOLATILITY_LOW || 
       g_volatilityContext.volatilityState == VOLATILITY_VERY_LOW) {
        // Extend block lifetime during low volatility periods
        maxAgeHours *= 1.5;
        Print("[BLOCK-AGE] Extended max age to ", DoubleToString(maxAgeHours, 1), 
              " hours due to low volatility");
    }
    
    // Check block age against adjusted max age
    if(blockAgeHours > maxAgeHours) {
        Print("[BLOCK-INVALID] ", symbol, " Block age ", DoubleToString(blockAgeHours, 1), 
              " hours exceeds max ", DoubleToString(maxAgeHours, 1), " hours");
        return false;
    }
    
    // 3. Check if price has already breached the block
    double currentPrice = isBullish ? 
        SymbolInfoDouble(symbol, SYMBOL_BID) : 
        SymbolInfoDouble(symbol, SYMBOL_ASK);
    
    bool priceBreached = isBullish ? 
        (currentPrice < price - (GetATR(Symbol(), Period(), 14, 0) * 0.5)) : 
        (currentPrice > price + (GetATR(Symbol(), Period(), 14, 0) * 0.5));
    
    if(priceBreached) {
        Print("[BLOCK-INVALID] ", symbol, " Price has breached the block");
        return false;
    }
    
    // 4. Check block quality based on pattern weights
    double patternQuality = 0.5; // Default quality
    
    // Reduce quality score as block ages
    double ageQualityFactor = 1.0 - (blockAgeHours / maxAgeHours) * 0.5; // At max age, quality is reduced by 50%
    patternQuality *= ageQualityFactor;
    
    // Minimum quality threshold
    if(patternQuality < 0.4) { // 40% minimum quality threshold
        Print("[BLOCK-INVALID] ", symbol, " Block quality too low: ", 
              DoubleToString(patternQuality, 2), " (age factor: ", DoubleToString(ageQualityFactor, 2), ")");
        return false;
    }
    
    // Check if block has been tested too many times (using a static counter for this implementation)
    static int testCount = 0;
    int maxTests = isHighValue ? 3 : 2; // More permissive for high-value assets
    
    if(testCount >= maxTests) {
        Print("[BLOCK-INVALID] ", symbol, " Block tested too many times: ", testCount);
        return false;
    }
    
    // Increment test count for this block
    testCount++;
    
    // Get pair settings index for tracking valid blocks
    int pairIndex = GetPairSettingsIndex(symbol);
    
    // Increment valid blocks count for this pair
    if(pairIndex >= 0) {
        g_pairSettings[pairIndex].validBlocksCount++;
    }
    
    // Block passes all validation criteria
    Print("[BLOCK-VALID] ", symbol, " Valid block with strength ", IntegerToString(strength), 
          ", age ", DoubleToString(blockAgeHours, 1), " hours, quality ", 
          DoubleToString(patternQuality, 2), ", test count: ", testCount);
    
    return true;
}

//+------------------------------------------------------------------+
//| Update market regime classification across timeframes              |
//+------------------------------------------------------------------+
void UpdateMarketRegimeClassification(string symbol) {
    // Get pair settings index
    int pairIndex = GetPairSettingsIndex(symbol);
    
    // Timeframes to analyze (from short-term to long-term)
    ...
    // Arrays to store regime classification for each timeframe
    ENUM_MARKET_REGIME regimes[];
    ArrayResize(regimes, timeframeCount);
    
    // Analyze each timeframe
    for(int i = 0; i < timeframeCount; i++) {
        // Detect trend direction and strength for this timeframe
        regimes[i] = DetectMarketRegime(symbol, timeframes[i]);
    }
    
    // Count regime types across timeframes
    int strongTrendCount = 0;
    int trendingCount = 0;
    int rangingCount = 0;
    int volatileCount = 0;
    int mixedCount = 0;
    ...
    for(int i = 0; i < timeframeCount; i++) {
        switch(regimes[i]) {
            case REGIME_STRONG_TREND: strongTrendCount++; break;
            case REGIME_TRENDING:     trendingCount++; break;
            case REGIME_RANGING:      rangingCount++; break;
            case REGIME_VOLATILE:     volatileCount++; break;
            case REGIME_MIXED:        mixedCount++; break;
        }
    }
    
    // Determine the dominant regime
    ENUM_MARKET_REGIME dominantRegime = REGIME_MIXED; // Default
    
    // Strong trend takes precedence if present in multiple timeframes
    if(strongTrendCount >= 2) {
        dominantRegime = REGIME_STRONG_TREND;
    }
    // General trending market if trending in multiple timeframes
    else if(trendingCount + strongTrendCount >= 2) {
        dominantRegime = REGIME_TRENDING;
    }
    // Ranging market if ranging in multiple timeframes
    else if(rangingCount >= 2) {
        dominantRegime = REGIME_RANGING;
    }
    // Volatile market if volatile in any timeframe
    else if(volatileCount > 0) {
        dominantRegime = REGIME_VOLATILE;
    }
    // Otherwise mixed/unclear regime
    
    // Store the dominant regime
    g_pairSettings[pairIndex].marketRegime = dominantRegime;
    
    // Log the regime classification
    string regimeNames[] = {"Strong Trend", "Trending", "Ranging", "Mixed", "Volatile"};
    
    Print("[REGIME] ", symbol, " Market Regime: ", regimeNames[dominantRegime], 
          " (ST:", strongTrendCount, ", T:", trendingCount, ", R:", rangingCount, 
          ", M:", mixedCount, ", V:", volatileCount, ")");
    
    // Adjust trading parameters based on market regime
    AdjustParametersForRegime(symbol, dominantRegime);
}

//+------------------------------------------------------------------+
//| Detect market regime for a specific timeframe                     |
//+------------------------------------------------------------------+
ENUM_MARKET_REGIME DetectMarketRegime(string symbol, ENUM_TIMEFRAMES timeframe) {
    // Get price data
    MqlRates rates[];
    ArraySetAsSeries(rates, true);
    ...
    if(copied < 100) {
        Print("[REGIME-ERROR] Failed to copy rates data for ", symbol, " timeframe ", EnumToString(timeframe));
        return REGIME_MIXED; // Default if not enough data
    }
    
    // Calculate indicators
    double atr = GetATR(symbol, timeframe, 14, 0, 1);      // Current ATR
    double atrAvg = GetATR(symbol, timeframe, 14, 0, 20);  // 20-period average ATR
    ...
    if(adxHandle == INVALID_HANDLE) {
        Print("[REGIME-ERROR] Failed to create ADX indicator handle");
        return REGIME_MIXED;
    }
    
    if(CopyBuffer(adxHandle, 0, 0, 10, adxMain) <= 0 ||
       CopyBuffer(adxHandle, 1, 0, 10, plusDI) <= 0 ||
       CopyBuffer(adxHandle, 2, 0, 10, minusDI) <= 0) {
        Print("[REGIME-ERROR] Failed to copy ADX buffer data");
    ...
    int upBars = 0, downBars = 0, dojiBars = 0;
    for(int i = 0; i < 20; i++) {
        double bodySize = MathAbs(rates[i].close - rates[i].open);
        double totalSize = rates[i].high - rates[i].low;
        
        if(bodySize < totalSize * 0.2) { // Small body relative to total range
            dojiBars++;
    ...
    bool isUptrend = rates[0].close > rates[20].close && upBars > downBars;
    bool isDowntrend = rates[0].close < rates[20].close && downBars > upBars;
    
    // Check for strong trend
    bool isStrongTrend = adxMain[0] > 25 && (isUptrend || isDowntrend);
    
    // Check for ranging market
    bool isRanging = MathAbs(rates[0].close - rates[20].close) < atrAvg * 2 && dojiBars >= 5;
    
    // Check for volatility
    bool isVolatile = atr > atrAvg * 1.5;
...
//| Adjust trading parameters based on market regime                  |
//+------------------------------------------------------------------+
void AdjustParametersForRegime(string symbol, ENUM_MARKET_REGIME regime) {
    // Get pair settings index
    int pairIndex = GetPairSettingsIndex(symbol);
    ...
    switch(regime) {
        case REGIME_STRONG_TREND:
            // In strong trends, focus on trend continuation patterns
            g_pairSettings[pairIndex].patternWeights.bosWeight *= 1.2;     // Boost breakout of structure
            g_pairSettings[pairIndex].patternWeights.chochWeight *= 1.2;   // Boost change of character
            g_pairSettings[pairIndex].patternWeights.orderBlockWeight *= 0.8; // Reduce order blocks
            
            // Extend profit targets in trending markets
            g_pairSettings[pairIndex].tpMultiplier = 1.5;
            g_pairSettings[pairIndex].slMultiplier = 1.0;
            break;
            
        case REGIME_TRENDING:
            // In normal trends, balanced approach
            g_pairSettings[pairIndex].patternWeights.bosWeight *= 1.1;
            g_pairSettings[pairIndex].patternWeights.chochWeight *= 1.1;
            g_pairSettings[pairIndex].patternWeights.orderBlockWeight *= 0.9;
            
            g_pairSettings[pairIndex].tpMultiplier = 1.3;
            g_pairSettings[pairIndex].slMultiplier = 1.0;
            break;
            
        case REGIME_RANGING:
            // In ranging markets, focus on reversal patterns
            g_pairSettings[pairIndex].patternWeights.orderBlockWeight *= 1.2; // Boost order blocks
            g_pairSettings[pairIndex].patternWeights.fvgWeight *= 1.2;       // Boost fair value gaps
            g_pairSettings[pairIndex].patternWeights.bosWeight *= 0.8;       // Reduce breakouts
            
            // Tighter targets in ranging markets
            g_pairSettings[pairIndex].tpMultiplier = 1.0;
            g_pairSettings[pairIndex].slMultiplier = 1.0;
            break;
            
        case REGIME_VOLATILE:
            // In volatile markets, be more conservative
            g_pairSettings[pairIndex].patternWeights.orderBlockWeight *= 0.8;
            g_pairSettings[pairIndex].patternWeights.chochWeight *= 0.8;
            g_pairSettings[pairIndex].patternWeights.bosWeight *= 0.8;
            g_pairSettings[pairIndex].patternWeights.fvgWeight *= 0.8;
            
            // Wider stops, smaller targets in volatile markets
            g_pairSettings[pairIndex].tpMultiplier = 0.9;
            g_pairSettings[pairIndex].slMultiplier = 1.3;
            break;
            
        case REGIME_MIXED:
        default:
            // In mixed/unclear markets, use balanced settings
            g_pairSettings[pairIndex].patternWeights.orderBlockWeight = 1.0;
            g_pairSettings[pairIndex].patternWeights.chochWeight = 1.0;
            g_pairSettings[pairIndex].patternWeights.bosWeight = 1.0;
            g_pairSettings[pairIndex].patternWeights.fvgWeight = 1.0;
    ...
    g_pairSettings[pairIndex].patternWeights.orderBlockWeight = 
        MathMax(0.1, MathMin(1.5, g_pairSettings[pairIndex].patternWeights.orderBlockWeight));
    g_pairSettings[pairIndex].patternWeights.chochWeight = 
        MathMax(0.1, MathMin(1.5, g_pairSettings[pairIndex].patternWeights.chochWeight));
    g_pairSettings[pairIndex].patternWeights.bosWeight = 
        MathMax(0.1, MathMin(1.5, g_pairSettings[pairIndex].patternWeights.bosWeight));
    g_pairSettings[pairIndex].patternWeights.fvgWeight = 
        MathMax(0.1, MathMin(1.5, g_pairSettings[pairIndex].patternWeights.fvgWeight));
}
...
void OnTick()
{
   ...
   // Enhanced order block validation using our new function
    int validBuyBlocks = 0, validSellBlocks = 0;
    for(int i=0; i<ArraySize(recentBlocks); i++) {
       // Re-validate blocks using our new comprehensive validation function
       if(recentBlocks[i].time > 0) { // Only process blocks that have been initialized
          bool wasValid = recentBlocks[i].valid;
          recentBlocks[i].valid = IsOrderBlockValid(Symbol(), 
                                                  recentBlocks[i].isBuy ? recentBlocks[i].low : recentBlocks[i].high,
                                                  recentBlocks[i].time,
                                                  recentBlocks[i].strength,
                                                  recentBlocks[i].isBuy);
          
          // Log validation changes
          if(wasValid && !recentBlocks[i].valid) {
             Print("[BLOCK-VALIDATION] Block at ", TimeToString(recentBlocks[i].time), 
                   " was invalidated by enhanced validation criteria");
          }
          else if(!wasValid && recentBlocks[i].valid) {
             Print("[BLOCK-VALIDATION] Block at ", TimeToString(recentBlocks[i].time), 
                   " was validated by enhanced validation criteria");
          }
   ...
   // Update market regime classification and pair ranking periodically
   static datetime lastRegimeUpdate = 0;
   if(TimeCurrent() - lastRegimeUpdate > 3600) { // Update every hour
       UpdateMarketRegimeClassification(Symbol());
       RankAndSelectPairs(); // Rank pairs and adjust capital allocation
       lastRegimeUpdate = TimeCurrent();
       Print("[ADAPTIVE] Updated market regime classification and pair ranking");
   }
   ...
   if(canTradeNow) {
      ...
      if(bestBuyBlockIndex >= 0) {
         ...
         if(MathAbs(currentPrice - blockPrice) < 5 * _Point) {
            ...
            double stopLoss = blockPrice - (GetATR(Symbol(), Period(), 14, 0) * 1.5);
   ...
   // Test trade execution with enhanced error handling (shortened for testing)
     ...
     // Manage ATR-based trailing stops for open positions (alternative implementation)
     if(TrailingStopEnabled && !EnableAggressiveTrailing) {
         ManageATRTrailingStops();
     } else if(TrailingStopEnabled) {
         ManageTrailingStops(); // Use existing implementation when aggressive trailing is enabled
     }
...
void ManageOpenTrade(void)
{
   ...
   double atrBuffer[];
   ArraySetAsSeries(atrBuffer, true);
   CopyBuffer(/* atrHandle removed: use GetATR utility instead */, 0, 0, 1, atrBuffer);
   double GetATR(Symbol(), Period(), 14, 0) = (ArraySize(atrBuffer) > 0) ? atrBuffer[0] : 0.01;
...
void StorePositionForReentry(ulong positionId, double stopPrice, ENUM_POSITION_TYPE posType)
{
   ...
   if(posType == POSITION_TYPE_BUY) {
      ReentryStops[idx] = stopPrice - GetATR(Symbol(), Period(), 14, 0) * atrMultiplier;
      ReentryTargets[idx] = stopPrice + GetATR(Symbol(), Period(), 14, 0) * 2 * atrMultiplier;
   } else {
      ReentryStops[idx] = stopPrice + GetATR(Symbol(), Period(), 14, 0) * atrMultiplier;
      ReentryTargets[idx] = stopPrice - GetATR(Symbol(), Period(), 14, 0) * 2 * atrMultiplier;
   }
...
bool IsSpreadAcceptable()
{
   ...
   double GetATR(Symbol(), Period(), 14, 0) = atrBuffer[0];
   
   // Set dynamic threshold based on ATR
   double maxAcceptableSpread = GetATR(Symbol(), Period(), 14, 0) * 0.25; // Default 25% of ATR
   ...
   if(isHighValueAsset) {
      ...
      maxAcceptableSpread = GetATR(Symbol(), Period(), 14, 0) * 0.625; // 2.5x the normal threshold
...
double GetCorrelatedExposure()
{
   ...
   for(int i=0; i<PositionsTotal(); i++) {
      ...
      if(isCorrelated) {
         ...
         // Get pair settings index for capital allocation
         int pairIndex = GetPairSettingsIndex(posSymbol);
         ...
         double pipValue = SymbolInfoDouble(posSymbol, SYMBOL_TRADE_TICK_VALUE) * (SymbolInfoDouble(posSymbol, SYMBOL_POINT) / SymbolInfoDouble(posSymbol, SYMBOL_TRADE_TICK_SIZE));
         double pipDistance = MathAbs(posOpenPrice - posStopLoss) / SymbolInfoDouble(posSymbol, SYMBOL_POINT);
         double positionSize = 0.0;
   
         Print("[POSITION-SIZING] Base risk: ", DoubleToString(RiskPercent, 2), "%, Capital allocation: ", 
               DoubleToString(capitalAllocationMultiplier, 2), "x, Correlation adjustment: ", 
               DoubleToString(correlationRiskMultiplier, 2), "x");
         ...
         double riskAmountInMoney = riskPoints * pointValue * posVolume;
   
         // Apply correlation factor
         totalExposure += riskAmountInMoney * correlationFactor;
      }
   }
...
void CheckDrawdownProtection()
{
   ...
   for(int i=0; i<totalDeals; i++) {
      ...
      if(dealProfit < 0) todayLoss += MathAbs(dealProfit);
   }
   
   // Calculate maximum daily loss
   double todayMaxLoss = AccountInfoDouble(ACCOUNT_BALANCE) * MaxDailyRiskPercent / 100.0;
   
   // Check if daily loss exceeds maximum
   if(todayLoss > todayMaxLoss) {
      Print("[DRAWDOWN] Daily loss limit reached ($", todayLoss, "). Trading paused for today.");
      ::TradingPaused = true;
      return;
   }
...
ENUM_MARKET_REGIME DetectMarketRegime()
{
   ...
   double atrBuffer[];
   ArraySetAsSeries(atrBuffer, true);
   // Use GetATR function instead of direct handle
   for(int i=0; i<VolatilityLookback + 10; i++) {
      ArrayResize(atrBuffer, i+1);
      atrBuffer[i] = GetATR(Symbol(), PERIOD_CURRENT, 14, i);
   }
   ...
   if(AdaptToVolatility) {
      switch(regime) {
         ...
         case REGIME_TRENDING: 
         case REGIME_STRONG_TREND: VolatilityMultiplier = 1.2; break; // Increase in trending markets
...
//| Check if the current spread is acceptable for trading          |
//+------------------------------------------------------------------+
// COMMENTED OUT: This function is already defined elsewhere
/*
bool IsSpreadAcceptable(string symbol = NULL)
{
   // Use current symbol if none provided
   ...
   // Get ATR directly using the GetATR utility function
   double atr = GetATR(symbol, PERIOD_CURRENT, 14, 0);
   
   // If we couldn't get ATR for some reason, use a reasonable default
   if(atr <= 0) {
      atr = 0.0010; // Default to roughly 10 pips for major pairs
   }
   
   // Compare spread to ATR
   double spreadToATR = currentSpread / atr;
   ...
   if(EnableSafetyLogging) {
      Print("[SPREAD] Symbol: ", symbol, ", Spread: ", DoubleToString(currentSpread/_Point, 1),
            " pips, ATR: ", DoubleToString(atr/_Point, 1), " pips, Ratio: ",
...
*/
   ...
   // --- BEGIN: Commented-out duplicate drawdown protection logic (should be in function body) ---
   /*
   ...
   // }
   */
   // --- END: Commented-out duplicate drawdown protection logic ---
