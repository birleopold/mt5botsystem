    ...
    if(signal != 0 && signalQuality >= minSignalQuality) {
        ...
        if(stopLoss > 0 && posSize > 0) {
            ...
            if(!hasExistingOrder) {
                ...
                tradeResult = ExecuteTradeWithRetry(signal, entryPrice, stopLoss, takeProfit, posSize, comment);
...
//+------------------------------------------------------------------+
double GetATRStop(int period=14) {
    // Get handle to ATR indicator
    int atrHandle = iATR(Symbol(), PERIOD_M15, period);
    
    // Get ATR value from the indicator
    double atrBuffer[];
    ArraySetAsSeries(atrBuffer, true);
    CopyBuffer(atrHandle, 0, 0, 1, atrBuffer);
    
    // Apply multiplier from input parameter SL_ATR_Mult
    return atrBuffer[0] * SL_ATR_Mult;
}
